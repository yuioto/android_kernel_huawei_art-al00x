...
static const struct xt_option_entry owner_mt_opts[] = {
	...
	{.name = "socket-exists", .id = O_SOCK_EXISTS, .type = XTTYPE_NONE,
	 .flags = XTOPT_INVERT},
	{.name = "pid-owner", .id = O_PROCESS, .type = XTTYPE_STRING,
	 .flags = XTOPT_INVERT},
...
static void owner_mt_check(struct xt_fcheck_call *cb)
{
	if (cb->xflags == 0)
		xtables_error(PARAMETER_PROBLEM, "owner: At least one of "
		           "--uid-owner, --gid-owner, --pid-owner or --socket-exists "
		           "is required");
}
                    ...
                    uint8_t flag, bool numeric)
{
	...
	case XT_OWNER_PID:
		if (info->gid_min != info->gid_max) {
			printf(" %u-%u", (unsigned int)info->gid_min, (unsigned int)info->gid_max);
			break;
		}
		printf(" %u", (unsigned int)info->gid_min);
		break;
...
static int owner_mt_print_pid_xlate(const struct xt_owner_match_info *info,
	struct xt_xlate *xl)
{
	xt_xlate_add(xl, "skpid%s ", info->invert ? " !=" : "");

	if (info->gid_min != info->gid_max)
		xt_xlate_add(xl, "%u-%u", (unsigned int)info->gid_min, (unsigned int)info->gid_max);
	else
		xt_xlate_add(xl, "%u", (unsigned int)info->gid_min);
			  ...
			  const struct xt_xlate_mt_params *params)
{
	...
	case XT_OWNER_GID:
		...
		break;
	case XT_OWNER_PID:
		ret = owner_mt_print_pid_xlate(info, xl);
		break;
