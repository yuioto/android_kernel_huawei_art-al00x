...
static int mipi_tm_panel_set_backlight(struct platform_device *pdev, uint32_t bl_level)
{
	...
	if (dpufd->panel_info.bl_set_type & BL_SET_BY_PWM) {
		...
		bl_cmd[0].payload[1] = bl_level * 255 / dpufd->panel_info.bl_max;

		mipi_dsi_cmds_tx(bl_cmd, ARRAY_SIZE(bl_cmd), dpufd->mipi_dsi0_base);
	} else {
...
static int mipi_tm_panel_off(struct platform_device *pdev)
{
	...
	set_reg(dpufd->mipi_dsi0_base + MIPIDSI_CMD_MODE_CFG_OFFSET, 0x7f, 7, 8);
	set_reg(dpufd->mipi_dsi0_base + MIPIDSI_CMD_MODE_CFG_OFFSET, 0xf, 4, 16);
	set_reg(dpufd->mipi_dsi0_base + MIPIDSI_CMD_MODE_CFG_OFFSET, 0x1, 1, 24);
...
static int mipi_jdi_panel_lcd_fps_scence_handle(struct platform_device *pdev, uint32_t scence)
{
	...
	switch(scence){
		case LCD_FPS_SCENCE_IDLE:
			...
			DPU_FB_DEBUG("scence is LCD_FPS_SCENCE_IDLE, framerate is 30fps\n");
			break;
		case LCD_FPS_SCENCE_NORMAL:
			pinfo->fps_updt = LCD_FPS_60;
			DPU_FB_DEBUG("scence is LCD_FPS_SCENCE_NORMAL, framerate is 60fps\n");
			break;
		case LCD_FPS_SCENCE_VIDEO:
			DPU_FB_INFO("disable drf\n");
		...
		case LCD_FPS_SCENCE_FORCE_30FPS:
			...
			DPU_FB_INFO("scence is LCD_FPS_SCENCE_IDLE_FORCE_ENABLE, framerate is 30fps\n");
			break;
		default:
			pinfo->fps_updt = LCD_FPS_60;
			DPU_FB_DEBUG("scence is LCD_FPS_SCENCE_NORMAL, framerate is 60fps\n");
...
static void mipi_tm_fpga_probe_config(struct dpu_panel_info *pinfo, struct device_node *np)
{
	if (g_lcd_fpga_flag == 1) {
		...
		if (np != NULL) {
			...
			DPU_FB_INFO("vcc1v8=%d, p5v5=%d, n5v5=%d, reset=%d, bl=%d, vcc2v8=%d\n",
