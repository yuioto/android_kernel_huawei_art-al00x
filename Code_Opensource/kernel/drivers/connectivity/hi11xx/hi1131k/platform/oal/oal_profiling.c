...
{
    memset_s(&g_mips_tx_statistic, OAL_SIZEOF(oal_profiling_tx_statistic_stru), 0,
             OAL_SIZEOF(oal_profiling_tx_statistic_stru));
...
oal_void oal_profiling_tx_save_data(oal_profiling_tx_func_enum_uint8 en_func_index)
{
    ...
    g_mips_tx_statistic.ast_tx_pass_cycles[0][en_func_index] = cpu_clock(UINT_MAX);//lint !e569
}
...
oal_void oal_profiling_tx_mips_show(oal_void)
{
    ...
    for (ul_packet_idx = 0; ul_packet_idx < OAL_MIPS_TX_PACKET_MAX_COUNT; ul_packet_idx++) {
        ...
        for (ul_loop = HMAC_PROFILING_FUNC_BRIDGE_VAP_XMIT; \
            ul_loop <= HMAC_PROFILING_FUNC_TX_EVENT_TO_DMAC; ul_loop++) {
...
{
    memset_s(&g_mips_rx_statistic, OAL_SIZEOF(oal_profiling_rx_statistic_stru), 0,
             OAL_SIZEOF(oal_profiling_rx_statistic_stru));
...
oal_void oal_profiling_rx_save_data(oal_profiling_rx_func_enum_uint8 en_func_index)
{
    ...
    g_mips_rx_statistic.ast_rx_pass_cycles[0][en_func_index] = cpu_clock(UINT_MAX);//lint !e569
}
