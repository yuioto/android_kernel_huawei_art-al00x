...
oal_uint16 oal_netbuf_select_queue(oal_netbuf_stru *pst_buf)
{
    ...
    switch (pst_ether_header->us_ether_type) {
        ...
        case OAL_HOST2NET_SHORT(ETHER_TYPE_IPV6):
            ...
            break;
        case OAL_HOST2NET_SHORT(ETHER_TYPE_PAE): /* 如果是EAPOL帧，则进入VO队列发送 */
        case OAL_HOST2NET_SHORT(ETHER_TYPE_TDLS): /* TDLS帧处理，建链保护，入高优先级TID队列 */
            us_queue = WLAN_DATA_VIP_QUEUE;
            break;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 44))// 当前内核没有portid这个字段
