...
/* 由于3.4内核此枚举值不同重新定义 */
#define RDEF_NL80211_AUTHTYPE_AUTOMATIC     5
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,35)) // __plat_modify
/* To be implement! */
...
OAL_STATIC OAL_INLINE oal_int32 oal_ieee80211_channel_to_frequency(oal_int32 l_channel, enum ieee80211_band band)
{
    ...
    switch (band) {
        ...
        case IEEE80211_BAND_5GHZ: // 此case 不跳出直接返回
        {
...
#if (defined(_PRE_BOARD_SD5610) || defined(_PRE_BOARD_SD5115))
    if (memcpy_s(pst_cpu_stat, OAL_SIZEOF(oal_cpu_usage_stat_stru),
                 &kstat_cpu(0).cpustat, OAL_SIZEOF(oal_cpu_usage_stat_stru)) != EOK) {
        OAL_IO_PRINT("oal_get_cpu_stat: memcpy_s failed!\n");
        return;
    }
#endif
}
...
OAL_STATIC OAL_INLINE oal_net_device_stru *oal_net_alloc_netdev(oal_uint32 ul_sizeof_priv,
                                                                const oal_int8 *puc_name,
                                                                oal_void *p_set_up)
{
...
OAL_STATIC OAL_INLINE oal_net_device_stru *oal_net_alloc_netdev_mqs(oal_uint32 ul_sizeof_priv,
                                                                    const oal_int8 *puc_name,
                                                                    oal_void *p_set_up,
                                                                    oal_uint32 ul_txqs,
                                                                    oal_uint32 ul_rxqs)
...
OAL_STATIC OAL_INLINE oal_int32 oal_net_device_set_macaddr(oal_net_device_stru *pst_dev, oal_void *pst_addr)
{
    ...
    l_ret = memcpy_s(pst_dev->dev_addr, ETHER_ADDR_LEN, pst_mac->sa_data, ETHER_ADDR_LEN);
    if (l_ret != EOK) {
...
OAL_STATIC OAL_INLINE oal_netbuf_stru* oal_netbuf_alloc(oal_uint32 ul_size, oal_int32 l_reserve, oal_uint32 l_align)
{
    ...
    if (NET_SKB_PAD < 64) {
        skb_reserve(pst_netbuf, l_reserve + 32);
    } else {
...
{
    return (oal_uint32)skb_tailroom(pst_netbuf);
}
...
{
    struct sk_buff *next = OAL_PTR_NULL, *prev = OAL_PTR_NULL;
...
OAL_STATIC OAL_INLINE oal_void  oal_netbuf_free_list_any(oal_netbuf_head_stru *pst_head)
{
    oal_netbuf_stru *pst_netbuf = OAL_PTR_NULL;
    for (; ;) {
 ...
 */
OAL_STATIC OAL_INLINE int32_t oal_nla_type(OAL_CONST oal_nlattr_stru *nla)
{
    return nla_type(nla);
}
 ...
 */
OAL_STATIC OAL_INLINE uint32_t oal_nla_get_u32(OAL_CONST oal_nlattr_stru *nla)
{
 ...
 */
OAL_STATIC OAL_INLINE void *oal_nla_data(OAL_CONST oal_nlattr_stru *nla)
{
    return nla_data(nla);
}
...
{
    return (oal_uint32)skb_copy_bits(pst_netbuf_sc, ul_offset, p_dst, ul_len);
}
...
OAL_STATIC OAL_INLINE oal_uint32 oal_in_aton(const oal_uint8 *pul_str)
{
...
OAL_STATIC OAL_INLINE oal_int32  oal_dev_hard_header(oal_netbuf_stru *pst_nb,
                                                     ...
                                                     oal_uint16 us_type,
                                                     const oal_uint8 *puc_addr_d,
                                                     const oal_uint8 *puc_addr_s,
                                                     oal_uint32 ul_len)
{
...
OAL_STATIC OAL_INLINE oal_netbuf_stru  *oal_arp_create(oal_int32 l_type, oal_int32 l_ptype, oal_uint32 ul_dest_ip,
                                                       ...
                                                       const oal_uint8 *puc_dest_hw,
                                                       const oal_uint8 *puc_src_hw,
                                                       const oal_uint8 *puc_target_hw)
{
