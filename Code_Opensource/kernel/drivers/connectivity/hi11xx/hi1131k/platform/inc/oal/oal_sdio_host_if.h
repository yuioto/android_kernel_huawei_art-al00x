...
struct hsdio_func1_info {
    ...
    oal_uint32 func1_unknow_int_count;
...
typedef struct _wlan_pm_callback {
    oal_ulong (*wlan_pm_wakeup_dev)(oal_void);                        // SDIO发包过程中中PM状态检查，如果是睡眠状态，同时唤醒
    oal_ulong (*wlan_pm_state_get)(oal_void);                         // 获取当前PM状态
    oal_ulong (*wlan_pm_wakeup_host)(oal_void);                       // device唤醒host中断处理
...
struct oal_sdio {
    ...
    oal_ulong                    ul_wlan_irq;        // wlan gpio中断
    ...
    oal_uint64                  data_int_finish_count;
    ...
    oal_uint32      ul_last_step_time[10];
...
extern struct oal_sdio* oal_sdio_init_module(void* data);
...
int32_t oal_sdio_message_register(struct oal_sdio *hi_sdio, oal_uint8 msg, sdio_msg_rx cb, oal_void* data);
...
extern oal_ulong oal_sdio_get_sleep_state(struct oal_sdio *hi_sdio);
extern oal_void oal_sdio_get_dev_pm_state(struct oal_sdio *hi_sdio, oal_ulong* pst_ul_f1,
...
extern oal_ulong oal_sdio_wakelock_active(struct oal_sdio *pst_hi_sdio);
...
extern oal_int32 oal_sdio_transfer_prepare(struct oal_sdio *hi_sdio);
#else
...
OAL_STATIC OAL_INLINE oal_int32 oal_sdio_send_msg(struct oal_sdio* hi_sdio, oal_ulong val)
{
    return OAL_SUCC;
}
...
OAL_STATIC OAL_INLINE uint32_t oal_sdio_transfer_prepare(struct oal_sdio* hi_sdio)
{
    return OAL_SUCC;
}
#endif
