...
oal_uint32 oam_reg_malloc(oam_reg_type_enum_uint8 en_type)
{
    ...
    if (g_st_oam_reg_mng.past_reg[en_type] != OAL_PTR_NULL) {
    ...
    pst_reg = (oam_reg_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL,
                                            (oal_uint16)(ul_reg_num * sizeof(oam_reg_stru)), OAL_TRUE);
                          ...
                          oal_bool_enum_uint8 en_flag)
{
    ...
    g_st_oam_reg_mng.ul_reg_flag_bitmap &= 0xFFFFFFFF - (1 << en_type);
}
...
oal_uint8 oam_reg_is_need_refresh(oam_reg_evt_enum_uint32 en_evt_type)
{
    ...
    if (g_st_oam_reg_mng.ul_reg_flag_bitmap == 0) {
        return OAL_FALSE;
    }

    /* 判断该事件上报开关是否打开  */
    /* 用户没有设置该事件，不需要上报 */
    if (g_st_oam_reg_mng.aul_refresh_evt[en_evt_type] != OAL_TRUE) {
        return OAL_FALSE;
    }

    /* 事件需要判断evt tick有没有计到0，如果计到0，则刷新上报数据，并更新evt_tick  */
    if (g_st_oam_reg_mng.aul_evt_tick[en_evt_type] == 0) {
        return OAL_FALSE;
    }
...
{
    oal_int32 l_ret;
    if (puc_evt_name == NULL) {
        return OAL_FAIL;
    }
    switch (en_evt_type) {
        case OAM_REG_EVT_TX:
        {
            l_ret = memcpy_s(puc_evt_name, sizeof("TX"), "TX", sizeof("TX"));
            break;
        }
        case OAM_REG_EVT_RX:
        {
            l_ret = memcpy_s(puc_evt_name, sizeof("RX"), "RX", sizeof("RX"));
            break;
        }
        case OAM_REG_EVT_TBTT:
        {
            l_ret = memcpy_s(puc_evt_name, sizeof("tbtt"), "tbtt", sizeof("tbtt"));
            break;
        }
        case OAM_REG_EVT_PRD:
        {
            l_ret = memcpy_s(puc_evt_name, sizeof("period"), "period", sizeof("period"));
            break;
        }
        case OAM_REG_EVT_ERR:
        {
            l_ret = memcpy_s(puc_evt_name, sizeof("err"), "err", sizeof("err"));
            break;
        }
        default:
        {
            l_ret = memcpy_s(puc_evt_name, sizeof("undefined"), "undefined", sizeof("undefined"));
            break;
        }
    }
    if (l_ret != EOK) {
        OAL_IO_PRINT("memcpy_s fail, type:%u.\n", en_evt_type);
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
{
    oal_uint32 ul_evt_type;
    /* 处理所有事件 */
    if (en_evt_type >= OAM_REG_EVT_BUTT) {
        for (ul_evt_type = 0; ul_evt_type < OAM_REG_EVT_BUTT; ul_evt_type++) {
            g_st_oam_reg_mng.aul_refresh_evt[ul_evt_type] = ((ul_tick == 0) ? OAL_FALSE : OAL_TRUE);
    ...
    g_st_oam_reg_mng.aul_refresh_evt[en_evt_type] = ((ul_tick == 0) ? OAL_FALSE : OAL_TRUE);
...
oal_uint32 oam_reg_rpt_buff(oal_void)
{
    ...
    do {
        ...
        oam_netlink_kernel_send(((oal_uint8 *)pst_send_head), \
            (sizeof(oam_reg_send_head_stru) + ul_pkt_num * sizeof(oam_reg_rpt)), OAM_NL_CMD_REG);
        l_bytes = oam_netlink_kernel_send((oal_uint8 *)pst_send_head, \
            (sizeof(oam_reg_send_head_stru) + ul_pkt_num * sizeof(oam_reg_rpt)), OAM_NL_CMD_REG);
        if (l_bytes <= 0) {
...
oal_uint32 oam_reg_nb2buf(oal_netbuf_stru *pst_netbuf)
{
    ...
    if (memcpy_s((oal_uint8 *)pst_send_buff + (uintptr_t)sizeof(oam_reg_send_head_stru) + \
                 (uintptr_t)ul_pkt_num * sizeof(oam_reg_rpt),
                 sizeof(oam_reg_rpt),
                 oal_netbuf_data(pst_netbuf),
                 sizeof(oam_reg_rpt)) != EOK) {
        OAL_IO_PRINT("oam_reg_nb2buf memcpy_s failed! \n");
        return OAL_FAIL;
    }
...
{
    memset_s(&g_st_oam_reg_mng, sizeof(oam_reg_manage_stru), 0, sizeof(oam_reg_manage_stru));

    g_st_oam_reg_mng.puc_send_buff = (oal_uint8 *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL,
                                                                OAM_REG_MAX_SEND_BUF_SIZE, OAL_TRUE);
    if (g_st_oam_reg_mng.puc_send_buff == OAL_PTR_NULL) {
        OAL_IO_PRINT("oam_reg_init: puc_send_buff is NULL!\n");
        return;
    }
    memset_s(g_st_oam_reg_mng.puc_send_buff, OAM_REG_MAX_SEND_BUF_SIZE, 0, OAM_REG_MAX_SEND_BUF_SIZE);
    ...
    /* 初始化工作队列 */
    memset_s((void *)&g_st_oam_reg_mng.st_wq, OAL_SIZEOF(oam_reg_workqueue_stru), 0,
             OAL_SIZEOF(oam_reg_workqueue_stru));
    g_st_oam_reg_mng.st_wq.pst_wq = OAL_CREATE_SINGLETHREAD_WORKQUEUE("oam_reg_tx_queue");
    if (g_st_oam_reg_mng.st_wq.pst_wq == OAL_PTR_NULL) {
        OAL_MEM_FREE(g_st_oam_reg_mng.puc_send_buff, OAL_TRUE);
        g_st_oam_reg_mng.puc_send_buff = OAL_PTR_NULL;
        OAL_IO_PRINT("oam_reg_init: creat workqueue oam_reg_tx_queue failed!\n");
        return;
    }
...
{
    oal_netbuf_stru                *pst_queue_netbuf = OAL_PTR_NULL;
    oam_reg_rpt                    *pst_reg_rpt_nb = OAL_PTR_NULL;
...
oal_void  oam_reg_info(oal_void)
{
    ...
    if (snprintf_s(ac_tmp_buff, OAL_SIZEOF(ac_tmp_buff), OAL_SIZEOF(ac_tmp_buff) - 1,
                   "rpt cnt %u \n", g_st_oam_reg_mng.ul_rpt_count) < EOK) {
        OAL_IO_PRINT("oam_reg_info::snprintf_s failed ! \n");
        return;
    }
    ac_tmp_buff[63] = '\0';
