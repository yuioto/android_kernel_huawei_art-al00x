...
extern oal_void frw_event_deploy_register(
    oal_uint32 (*p_func)(frw_event_mem_stru *pst_event_mem, oal_uint8 *puc_deploy_result));
...
extern oal_uint32 frw_event_lookup_process_entry(frw_event_mem_stru *pst_event_mem,
...
OAL_STATIC OAL_INLINE oal_uint32 frw_event_to_qid(frw_event_mem_stru *pst_event_mem, oal_uint16 *pus_qid)
{
    ...
    if ((us_qid >= FRW_EVENT_MAX_NUM_QUEUES)) {
        OAM_ERROR_LOG4(0, OAM_SF_FRW,
            "{frw_event_to_qid, array overflow! us_qid[%d], vap_id[%d], en_type[%d], sub_type[%d]}", us_qid,
            pst_event_hrd->uc_vap_id, pst_event_hrd->en_type, pst_event_hrd->uc_sub_type);
...
OAL_STATIC OAL_INLINE oal_void frw_event_report(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* 复制事件头 */
    if (memcpy_s((oal_void *)auc_event, OAM_EVENT_INFO_MAX_LEN, (const oal_void *)&pst_event->st_event_hdr,
        OAL_SIZEOF(frw_event_hdr_stru)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_FRW, "{frw_event_report: memcpy_s fail!}");
        return;
    }
...
{
    oal_uint32 ul_pos;
    frw_event_trace_item_stru *pst_trace_item = OAL_PTR_NULL;
