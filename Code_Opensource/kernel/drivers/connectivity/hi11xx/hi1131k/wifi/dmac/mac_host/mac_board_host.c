...
#undef  THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_MAC_BOARD_HOST_C
...
oal_void  event_fsm_unregister(oal_void)
{
    /* Part1: 以下是Dmac侧的事件 */
    /* 去注册DMAC模块HOST_CRX事件 */
    memset_s(g_ast_dmac_host_crx_table, OAL_SIZEOF(g_ast_dmac_host_crx_table), 0,
             OAL_SIZEOF(g_ast_dmac_host_crx_table));

    /* 去注册DMAC模块HOST_DRX事件处理函数表 */
    memset_s(g_ast_dmac_tx_host_drx, OAL_SIZEOF(g_ast_dmac_tx_host_drx), 0, OAL_SIZEOF(g_ast_dmac_tx_host_drx));

    /* 去注册DMAC模块WLAN_DTX事件处理函数表 */
    memset_s(g_ast_dmac_tx_wlan_dtx, OAL_SIZEOF(g_ast_dmac_tx_wlan_dtx), 0, OAL_SIZEOF(g_ast_dmac_tx_wlan_dtx));

    /* 去注册DMAC模块WLAN_DRX事件子表 */
    memset_s(g_ast_dmac_wlan_drx_event_sub_table, OAL_SIZEOF(g_ast_dmac_wlan_drx_event_sub_table), 0,
             OAL_SIZEOF(g_ast_dmac_wlan_drx_event_sub_table));

    /* 去注册DMAC模块WLAN_CRX事件子表 */
    memset_s(g_ast_dmac_wlan_crx_event_sub_table, OAL_SIZEOF(g_ast_dmac_wlan_crx_event_sub_table), 0,
             OAL_SIZEOF(g_ast_dmac_wlan_crx_event_sub_table));

    /* 去注册DMAC模块TX_COMP事件子表 */
    memset_s(g_ast_dmac_tx_comp_event_sub_table, OAL_SIZEOF(g_ast_dmac_tx_comp_event_sub_table), 0,
             OAL_SIZEOF(g_ast_dmac_tx_comp_event_sub_table));

    /* 去注册DMAC模块TBTT事件字表 */
    memset_s(g_ast_dmac_tbtt_event_sub_table, OAL_SIZEOF(g_ast_dmac_tbtt_event_sub_table), 0,
             OAL_SIZEOF(g_ast_dmac_tbtt_event_sub_table));

    /* 去注册DMAC模块MAC_ERROR事件子表 */
    memset_s(g_ast_dmac_high_prio_event_sub_table, OAL_SIZEOF(g_ast_dmac_high_prio_event_sub_table), 0,
             OAL_SIZEOF(g_ast_dmac_high_prio_event_sub_table));

    /* 去注册DMAC模块MAC_ERROR事件子表 */
    memset_s(g_ast_dmac_misc_event_sub_table, OAL_SIZEOF(g_ast_dmac_misc_event_sub_table), 0,
             OAL_SIZEOF(g_ast_dmac_misc_event_sub_table));

    /* Part2: 以下是Hmac侧的事件 */
    memset_s(g_ast_hmac_wlan_dtx_event_sub_table, OAL_SIZEOF(g_ast_hmac_wlan_dtx_event_sub_table), 0,
             OAL_SIZEOF(g_ast_hmac_wlan_dtx_event_sub_table));
    memset_s(g_ast_hmac_wlan_drx_event_sub_table, OAL_SIZEOF(g_ast_hmac_wlan_drx_event_sub_table), 0,
             OAL_SIZEOF(g_ast_hmac_wlan_drx_event_sub_table));
    memset_s(g_ast_hmac_wlan_crx_event_sub_table, OAL_SIZEOF(g_ast_hmac_wlan_crx_event_sub_table), 0,
             OAL_SIZEOF(g_ast_hmac_wlan_crx_event_sub_table));
    memset_s(g_ast_hmac_tbtt_event_sub_table, OAL_SIZEOF(g_ast_hmac_tbtt_event_sub_table), 0,
             OAL_SIZEOF(g_ast_hmac_tbtt_event_sub_table));
    memset_s(g_ast_hmac_wlan_ctx_event_sub_table, OAL_SIZEOF(g_ast_hmac_wlan_ctx_event_sub_table), 0,
             OAL_SIZEOF(g_ast_hmac_wlan_ctx_event_sub_table));
    memset_s(g_ast_hmac_wlan_misc_event_sub_table, OAL_SIZEOF(g_ast_hmac_wlan_misc_event_sub_table), 0,
             OAL_SIZEOF(g_ast_hmac_wlan_misc_event_sub_table));
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
    memset_s(g_ast_hmac_hcc_rx_event_sub_table, OAL_SIZEOF(g_ast_hmac_hcc_rx_event_sub_table), 0,
             OAL_SIZEOF(g_ast_hmac_hcc_rx_event_sub_table));
#endif
}
/*lint -e19*/
...
oal_uint32 dfx_get_performance_log_switch_enable(dfx_performance_log_switch_enum_uint8 log_switch_type)
{
    return g_auc_dfx_performance_log_switch[log_switch_type];
}


oal_void dfx_set_performance_log_switch_enable(dfx_performance_log_switch_enum_uint8 log_switch_type,
                                               oal_uint8 uc_value)
{
    if (log_switch_type >= DFX_PERFORMANCE_LOG_BUTT) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "dfx_set_performance_log_switch_enable::log_switch type:%d.",
                         log_switch_type);
        return;
    }
    g_auc_dfx_performance_log_switch[log_switch_type] = uc_value;
}
#endif
