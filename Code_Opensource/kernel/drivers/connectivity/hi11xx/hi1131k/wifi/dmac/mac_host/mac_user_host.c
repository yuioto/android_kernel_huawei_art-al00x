...
#undef  THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_MAC_USER_HOST_C
...
oal_uint32 mac_user_add_wep_key(mac_user_stru *pst_mac_user, oal_uint8 uc_key_index, mac_key_params_stru *pst_key)
{
    ...
    ret = memcpy_s(&pst_mac_user->st_key_info.ast_key[uc_key_index].auc_key, WLAN_WPA_KEY_LEN, pst_key->auc_key,
        (uint32_t)pst_key->key_len);
    if (ret != EOK) {
        OAM_WARNING_LOG1(0, OAM_SF_WPA, "mac_user_add_wep_key::memcopy_s return value %d", ret);
        return OAL_FAIL;
    }
    ret = memcpy_s(&pst_mac_user->st_key_info.ast_key[uc_key_index].auc_seq, WLAN_WPA_SEQ_LEN, pst_key->auc_seq,
        (uint32_t)pst_key->seq_len);
    if (ret != EOK) {
        OAM_WARNING_LOG1(0, OAM_SF_WPA, "mac_user_add_wep_key::memcopy_s return value %d", ret);
        return OAL_FAIL;
    }
...
oal_uint32 mac_user_add_bip_key(mac_user_stru *pst_mac_user, oal_uint8 uc_key_index, mac_key_params_stru *pst_key)
{
    ...
    ret = memcpy_s(&pst_mac_user->st_key_info.ast_key[uc_key_index].auc_key, WLAN_WPA_SEQ_LEN, pst_key->auc_key,
        (uint32_t)pst_key->key_len);
    if (ret != EOK) {
        OAM_WARNING_LOG1(0, OAM_SF_WPA, "mac_user_add_wep_key::memcopy_s return value %d", ret);
        return OAL_FAIL;
                            ...
                            oal_uint8       uc_keyid)
{
    ...
    OAM_WARNING_LOG4(0, OAM_SF_WPA, "{mac_user_set_key::keytpe==%u, ciphertype==%u, keyid==%u, usridx==%u}",
                                     ...
                                     wlan_bw_cap_enum_uint8 en_cur_bandwidth)
{
    ...
    /* Autorate将协议从11n切换成11b后, cur_bandwidth会变为20M,此时如果软件将带宽改为40M, cur_bandwidth仍需要保持20M */
...
oal_void  mac_user_get_ap_opern_bandwidth(mac_user_stru *pst_mac_user, wlan_bw_cap_enum_uint8 *pen_bandwidth_cap)
{
    ...
    if (pst_mac_vht_hdl->en_vht_capable == OAL_TRUE) {
        ...
        if (pst_mac_vht_hdl->uc_channel_width == 1) {
            en_bandwidth_cap = WLAN_BW_CAP_80M;
        } else if (pst_mac_vht_hdl->uc_channel_width == 2) {
            en_bandwidth_cap = WLAN_BW_CAP_160M;
        }
    }
    *pen_bandwidth_cap = en_bandwidth_cap;
...
oal_void mac_user_set_avail_op_rates(mac_user_stru *pst_mac_user, oal_uint8 uc_rs_nrates, oal_uint8 *puc_rs_rates)
{
    ...
    ret = memcpy_s(pst_mac_user->st_avail_op_rates.auc_rs_rates, WLAN_RATE_MAXSIZE, puc_rs_rates, WLAN_RATE_MAXSIZE);
    if (ret != EOK) {
        OAM_WARNING_LOG1(0, OAM_SF_WPA, "mac_user_set_avail_op_rates::memcopy_s return value %d", ret);
        return;
    }
}
...
oal_void mac_user_set_vht_hdl(mac_user_stru *pst_mac_user, mac_vht_hdl_stru *pst_vht_hdl)
{
    ...
    ret = memcpy_s((uint8_t *)(&pst_mac_user->st_vht_hdl), OAL_SIZEOF(mac_vht_hdl_stru),
        (uint8_t *)pst_vht_hdl, OAL_SIZEOF(mac_vht_hdl_stru));
    if (ret != EOK) {
        OAM_WARNING_LOG1(0, OAM_SF_WPA, "mac_user_set_vht_hdl::memcopy_s return value %d", ret);
        return;
    }
}
...
oal_void mac_user_get_vht_hdl(mac_user_stru *pst_mac_user, mac_vht_hdl_stru *pst_vht_hdl)
{
    ...
    ret = memcpy_s((uint8_t *)pst_vht_hdl, OAL_SIZEOF(mac_vht_hdl_stru),
        (uint8_t *)(&pst_mac_user->st_vht_hdl), OAL_SIZEOF(mac_vht_hdl_stru));
    if (ret != EOK) {
        OAM_WARNING_LOG1(0, OAM_SF_WPA, "mac_user_get_vht_hdl::memcopy_s return value %d", ret);
        return;
    }
}
...
oal_void mac_user_set_ht_hdl(mac_user_stru *pst_mac_user, mac_user_ht_hdl_stru *pst_ht_hdl)
{
    ...
    ret = memcpy_s((uint8_t *)(&pst_mac_user->st_ht_hdl), OAL_SIZEOF(mac_user_ht_hdl_stru),
        (uint8_t *)pst_ht_hdl, OAL_SIZEOF(mac_user_ht_hdl_stru));
    if (ret != EOK) {
        OAM_WARNING_LOG1(0, OAM_SF_WPA, "mac_user_set_ht_hdl::memcopy_s return value %d", ret);
        return;
    }
}
...
oal_void mac_user_get_ht_hdl(mac_user_stru *pst_mac_user, mac_user_ht_hdl_stru *pst_ht_hdl)
{
    ...
    ret = memcpy_s((uint8_t *)pst_ht_hdl, OAL_SIZEOF(mac_user_ht_hdl_stru),
        (uint8_t *)(&pst_mac_user->st_ht_hdl), OAL_SIZEOF(mac_user_ht_hdl_stru));
    if (ret != EOK) {
        OAM_WARNING_LOG1(0, OAM_SF_WPA, "mac_user_get_ht_hdl::memcopy_s return value %d", ret);
        return;
    }
}
...
oal_void mac_user_set_sm_power_save(mac_user_stru *pst_mac_user, uint8_t uc_sm_power_save)
{
...
oal_uint32 mac_user_update_wep_key(mac_user_stru *pst_mac_usr, oal_uint16 us_multi_user_idx)
{
    ...
    ret = memcpy_s(&pst_mac_usr->st_key_info, OAL_SIZEOF(mac_key_mgmt_stru), &pst_multi_user->st_key_info,
                   OAL_SIZEOF(mac_key_mgmt_stru));
    if (ret != EOK) {
        OAM_WARNING_LOG1(0, OAM_SF_WPA, "mac_user_update_wep_key::memcopy_s return value %d", ret);
        return OAL_FAIL;
    }

    // 获取WEP default key id
    pst_mac_usr->st_user_tx_info.st_security.en_cipher_key_type =
        pst_mac_usr->st_key_info.uc_default_index + HAL_KEY_TYPE_PTK;

    return OAL_SUCC;
}


oal_bool_enum_uint8 mac_addr_is_zero(const oal_uint8 *puc_mac)
{
