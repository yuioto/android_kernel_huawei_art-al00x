...
typedef oal_uint32  (*wal_config_set_func)(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, const oal_uint8 *puc_param);

/* 填写配置消息头 */
#define WAL_CFG_MSG_HDR_INIT(_pst_cfg_msg_hdr, _en_type, _us_len, _uc_sn)  do { \
...
/* 填写write msg消息头 */
#define WAL_WRITE_MSG_HDR_INIT(_pst_write_msg, _en_wid, _us_len) do { \
...
#define WAL_MSG_WRITE_MAX_LEN   (WLAN_MEM_EVENT_SIZE2 - FRW_EVENT_HDR_LEN - \
    FRW_IPC_MSG_HEADER_LENGTH - WAL_MSG_WRITE_MSG_HDR_LENGTH)
...
// 3 枚举定义
typedef enum {
    WAL_CUSTOMIZIED_PROBE_REQUEST_OFF,     /* 打开定制化request帧 */
    WAL_CUSTOMIZIED_PROBE_REQUEST_ON,      /* 关闭定制化request帧 */

    WAL_CUSTOMIZIED_PROBE_REQUEST_BUTT
} wal_customized_probe_request_switch_enum;
typedef oal_uint8 wal_customized_probe_request_switch_enum_uint8;
...
#define WAL_MSG_REQ_STRU_INIT(name) do {    \
...
extern oal_int32 wal_set_msg_response_by_addr(oal_ulong addr,
                                              oal_void *pst_resp_mem,
                                              oal_uint32 ul_resp_ret,
...
extern oal_uint32 wal_config_set_channel(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, const oal_uint8 *puc_param);
