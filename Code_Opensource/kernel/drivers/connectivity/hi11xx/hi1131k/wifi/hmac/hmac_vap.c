...
#if ((LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 44)) && (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION))
...
OAL_STATIC oal_net_device_ops_stru gst_vap_net_dev_cfg_vap_ops = { oal_net_device_init,
                                                                   oal_net_device_open,
...
#endif
#ifdef _PRE_WLAN_FEATURE_SAE
void hmac_report_ext_auth_worker(oal_work_stru *sae_report_external_auth_worker);
#endif
// 3 函数实现

oal_uint32 hmac_vap_init(hmac_vap_stru *pst_hmac_vap, oal_uint8 uc_chip_id, oal_uint8 uc_device_id, oal_uint8 uc_vap_id,
    mac_cfg_add_vap_param_stru *pst_param)
{
    ...
    if (ul_soft_ver >= SOFT_VER_BUTT) {
        OAM_ERROR_LOG0(uc_vap_id, OAM_SF_ANY, "device soft version is invalid!\n");
        return OAL_FAIL;
    } else if (ul_soft_ver == SOFT_VER_CO1) {
        /* 设置vap最大用户数为4 */
        pst_hmac_vap->us_user_nums_max = WLAN_ASSOC_USER_MAX_NUM_SPEC_C01;
    } else {
        /* 设置vap最大用户数为8 */
        pst_hmac_vap->us_user_nums_max = WLAN_ASSOC_USER_MAX_NUM_SPEC;
    }
    ...
    switch (pst_param->en_vap_mode) {
        case WLAN_VAP_MODE_BSS_AP:
            ...
            FRW_TIMER_CREATE_TIMER(&(pst_hmac_vap->st_edca_opt_timer), hmac_edca_opt_timeout_fn,
                pst_hmac_vap->ul_edca_opt_time_ms, pst_hmac_vap, OAL_TRUE, OAM_MODULE_ID_ALG_SCHEDULE,
        ...
        default:
            OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                "{hmac_vap_init::unsupported mod=%d.}", pst_param->en_vap_mode);
    ...
    oal_atomic_set(&pst_hmac_vap->ul_tx_event_num, 1);
...
#ifdef _PRE_WLAN_FEATURE_SAE
    OAL_INIT_WORK(&(pst_hmac_vap->sae_report_ext_auth_worker), hmac_report_ext_auth_worker);
...
oal_uint32 hmac_vap_creat_netdev(hmac_vap_stru *pst_hmac_vap, oal_int8 *puc_netdev_name, oal_uint8 puc_netdev_name_len,
...

{
    ...
    if (puc_netdev_name_len > MAC_NET_DEVICE_NAME_LENGTH) {
        return OAL_FAIL;
    }
    ...
    l_ret += memcpy_s(OAL_NETDEVICE_MAC_ADDR(pst_net_device), WLAN_MAC_ADDR_LEN, puc_mac_addr, WLAN_MAC_ADDR_LEN);
    ...
    if (l_ret != EOK) {
        OAM_ERROR_LOG2(0, OAM_SF_ANY, "mac_device_init::memcpy_s failed![%d][%d]", l_ret, OAL_IF_NAME_SIZE);
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
oal_uint16 hmac_vap_check_ht_capabilities_ap(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_payload,
    oal_uint16 us_info_elem_offset, oal_uint32 ul_msg_len, hmac_user_stru *pst_hmac_user_sta)
{
    ...
    if (us_info_elem_offset < ul_msg_len) {
        ...
        ie_data = mac_find_ie(MAC_EID_HT_CAP, puc_payload, (uint32_t)ul_msg_len);
        if (ie_data != OAL_PTR_NULL) {
            ...
            uc_grp_policy =
                pst_hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_rsna_cfg.uc_dot11RSNAGroupCipherRequested;

            if (uc_pcip_policy == WLAN_80211_CIPHER_SUITE_TKIP) {
                OAM_WARNING_LOG2(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                    "{hmac_vap_check_ht_capabilities_ap::HT STA with TKIP Pairwise Cipher,\
                    uc_pcip_policy=%d uc_grp_policy=%d.}", uc_pcip_policy, uc_grp_policy);
            } else {
                /* 搜索 HT Capabilities Element */
                hmac_search_ht_cap_ie_ap(pst_hmac_vap, pst_hmac_user_sta, ie_data, 0, en_prev_asoc_ht);
            }
        }
#ifdef _PRE_WLAN_FEATURE_20_40_80_COEXIST
        ie_data = mac_find_ie(MAC_EID_EXT_CAPS, puc_payload, (uint32_t)ul_msg_len);
        if (ie_data != OAL_PTR_NULL) {
            mac_ie_proc_ext_cap_ie(&(pst_hmac_user_sta->st_user_base_info), ie_data);
        }
    ...
    if (pst_ht_hdl->en_ht_capable == OAL_FALSE) {
        ...
        /* else:STA 之前已经作为非HT站点与AP关联--继续往下执行 */
    } else {
        /* STA支持HT */
...
oal_uint16 hmac_vap_check_vht_capabilities_ap(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_payload,
    oal_uint16 us_info_elem_offset, oal_uint32 ul_msg_len, hmac_user_stru *pst_hmac_user_sta)
{
    ...
    if (pst_hmac_vap == OAL_PTR_NULL || puc_payload == OAL_PTR_NULL || pst_hmac_user_sta == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{hmac_vap_check_vht_capabilities_ap::param null}");
    ...
    if (puc_vht_cap_ie != OAL_PTR_NULL) {
        ...
        if (uc_pcip_policy == WLAN_80211_CIPHER_SUITE_TKIP) {
            OAM_WARNING_LOG2(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                "{hmac_vap_check_vht_capabilities_ap::VHT STA with TKIP Pairwise Cipher,uc_pcip_policy=%d "
                "uc_grp_policy=%d.}",
...
oal_void hmac_search_txbf_cap_ie_ap(mac_user_ht_hdl_stru *pst_ht_hdl, oal_uint32 ul_info_elem)
{
...
oal_uint32 hmac_search_ht_cap_ie_ap(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user_sta,
    oal_uint8 *puc_payload, oal_uint16 us_current_offset, oal_bool_enum en_prev_asoc_ht)
{
    oal_uint8                   uc_smps;
    ...
    if (puc_tmp_payload[us_current_offset + 1] < MAC_HT_CAP_LEN) {
    ...
    if ((pst_ht_hdl->bit_dsss_cck_mode_40mhz == 0) && (pst_ht_hdl->bit_supported_channel_width == 1)) {
    ...
    if (pst_ht_hdl->bit_forty_mhz_intolerant != 0) {
    ...
    for (uc_mcs_bmp_index = 0; uc_mcs_bmp_index < WLAN_HT_MCS_BITMASK_LEN; uc_mcs_bmp_index++) {
        pst_ht_hdl->uc_rx_mcs_bitmask[uc_mcs_bmp_index] =
            (pst_hmac_vap->st_vap_base_info.pst_mib_info->st_supported_mcstx
                .auc_dot11SupportedMCSTxValue[uc_mcs_bmp_index]) &
...
oal_bool_enum_uint8 hmac_flowctl_check_device_is_sta_mode(oal_void)
{
        ...
        for (uc_vap_index = 0; uc_vap_index < pst_dev->uc_vap_num; uc_vap_index++) {
            ...
            if (pst_vap == OAL_PTR_NULL) {
                OAM_INFO_LOG1(0, OAM_SF_BA,
                    "{hmac_flowctl_check_device_is_sta_mode::mac_res_get_mac_vap fail.vap_index = %u}", uc_vap_index);
                continue;
            }
    ...
    OAM_INFO_LOG2(0, OAM_SF_TX, "{hmac_flowctl_check_device_is_sta_mode:: vap_num = %d, is_sta mode = %d}",
        pst_dev->uc_vap_num, en_device_is_sta);
...
oal_uint32 hmac_check_opmode_notify(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_mac_hdr, oal_uint8 *puc_payload,
    oal_uint16 us_info_elem_offset, oal_uint32 ul_msg_len, hmac_user_stru *pst_hmac_user)
{
    ...
    if (OAL_ANY_NULL_PTR4(pst_hmac_vap, puc_payload, pst_hmac_user, puc_mac_hdr)) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{hmac_check_opmode_notify::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if ((puc_opmode_notify_ie != OAL_PTR_NULL) && (puc_opmode_notify_ie[1] >= MAC_OPMODE_NOTIFY_LEN)) {
        ...
        if (OAL_UNLIKELY(ul_relt != OAL_SUCC)) {
            OAM_WARNING_LOG1(pst_mac_user->uc_vap_id, OAM_SF_CFG,
                "{hmac_check_opmode_notify::hmac_ie_proc_opmode_field failed[%d].}", ul_relt);
            return ul_relt;
        }
        /* opmode息同步dmac */
        ...
        if (OAL_UNLIKELY(ul_relt != OAL_SUCC)) {
            OAM_WARNING_LOG1(pst_mac_user->uc_vap_id, OAM_SF_CFG,
                "{hmac_check_opmode_notify::hmac_config_update_opmode_event failed[%d].}", ul_relt);
            return ul_relt;
        }
    }
    return OAL_SUCC;
...
#ifdef _PRE_WLAN_FEATURE_SAE
/* 上报EXTERNAL_AUTH_START事件到wal */
void hmac_report_ext_auth_worker(oal_work_stru *sae_report_external_auth_worker)
{
    hmac_vap_stru *hmac_vap = NULL;

    hmac_vap = OAL_CONTAINER_OF(sae_report_external_auth_worker, hmac_vap_stru, sae_report_ext_auth_worker);
    if (hmac_vap == NULL) {
    ...
    OAM_WARNING_LOG0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
        "{hmac_report_ext_auth_worker:: hmac_report_ext_auth_worker auth start}");
    ...
    hmac_report_disasoc_reason_uint16 en_disasoc_reason)
{
    ...
    switch (pst_hmac_vap->st_vap_base_info.en_vap_mode) {
        ...
        case WLAN_VAP_MODE_BSS_STA: {
            ...
            hmac_sta_handle_disassoc_rsp(pst_hmac_vap, en_disasoc_reason, DMAC_DISASOC_MISC_KICKUSER);
        }
        break;
        default:
            break;
