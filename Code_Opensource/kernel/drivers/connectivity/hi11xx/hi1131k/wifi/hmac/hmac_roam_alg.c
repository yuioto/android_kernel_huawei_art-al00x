...
void hmac_roam_alg_init(hmac_roam_info_stru *roam_info, int8_t c_current_rssi)
{
    ...
    /*lint +e571*/
    pst_roam_alg = &(roam_info->st_alg);
    if (c_current_rssi == ROAM_RSSI_LINKLOSS_TYPE) {
        roam_info->st_static.ul_trigger_linkloss_cnt++;
    } else {
        roam_info->st_static.ul_trigger_rssi_cnt++;
    }
...
OAL_STATIC oal_int8 hmac_roam_alg_adjust_rssi_increase(hmac_roam_info_stru *pst_roam_info,
    ...
    for (ul_current_index = 0; ul_current_index < ROAM_LIST_MAX; ul_current_index++) {
        ...
        if (oal_compare_mac_addr(pst_cur_bss->auc_bssid, puc_bssid) == 0) {
            ...
            if (OAL_TIME_GET_RUNTIME(pst_cur_bss->ul_time_stamp, ul_now) > ul_timeout) {
                ...
                /* 漫游黑名单，以后有用 */
                if (list_type == ROAM_BLACKLIST_TYPE_REJECT_AP) {
                ...
                if (pst_cur_bss->us_count == pst_cur_bss->us_count_limit) {
                    ...
                    OAM_WARNING_LOG4(0, OAM_SF_ROAM,
                        "{hmac_roam_alg_add_bsslist::  add new, [%02X:XX:XX:%02X:%02X:%02X]}",
                        pst_cur_bss->auc_bssid[0], pst_cur_bss->auc_bssid[3], pst_cur_bss->auc_bssid[4],
                        pst_cur_bss->auc_bssid[5]);
                }
            }
            return OAL_SUCC;
        }
    ...
    oal_uint8 *puc_bssid)
{
    ...
    for (ul_current_index = 0; ul_current_index < ROAM_LIST_MAX; ul_current_index++) {
        ...
        if (oal_compare_mac_addr(pst_cur_bss->auc_bssid, puc_bssid) == 0) {
            ...
            if ((ul_delta_time <= ul_timeout) && (pst_cur_bss->us_count >= us_count_limit)) {
                OAM_WARNING_LOG4(0, OAM_SF_ROAM,
                    "{hmac_roam_alg_find_in_bsslist::  time_left = [%d], [%02X:XX:XX:XX:%02X:%02X]}",
                    ul_timeout - ul_delta_time, pst_cur_bss->auc_bssid[0], pst_cur_bss->auc_bssid[4],
                    pst_cur_bss->auc_bssid[5]);
                return OAL_TRUE;
            }
            return OAL_FALSE;
...
OAL_STATIC oal_uint32 hmac_roam_alg_calc_avail_channel_capacity(mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    puc_obss_ie = mac_find_ie(MAC_EID_QBSS_LOAD, (oal_uint8 *)(pst_bss_dscr->auc_mgmt_buff + uc_ie_offset),
...
oal_uint32 hmac_roam_alg_bss_check(hmac_roam_info_stru *pst_roam_info, mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    mac_vap_stru            *pst_mac_vap = NULL;
    hmac_roam_alg_stru      *pst_roam_alg = NULL;
    mac_cap_info_stru       *pst_cap_info = NULL;
    mac_cfg_ssid_param_stru st_cfg_ssid;
    oal_uint32              ul_ret;
    oal_uint8               uc_stru_len;
    ...
    if ((pst_bss_dscr->st_bss_sec_info.uc_bss_80211i_mode == 0) && (pst_cap_info->bit_privacy != 0)) {
        return OAL_SUCC;
    }

    /*  open加密方式到wpa/wpa2直接过滤掉 */
    /*lint -e731*/
    if ((pst_cap_info->bit_privacy == 0) != (mac_mib_get_privacyinvoked(&pst_hmac_vap->st_vap_base_info) != OAL_TRUE)) {
        return OAL_SUCC;
    }
    /*lint +e731*/
    ...
    if ((pst_roam_alg->pst_max_rssi_bss == OAL_PTR_NULL) || (pst_bss_dscr->c_rssi > pst_roam_alg->c_max_rssi)) {
