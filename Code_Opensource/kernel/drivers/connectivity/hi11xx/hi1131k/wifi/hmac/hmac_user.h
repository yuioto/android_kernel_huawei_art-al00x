...
typedef struct {
    ...
    oal_spin_lock_stru              ba_status_lock;     /* TID对应的BA会话状态锁 */
} hmac_ba_tx_stru;
...
typedef struct tag_hmac_wapi_stru {
    ...
    oal_bool_enum_uint8     (*wapi_is_pn_odd)(const oal_uint8 *puc_pn);     /* 判断pn是否为奇数 */
...
typedef struct {
    ...
    /* 发送/接收 tcp/udp be,bk,vi,vo报文 */
    oal_uint32                      aaul_txrx_data_stat[WLAN_WME_AC_BUTT][WLAN_TXRX_DATA_BUTT];
#endif
...
extern hmac_user_stru *mac_vap_get_hmac_user_by_addr(mac_vap_stru *pst_mac_vap, const oal_uint8 *puc_mac_addr);
#ifdef _PRE_WLAN_FEATURE_WAPI
...
#endif
extern void hmac_user_clear_defrag_res(hmac_user_stru *hmac_user);
