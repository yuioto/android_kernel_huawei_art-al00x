                               ...
                               mac_action_mgmt_args_stru *pst_action_args)
{
    if (OAL_ANY_NULL_PTR3(pst_hmac_vap, pst_hmac_user, pst_action_args)) {
        OAM_ERROR_LOG0(0, OAM_SF_TX, "{hmac_mgmt_tx_action::param null}");
                                 ...
                                 mac_priv_req_args_stru *pst_priv_req)
{
    ...
    if (OAL_ANY_NULL_PTR3(pst_hmac_vap, pst_hmac_user, pst_priv_req)) {
        OAM_ERROR_LOG0(0, OAM_SF_TX, "{hmac_mgmt_tx_priv_req::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
uint32_t hmac_rx_process_others_bss_management(oal_netbuf_stru *netbuf)
{
#if (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION) || (_PRE_OS_VERSION_WIN32 == _PRE_OS_VERSION)
    ...
    if (ret != OAL_SUCC) {
        OAM_ERROR_LOG1(0, OAM_SF_RX, "{hmac_rx_process_others_bss_data::hwal_send_others_bss_data fail,ret=%d.}", ret);
        return OAL_FAIL;
    }
#endif
#else
    int32_t ret;
    ret = wal_hilink_upload_frame(netbuf);
    if (ret != OAL_SUCC) {
        OAM_ERROR_LOG1(0, OAM_SF_RX, "{hmac_rx_process_others_bss_data::wal_hilink_upload_frame fail,ret=%d.}", ret);
        return OAL_FAIL;
    }
#endif
    return OAL_SUCC;
...
extern oal_uint8 g_uc_management_switch;
#endif
...
oal_uint32 hmac_rx_process_mgmt_event(frw_event_mem_stru *pst_event_mem)
{
...
#ifdef _PRE_WLAN_FEATURE_HILINK
    if (g_uc_management_switch == OAL_TRUE) {
        /* 处理上报的其他BSS 管理帧 */
        ul_ret = hmac_rx_process_others_bss_management(pst_netbuf);
        if (ul_ret != OAL_SUCC) {
            OAM_ERROR_LOG1(0, OAM_SF_RX, "hmac_rx_process_mgmt_event::process other bss data is failed! ul_ret=%d",
                ul_ret);
        }
        /* monitor模式上报的管理帧都在该处统一释放 */
    ...
    if (pst_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG1(0, OAM_SF_BA, "{hmac_rx_process_mgmt_event::hmac_fsm_call_func_ap fail.err code1 [%u]}",
                ul_ret);
        }
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG1(0, OAM_SF_BA, "{hmac_rx_process_mgmt_event::hmac_fsm_call_func_ap fail.err code2 [%u]}",
                ul_ret);
        }
    }
...
oal_uint32 hmac_mgmt_send_disasoc_deauth_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (uc_event == DMAC_WLAN_CRX_EVENT_SUB_TYPE_DEAUTH) {
        hmac_mgmt_send_deauth_frame(pst_mac_vap, puc_da, us_err_code, OAL_FALSE); // 非PMF
#ifdef _PRE_WLAN_FEATURE_P2P
        pst_mac_device = mac_res_get_dev(pst_mac_vap->uc_device_id);
        /* 判断是异频DBAC模式时，无法判断是哪个信道收到的数据帧，两个信道都需要发去认证 */
        if (pst_mac_device == OAL_PTR_NULL) {
            OAM_ERROR_LOG0(pst_event_hdr->uc_vap_id, OAM_SF_ASSOC,
                "{hmac_mgmt_send_disasoc_deauth_event::pst_mac_device null.}");
            return OAL_ERR_CODE_PTR_NULL;
        }
        ...
        hmac_mgmt_send_deauth_frame(pst_up_vap2, puc_da, us_err_code | MAC_SEND_TWO_DEAUTH_FLAG, OAL_FALSE);
#endif
    ...
    if (ul_rslt != OAL_SUCC) {
        OAM_WARNING_LOG4(0, OAM_SF_RX,
            ...
            just del DMAC user}", puc_da[0], puc_da[3], puc_da[4], puc_da[5]);
    ...
    hmac_mgmt_send_disassoc_frame(pst_mac_vap, puc_da, us_err_code,
        ((pst_hmac_user == OAL_PTR_NULL) ? OAL_FALSE : pst_hmac_user->st_user_base_info.st_cap_info.bit_pmf_active));
...
oal_uint32 hmac_proc_disasoc_misc_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    hmac_vap_stru          *pst_hmac_vap = OAL_PTR_NULL;
    hmac_user_stru         *pst_hmac_user = OAL_PTR_NULL;
    oal_bool_enum_uint8    en_is_protected;  /* PMF */
    ...
    if (g_st_dmac_test_mng.uc_chip_test_open != 0) {
        /* 如果有打开cca测试 */
        if (g_st_dmac_test_mng.uc_cca_flag == 1) {
        }
    }
#endif
    ...
    if (pst_hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
        ...
        hmac_mgmt_send_disassoc_frame(&pst_hmac_vap->st_vap_base_info,
            pst_hmac_user->st_user_base_info.auc_user_mac_addr, MAC_ASOC_NOT_AUTH, en_is_protected);
        ...
        if (pdmac_disasoc_misc_stru->en_disasoc_reason != DMAC_DISASOC_MISC_CHANNEL_MISMATCH) {
            /* 发送去认证帧到AP */
            hmac_mgmt_send_disassoc_frame(&pst_hmac_vap->st_vap_base_info,
                pst_hmac_user->st_user_base_info.auc_user_mac_addr, MAC_UNSPEC_REASON, en_is_protected);
        }
        ...
        hmac_sta_handle_disassoc_rsp(pst_hmac_vap, HMAC_REPORT_DEAUTH, pdmac_disasoc_misc_stru->en_disasoc_reason);
    }
