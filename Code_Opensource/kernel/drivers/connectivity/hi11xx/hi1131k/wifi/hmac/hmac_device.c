...
OAL_STATIC uint32_t hmac_vap_destroy_all(mac_device_stru *pst_device)
{
    uint8_t vap_idx;
    hmac_vap_stru *hmac_vap = OAL_PTR_NULL;
    uint32_t res;
    for (vap_idx = 0; vap_idx < pst_device->uc_vap_num; vap_idx++) {
        /* 获取最右边一位为1的位数，此值即为vap的数组下标 */
        hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(pst_device->auc_vap_id[vap_idx]);
        if (hmac_vap == OAL_PTR_NULL) {
            OAM_WARNING_LOG1(0, OAM_SF_ANY, "{hmac_device_exit::mac_res_get_hmac_vap failed vap_idx[%u].}", vap_idx);
            continue;
        }

        res  = hmac_vap_destroy(hmac_vap);
        if (res != OAL_SUCC) {
            OAM_WARNING_LOG1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                "{hmac_vap_destroy_all::hmac_vap_destroy failed[%d].}", res);
            return res;
        }
        pst_device->auc_vap_id[vap_idx] = 0;
    }

    /* 释放公共结构体 以及 对应衍生特性 */
    res = mac_device_exit(pst_device);
    if (res != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{hmac_vap_destroy_all::p_device_destroy_fun failed[%d].}", res);
        return res;
    }
    return OAL_SUCC;
}
...
oal_uint32 hmac_device_exit(mac_board_stru *pst_board, mac_chip_stru *pst_chip, hmac_device_stru *pst_hmac_device)
{
    ...
    /* 取消OBSS保护老化定时器 */
    hmac_protection_stop_timer(&(pst_vap->st_vap_base_info));

    memset_s(&st_down_vap, OAL_SIZEOF(mac_cfg_down_vap_param_stru), 0, OAL_SIZEOF(mac_cfg_down_vap_param_stru));
    ul_return = hmac_config_del_vap(&pst_vap->st_vap_base_info, OAL_SIZEOF(mac_cfg_down_vap_param_stru),
    ...
    ul_return = hmac_vap_destroy_all(pst_device);
    if (ul_return != OAL_SUCC) {
        return ul_return;
    }
...
OAL_STATIC oal_uint32  hmac_chip_exit(mac_board_stru *pst_board, mac_chip_stru *pst_chip)
{
    ...
    for (uc_device = 0; (uc_device < pst_chip->uc_device_nums) &&
...
OAL_STATIC oal_uint32 hmac_cfg_vap_init(mac_device_stru *pst_device)
{
    ...
    ul_ret = mac_res_alloc_hmac_vap(&pst_device->uc_cfg_vap_id,
                                    OAL_OFFSET_OF(hmac_vap_stru, st_vap_base_info)); //lint !e78
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{hmac_cfg_vap_init::mac_res_alloc_hmac_vap failed[%d].}", ul_ret);
        return ul_ret;
    }
    /*lint +e413*/
    ...
    memset_s(pst_vap, OAL_SIZEOF(hmac_vap_stru), 0, OAL_SIZEOF(hmac_vap_stru));
{
...
#endif
    ul_ret = hmac_vap_init(pst_vap, pst_device->uc_chip_id, pst_device->uc_device_id, pst_device->uc_cfg_vap_id,
                           &st_param);
    if (ul_ret != OAL_SUCC) {
    ...
    if (snprintf_s(ac_vap_netdev_name, MAC_NET_DEVICE_NAME_LENGTH, MAC_NET_DEVICE_NAME_LENGTH - 1, "Hisilicon%d",
        pst_device->uc_cfg_vap_id) < EOK) {
        OAM_WARNING_LOG0(pst_device->uc_cfg_vap_id, OAM_SF_ANY, "{hmac_cfg_vap_init::snprintf_s failed!.}");
        return OAL_FAIL;
    }

    ul_ret = hmac_vap_creat_netdev(pst_vap, ac_vap_netdev_name, sizeof(ac_vap_netdev_name),
        (oal_int8 *)(pst_device->auc_hw_addr));
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{hmac_cfg_vap_init::hmac_vap_creat_netdev fail[%d].}", ul_ret);
        return ul_ret;
    }
...
oal_uint32 hmac_send_evt2wal(mac_vap_stru *pst_mac_vap, oal_uint8 uc_evtid, const oal_uint8 *puc_evt,
    oal_uint32 ul_evt_len)
{
    ...
    if (memcpy_s((oal_void *)pst_event->auc_event_data, ul_evt_len, (oal_void *)puc_evt, ul_evt_len) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_SCAN, "hmac_send_evt2wal::memcpy_s fail!");
...
oal_uint32 hmac_config_host_dev_init(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, const oal_uint8 *puc_param)
{
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{hmac_device_init:: pst_mac_device[%d] NULL !}", pst_mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_UNLIKELY(pst_hmac_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{hmac_device_init::pst_hmac_device[%] null!}", pst_mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    hmac_pkt_mem_opt_init(pst_hmac_device);
#endif

    hmac_protection_start_timer(pst_mac_vap);
    return OAL_SUCC;
}
...
oal_uint32 hmac_config_host_dev_exit(mac_vap_stru *pst_mac_vap)
{
    ...
    if (OAL_UNLIKELY(pst_hmac_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY,
            "{hmac_config_host_dev_exit::pst_hmac_device[%] null!}", pst_mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    hmac_protection_stop_timer(pst_mac_vap);
...
oal_uint32  hmac_device_init(oal_uint8 *puc_device_id, mac_chip_stru *pst_chip)
{
    ...
    *puc_device_id = uc_dev_id;
    ...
    OAL_WAIT_QUEUE_INIT_HEAD(&(pst_hmac_device->psm_flt_stat_query.wait_queue));
    OAL_WAIT_QUEUE_INIT_HEAD(&(pst_hmac_device->psm_beacon_query.wait_queue));
...
oal_uint32  hmac_board_init(mac_board_stru *pst_board)
{
    ...
    for (uc_chip = 0; uc_chip < ul_chip_max_num; uc_chip++) {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG2(0, OAM_SF_ANY, "{hmac_init_event_process::hmac_chip_init failed[%d], uc_chip_id[%d].}",
                ul_ret, uc_chip);
            return OAL_FAIL;
        }
...
oal_uint32  hmac_device_init(oal_uint8  uc_device_id, mac_chip_stru *pst_dst_chip)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG2(0, OAM_SF_ANY, "{hmac_device_init::mac_device_init failed[%d], chip_ver[0x%x].}",
            ul_ret, pst_dst_chip->ul_chip_ver);
    ...
    for (uc_chip = 0; uc_chip < ul_chip_max_num; uc_chip++) {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG2(0, OAM_SF_ANY,
                "{hmac_init_event_process::hmac_chip_init failed[%d], uc_chip_id[%d].}", ul_ret, uc_chip);
            return OAL_FAIL;
        }
