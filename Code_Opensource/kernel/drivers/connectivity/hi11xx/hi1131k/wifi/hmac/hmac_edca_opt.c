...
OAL_STATIC oal_bool_enum_uint8 hmac_edca_opt_check_is_tcp_data(mac_ip_header_stru *pst_ip)
{
    ...
    if ((us_ip_len - uc_ip_header_len) == uc_tcp_header_len) {
        OAM_INFO_LOG3(0, OAM_SF_TX, "{hmac_edca_opt_check_is_tcp_data:is tcp ack, us_ip_len = %d,"
            "uc_ip_header_len = %d, uc_tcp_header_len = %d", us_ip_len, uc_ip_header_len, uc_tcp_header_len);
        return OAL_FALSE;
    } else {
        OAM_INFO_LOG3(0, OAM_SF_TX, "{hmac_edca_opt_check_is_tcp_data:is tcp data, us_ip_len = %d,"
            "uc_ip_header_len = %d, uc_tcp_header_len = %d", us_ip_len, uc_ip_header_len, uc_tcp_header_len);
        return OAL_TRUE;
    }
}
    ...
    for (; pst_list_pos != &(pst_vap->st_mac_user_list_head); pst_list_pos = pst_list_pos->pst_next) {
        ...
        for (uc_ac_idx = 0; uc_ac_idx < WLAN_WME_AC_BUTT; uc_ac_idx++) {
            for (uc_data_idx = 0; uc_data_idx < WLAN_TXRX_DATA_BUTT; uc_data_idx++) {
                OAM_INFO_LOG4(0, OAM_SF_TX, "mac_edca_opt_stat_traffic_num, assoc_id = %d, uc_ac = %d, uc_idx = %d,"
                    "pkt_num = %d", pst_user->us_assoc_id, uc_ac_idx, uc_data_idx,
...
{
    oal_uint8        aast_uc_traffic_num[WLAN_WME_AC_BUTT][WLAN_TXRX_DATA_BUTT] = {{0}};
    ...
    if (hmac_edca_opt_stat_traffic_num(pst_hmac_vap, aast_uc_traffic_num) != OAL_SUCC) {
        OAM_WARNING_LOG0(0, OAM_SF_ANY, "{hmac_edca_opt_stat_traffic_num error !}");
    }
    ...
    ***************************************************************************/
    ...
    if (memcpy_s(frw_get_event_payload(pst_event_mem), OAL_SIZEOF(aast_uc_traffic_num),
        ...
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_edca_opt_timeout_fn::memcpy_s fail!");
...
oal_void  hmac_edca_opt_rx_pkts_stat(oal_uint16 us_assoc_id, oal_uint8 uc_tidno, mac_ip_header_stru *pst_ip)
{
    ...
    if (pst_ip->uc_protocol == MAC_UDP_PROTOCAL) {
        ...
        OAM_INFO_LOG4(0, OAM_SF_RX, "{hmac_edca_opt_rx_pkts_stat:is udp_data, assoc_id = %d, tidno = %d, type = %d,"
            "num = %d", pst_hmac_user->st_user_base_info.us_assoc_id, uc_tidno, WLAN_RX_UDP_DATA,
            pst_hmac_user->aaul_txrx_data_stat[WLAN_WME_TID_TO_AC(uc_tidno)][WLAN_RX_UDP_DATA]);
        ...
        if (hmac_edca_opt_check_is_tcp_data(pst_ip)) {
            ...
            OAM_INFO_LOG4(0, OAM_SF_RX, "{hmac_edca_opt_rx_pkts_stat:is tcp_data, assoc_id = %d, tidno = %d,"
                "type = %d,num = %d", pst_hmac_user->st_user_base_info.us_assoc_id, uc_tidno, WLAN_RX_TCP_DATA,
                pst_hmac_user->aaul_txrx_data_stat[WLAN_WME_TID_TO_AC(uc_tidno)][WLAN_RX_TCP_DATA]);
        }
    } else {
...
oal_void  hmac_edca_opt_tx_pkts_stat(mac_tx_ctl_stru  *pst_tx_ctl, oal_uint8 uc_tidno, mac_ip_header_stru *pst_ip)
{
    ...
    if (pst_ip->uc_protocol == MAC_UDP_PROTOCAL) {
        ...
        OAM_INFO_LOG4(0, OAM_SF_TX, "{hmac_edca_opt_tx_pkts_stat:is udp_data, assoc_id = %d, tidno = %d, type = %d,"
            "num = %d", pst_hmac_user->st_user_base_info.us_assoc_id, uc_tidno, WLAN_TX_UDP_DATA,
            pst_hmac_user->aaul_txrx_data_stat[WLAN_WME_TID_TO_AC(uc_tidno)][WLAN_TX_UDP_DATA]);
    } else if (pst_ip->uc_protocol == MAC_TCP_PROTOCAL) {
        if (hmac_edca_opt_check_is_tcp_data(pst_ip)) {
            pst_hmac_user->aaul_txrx_data_stat[WLAN_WME_TID_TO_AC(uc_tidno)][WLAN_TX_TCP_DATA]++;
            OAM_INFO_LOG4(0, OAM_SF_TX, "{hmac_edca_opt_tx_pkts_stat:is tcp_data, assoc_id = %d, tidno = %d,"
                "type = %d, num = %d", pst_hmac_user->st_user_base_info.us_assoc_id, uc_tidno, WLAN_TX_TCP_DATA,
                pst_hmac_user->aaul_txrx_data_stat[WLAN_WME_TID_TO_AC(uc_tidno)][WLAN_TX_TCP_DATA]);
        }
    } else {
