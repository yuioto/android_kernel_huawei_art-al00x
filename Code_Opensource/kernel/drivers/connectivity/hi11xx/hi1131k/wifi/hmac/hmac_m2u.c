...
oal_uint32 hmac_m2u_add_member_list(hmac_m2u_grp_list_entry_stru *pst_grp_list,
    hmac_m2u_list_update_stru *pst_list_entry)
{
    ...
    pst_grp_member = (hmac_m2u_grp_member_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL,
                                                               ...
                                                               OAL_TRUE);
    if (OAL_UNLIKELY(pst_grp_member == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(pst_list_entry->pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
            "{hmac_m2u_add_member_list::pst_grp_member null.}");
...
OAL_STATIC hmac_m2u_grp_list_entry_stru *hmac_m2u_create_grp_list(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_grp_addr)
{
    ...
    if (OAL_UNLIKELY(pst_m2u == OAL_PTR_NULL)) {
        return pst_grp_list_member;
    }

    pst_snp_list = &(pst_m2u->st_m2u_snooplist);
    pst_grp_list_member = hmac_m2u_find_group_list(pst_hmac_vap, puc_grp_addr);
    if (pst_grp_list_member == OAL_PTR_NULL) {
        pst_grp_list_member = (hmac_m2u_grp_list_entry_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL,
                                                                            ...
                                                                            OAL_TRUE);
        if (OAL_UNLIKELY(pst_grp_list_member == OAL_PTR_NULL)) {
            OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                "{hmac_m2u_create_grp_list::pst_grp_list_member null.}");
            return (pst_grp_list_member);
        }
...
oal_uint32 hmac_m2u_update_snoop_list(hmac_m2u_list_update_stru *pst_list_entry)
{
    ...
    if (OAL_UNLIKELY(pst_grp_list == OAL_PTR_NULL)) {
    ...
    if (pst_list_entry->ul_src_ip_addr) {
        ...
        pst_grp_member_list = hmac_m2u_find_member_src(pst_grp_list, pst_list_entry->auc_grp_member,
                                                       pst_list_entry->ul_src_ip_addr);
        if (pst_grp_member_list != OAL_PTR_NULL) {
        ...
        if (pst_grp_member_list != OAL_PTR_NULL) {
            ...
            if (pst_grp_member_list->ul_src_ip_addr) {
                ...
                if (pst_grp_member_list->en_mode == HMAC_M2U_IGMP_CMD_ADD_INC_LIST) {
            ...
            } else if (pst_list_entry->en_cmd == HMAC_M2U_IGMP_CMD_ADD_EXC_LIST) {
                OAM_INFO_LOG1(pst_list_entry->pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                    "{hmac_m2u_update_snoop_list::pst_list_entry->en_cmd is [%d].}", pst_list_entry->en_cmd);
                oal_dlist_delete_entry(&(pst_grp_member_list->st_member_entry));
                OAL_MEM_FREE(pst_grp_member_list, OAL_TRUE);
            }
        } else { /* 特定组中不存在该节点，并且更新节点的CMD不为EXC则ADD */
            if (pst_list_entry->en_cmd != HMAC_M2U_IGMP_CMD_ADD_EXC_LIST) {
                OAM_INFO_LOG1(pst_list_entry->pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
...
oal_void hmac_m2u_show_snoop_deny_table(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_snp_list->uc_deny_count == 0) {
        return;
    }
...
oal_void hmac_m2u_print_all_snoop_list(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if ((pst_hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) && (pst_m2u->en_snoop_enable) &&
        (pst_snp_list != OAL_PTR_NULL)) {
        OAL_DLIST_SEARCH_FOR_EACH(pst_grp_list_entry, &(pst_snp_list->st_grp_list)) {
            pst_grp_list_member = (hmac_m2u_grp_list_entry_stru *)OAL_DLIST_GET_ENTRY(pst_grp_list_entry,
                hmac_m2u_grp_list_entry_stru, st_grp_entry);
            /* 打印组播组的mac地址 */
            ...
            OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_grp_member_entry, pst_grp_member_entry_temp,
                &(pst_grp_list_member->st_src_list)) {
                pst_grp_member = (hmac_m2u_grp_member_stru *)OAL_DLIST_GET_ENTRY(pst_grp_member_entry,
                    hmac_m2u_grp_member_stru, st_member_entry);
                ...
                l_ret += snprintf_s(ac_tmp_buff, OAL_SIZEOF(ac_tmp_buff), OAL_SIZEOF(ac_tmp_buff) - 1,
                                    ...
                                    (oal_uint32)(pst_grp_member->auc_grp_member_addr[5]), pst_grp_member->en_mode);
                oam_print(ac_tmp_buff);
            }
        }
    } else {
        OAM_WARNING_LOG2(0, OAM_SF_M2U, "{hmac_m2u_print_all_snoop_list::en_snoop_enable en_vap_mode = [%d].[%d]}\r\n",
                         pst_m2u->en_snoop_enable, pst_hmac_vap->st_vap_base_info.en_vap_mode);
    }
    if (l_ret < EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_M2U, "{hmac_m2u_print_all_snoop_list::snprintf_s failed!}");
        return;
    }
}
/*lint +e550*/
...
oal_uint32 hmac_m2u_igmp_v1v2_update(hmac_vap_stru *pst_hmac_vap, hmac_m2u_list_update_stru *pst_list_entry,
    mac_igmp_header_stru *pst_igmp)
{
    ...
    if (pst_igmp->uc_type != MAC_IGMPV2_LEAVE_TYPE) {
...
oal_uint32 hmac_m2u_igmp_v3_update(hmac_vap_stru *pst_hmac_vap, hmac_m2u_list_update_stru *pst_list_entry,
    mac_igmp_v3_report_stru *pst_igmpr3)
{
    oal_uint32                           ul_group_addr;                  /* to hold group address from group record */
    oal_uint8                            uc_group_addr[WLAN_MAC_ADDR_LEN]; /* group multicast mac address */
    ...
    if (us_no_grec == 0) {
        OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
            "{hmac_m2u_igmp_v3_update::us_no_grec is 0.}");
        return OAL_FAIL;
    }
    while (us_no_grec) {
        ...
        if (!IS_IGMPV3_MODE(pst_grec->uc_grec_type)) {
            OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                "{hmac_m2u_igmp_v3_update::uc_grec_type is [%x] not inside the six cmd.}", pst_grec->uc_grec_type);
            /* move the grec to next group record */
            pst_grec = (mac_igmp_v3_grec_stru *)((oal_uint8 *)pst_grec + IGMPV3_GRP_REC_LEN(pst_grec));
            us_no_grec--;
            continue;
        ...
        /* IGMP V3 exc的处理 */
        if ((pst_grec->uc_grec_type) == IGMPV3_CHANGE_TO_EXCLUDE ||
            (pst_grec->uc_grec_type) == IGMPV3_MODE_IS_EXCLUDE) {
            OAM_INFO_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                "{hmac_m2u_igmp_v3_update::pst_grec->uc_grec_type is %d.}", pst_grec->uc_grec_type);
            pst_list_entry->en_cmd = HMAC_M2U_IGMP_CMD_ADD_EXC_LIST;
            pst_grp_list = hmac_m2u_find_group_list(pst_hmac_vap, pst_list_entry->auc_grp_addr);
            /* 更新组播组内成员的状态，如果组播组内存在该目标成员，则清空目标成员 */
            if (pst_grp_list == OAL_PTR_NULL) {
                OAM_INFO_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                    "{hmac_m2u_igmp_v3_update::pst_grp_list is null.}");
            } else {
                hmac_m2u_remove_all_member_grp(pst_grp_list, pst_list_entry->auc_grp_member);
            }
            ...
            if (us_no_srec == 0) {
                OAM_INFO_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                    "{hmac_m2u_igmp_v3_update::not exist the user us_no_srec is %d.}", us_no_srec);
                pst_list_entry->ul_src_ip_addr = 0;
                pst_list_entry->en_cmd = HMAC_M2U_IGMP_CMD_ADD_INC_LIST;
                ul_ret = hmac_m2u_update_snoop_list(pst_list_entry);
            }
        /* IGMP V3 INC的处理 */
        } else if ((pst_grec->uc_grec_type) == IGMPV3_CHANGE_TO_INCLUDE ||
            (pst_grec->uc_grec_type) == IGMPV3_MODE_IS_INCLUDE) {
            pst_list_entry->en_cmd = HMAC_M2U_IGMP_CMD_ADD_INC_LIST;
            pst_grp_list = hmac_m2u_find_group_list(pst_hmac_vap, pst_list_entry->auc_grp_addr);
            if (pst_grp_list == OAL_PTR_NULL) {
                OAM_INFO_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
        ...
        } else if ((pst_grec->uc_grec_type) == IGMPV3_ALLOW_NEW_SOURCES) {
            pst_list_entry->en_cmd = HMAC_M2U_IGMP_CMD_ADD_INC_LIST;
        }
        ...
        while (us_no_srec) {
            ...
            if ((pst_grec->uc_grec_type) != IGMPV3_BLOCK_OLD_SOURCES) {
                ul_ret = hmac_m2u_update_snoop_list(pst_list_entry);
...
oal_void hmac_m2u_snoop_inspecting(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user, oal_netbuf_stru *pst_buf)
{
    ...
    oal_uint8                            *puc_src_addr      = OAL_PTR_NULL;
    ...
    pst_m2u = (hmac_m2u_stru *)(pst_hmac_vap->pst_m2u);
    if (pst_m2u->en_snoop_enable == OAL_FALSE) {
        OAM_INFO_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
            "{hmac_m2u_snoop_inspecting::snoop is [%d] not enable}", pst_m2u->en_snoop_enable);
        return;
    }
    ...
    if ((pst_hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) &&
        ...
        if (OAL_HOST2NET_SHORT(ETHER_TYPE_IP) != pst_ether_header->us_ether_type) {
            OAM_INFO_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                          "{hmac_m2u_snoop_inspecting::ether type is not IP Protocol.[0x%x]}",
                          OAL_HOST2NET_SHORT(pst_ether_header->us_ether_type));
            return;
        }
        /*lint +e778*/
        ...
        if (pst_ip_hdr->uc_protocol != IPPROTO_IGMP) {
            OAM_INFO_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                "{hmac_m2u_snoop_inspecting::ip type is not IGMP Protocol[%x].}", pst_ip_hdr->uc_protocol);
            return;
        }
        ...
        if (uc_ip_hdr_len < MIN_IP_HDR_LEN) {
            OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                "{hmac_m2u_snoop_inspecting::ip_hdr_len is [%x].}", uc_ip_hdr_len);
            return;
        }
        /* v1 & v2 igmp */
        pst_igmp = (mac_igmp_header_stru *)(oal_netbuf_data(pst_buf) +
            OAL_SIZEOF(mac_ether_header_stru) + 4 * uc_ip_hdr_len);
        ...
        if (!IS_IGMP_REPORT_LEAVE_PACKET(pst_igmp->uc_type)) {
            OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                "{hmac_m2u_snoop_inspecting::not igmp report[%x].}", pst_igmp->uc_type);
            return;
        }
    ...
    if (OAL_HOST2NET_SHORT(ETHER_TUNNEL_TYPE) == pst_ether_header->us_ether_type &&
        ...
        OAM_INFO_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
            "{hmac_m2u_snoop_inspecting::STA Mode && Ether Type is ETHER_TUNNEL_TYPE.}");
                                                      ...
                                                      oal_uint32 ul_timestamp)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_grp_member_entry, &(pst_grp_list->st_src_list))
    {
        pst_grp_member = (hmac_m2u_grp_member_stru *)OAL_DLIST_GET_ENTRY(pst_grp_member_entry,
                                                                         hmac_m2u_grp_member_stru,
                                                                         st_member_entry);
        if (pst_grp_member->ul_src_ip_addr == 0) {
                                                    ...
                                                    oal_uint32 ul_timestamp, oal_uint8 uc_count)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_grp_member_entry, &(pst_grp_list->st_src_list))
    {
        ...
        if (ul_src_ip_addr == pst_grp_member->ul_src_ip_addr) {
            if (pst_grp_member->en_mode == HMAC_M2U_IGMP_CMD_ADD_INC_LIST) {
                if (uc_count > MAX_SNOOP_ENTRIES) {
                    break;
                }
                oal_set_mac_addr(&puc_table[uc_count * WLAN_MAC_ADDR_LEN], pst_grp_member->auc_grp_member_addr);
            ...
            if (pst_grp_member->en_mode == HMAC_M2U_IGMP_CMD_ADD_EXC_LIST) {
                if (uc_count > MAX_SNOOP_ENTRIES) {
                    break;
                }
                oal_set_mac_addr(&puc_table[uc_count * WLAN_MAC_ADDR_LEN], pst_grp_member->auc_grp_member_addr);
                                                   ...
                                                   oal_uint32 ul_src_ip_addr, oal_uint8 *puc_table)
{
    ...
    if (pst_grp_list_member != OAL_PTR_NULL) {
        ...
        uc_count  = hmac_m2u_count_member_src_list(pst_grp_list_member, ul_src_ip_addr, &puc_table[0], ul_nowtime,
                                                   uc_count);
        OAM_INFO_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                      "{hmac_m2u_get_snooplist_member::uc_count is [%d]}", uc_count);
    }
...
OAL_STATIC oal_void hmac_m2u_remove_node_grp(hmac_m2u_grp_list_entry_stru *pst_grp_list, hmac_user_stru *pst_hmac_user)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_grp_member_entry, pst_grp_member_entry_temp, &pst_grp_list->st_src_list)
    {
        pst_grp_member = (hmac_m2u_grp_member_stru *)OAL_DLIST_GET_ENTRY(pst_grp_member_entry,
                                                                         hmac_m2u_grp_member_stru,
                                                                         st_member_entry);
        if ((pst_hmac_user == pst_grp_member->pst_hmac_user) || (pst_hmac_user == OAL_PTR_NULL)) {
            oal_dlist_delete_entry(&(pst_grp_member->st_member_entry));
            OAL_MEM_FREE(pst_grp_member, OAL_TRUE);
        }
    }
...
oal_void hmac_m2u_cleanup_snoopwds_node(hmac_user_stru *pst_hmac_user)
{
    ...
    if ((pst_hmac_vap->st_vap_base_info.en_vap_mode) == WLAN_VAP_MODE_BSS_AP &&
        pst_snp_list != OAL_PTR_NULL && (pst_m2u->en_snoop_enable)) {
        OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_grp_list_entry, pst_grp_list_entry_temp, &(pst_snp_list->st_grp_list))
        {
            pst_grp_list = (hmac_m2u_grp_list_entry_stru *)OAL_DLIST_GET_ENTRY(pst_grp_list_entry,
                                                                               hmac_m2u_grp_list_entry_stru,
            ...
            if ((oal_dlist_is_empty(&(pst_grp_list->st_src_list))) == OAL_TRUE) {
                oal_dlist_delete_entry(&(pst_grp_list->st_grp_entry));
                OAL_MEM_FREE(pst_grp_list, OAL_TRUE);
            }
        }
...
oal_void hmac_m2u_snoop_convert_count(hmac_vap_stru *pst_vap, oal_uint32 uc_newmaccnt, oal_uint32 ul_ret,
...
oal_void hmac_m2u_snoop_change_mac_hdr(hmac_m2u_stru *pst_m2u, mac_ether_header_stru  **pst_ucast_ether_hdr,
    oal_netbuf_stru *pst_buf)
{
    ...
    /* 加自定义tunnel协议 */
    if (pst_m2u->en_mcast_mode & BIT0) { /* en_mcast_mode为1的分支 */
...
oal_uint32 hmac_m2u_snoop_convert(hmac_vap_stru *pst_vap, oal_netbuf_stru *pst_buf)
{
    ...
    if (pst_m2u->en_snoop_enable == OAL_FALSE) {
        OAM_INFO_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
            "{hmac_m2u_snoop_convert::snoop is [%d] not enable}", pst_m2u->en_snoop_enable);
        OAM_STAT_VAP_INCR(pst_vap->st_vap_base_info.uc_vap_id, tx_m2u_mcast_cnt, 1);
        return HMAC_TX_PASS;
    }
    pst_ether_hdr  = (mac_ether_header_stru *)oal_netbuf_data(pst_buf);
    ...
    if (uc_newmaccnt > 0 && pst_m2u->en_mcast_mode == 0) { /* en_mcast_mode为0的分支 */
        OAM_INFO_LOG2(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
            "{hmac_m2u_snoop_convert:: %d user found , mcast mode is %d}", uc_newmaccnt, pst_m2u->en_mcast_mode);
        OAM_STAT_VAP_INCR(pst_vap->st_vap_base_info.uc_vap_id, tx_m2u_mcast_cnt, 1);
        return HMAC_TX_PASS;
    } else if (uc_newmaccnt == 0) { /* 如果没有STA需要此netbuf,则不需要发送，丢弃该组播帧 */
        OAM_INFO_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
    ...
    do {
        ...
        if (!oal_compare_mac_addr(puc_dstmac, auc_empty_entry_mac)) {
            OAM_INFO_LOG2(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                "{hmac_m2u_snoop_convert::dstmac == NULL && newmaccnt is %d && discard mcast is %d}", uc_newmaccnt,
                pst_m2u->en_discard_mcast);
        ...
        if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
            OAM_WARNING_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                "{hmac_m2u_snoop_convert::find user fail[%d].}", ul_ret);
            hmac_free_netbuf_list(pst_buf);
            OAM_STAT_VAP_INCR(pst_vap->st_vap_base_info.uc_vap_id, tx_m2u_ucast_droped, 1);
            hmac_m2u_convert_loop_end(pst_copy_buf, &pst_buf, &uc_newmaccnt, &uc_newmacidx);
            continue;
        ...
        if (OAL_UNLIKELY(pst_user->st_user_base_info.en_user_asoc_state != MAC_USER_STATE_ASSOC)) {
            ...
            OAM_WARNING_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                "{hmac_m2u_snoop_convert::asoc state is [%d].}", pst_user->st_user_base_info.en_user_asoc_state);
            /* 组播转单播发送循环的末尾处理 */
            hmac_m2u_convert_loop_end(pst_copy_buf, &pst_buf, &uc_newmaccnt, &uc_newmacidx);
            continue;
        }
