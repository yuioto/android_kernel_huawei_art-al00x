...
oal_int64 g_ll_mk1_32[3][128] = {
    {
        ...
        -410,  -648,  -882,  -1111, -1334, -1552, -1764, -1969, -2166, -2357, -2539, -2712, -2876, -3031, -3176,\
        -3311, -3434, -3547, -3647, -3736, -3811, -3874, -3922, -3957, -3977, -3982, -3972, -3946, -3903, -3843,\
        -3766, -3671, -3558, -3427, -3276, -3105, -2915, -2703, -2471, -2217, -1942, -1644, -1323, -978,  -610,\
        -218,  199,   641,   1109,  1603,  2124, 2671, 3246, 3849, 4481, 5141, 5830, 6550, 7300,
    },
    {
        -889, -2600, -4224, -5761, -7214, -8584, -9872, -11080, -12209, -13262, -14239, -15142, -15973, -16733, -17424,\
        -18047, -18605, -19097, -19526, -19894, -20202, -20451, -20643, -20780, -20863, -20894, -20873, -20804, -20687,\
        -20524, -20316, -20065, -19772, -19440, -19068, -18660, -18217, -17740, -17230, -16690, -16120, -15523, -14899,\
        -14251, -13579, -12887, -12174, -11442, -10694, -9930,  -9153, -8363,  -7562,  -6752,  -5935,  -5111,  -4282,\
        -3451,  -2618,  -1785,  -953,   -124,   699,    1517,   2327,   3128,   3918,   4696, 5460,   6209,   6941,\
        7654,   8347,   9019,   9668,   10292,  10890,  11461,  12002,  12512,  12990,  13434,  13843,  14215,  14549,\
        14842,  15094,  15303,  15468,  15586,  15656,  15678,  15648,  15566,  15431,  15240,  14992,  14686,  14319,\
        13891,  13400,  12845, 12223,  11534,  10775,  9946,   9045,   8069,   7018,   5891,   4684,   3398,   2030,\
        580,    -956,   -2577,  -4286,  -6085,  -7974, -9956,  -12031, -14202, -16469, -18835, -21301, -23869, -26540,\
        -29315,
    },
    {
        622,    1817,   2949,   4017,   5025,   5971,   6859,   7688,   8460,   9176,   9838,   10446,  11002,  11506,\
        11961,  12366,  12724, 13035,  13301,  13522,  13701,  13837,  13932,  13988,  14005,  13985,  13929,  13837,\
        13712,  13554,  13364,  13144,  12895,  12617, 12312,  11982,  11627,  11248,  10847,  10425,  9983,   9522,\
        9043,   8547,   8036,   7511,   6973,   6422,   5861,   5290,   4710, 4124,   3531,   2933,   2331,   1726,\
        1119,   513,    -94,    -698,   -1299,  -1896,  -2488,  -3073,  -3651,  -4219,  -4778,  -5326, -5862,  -6384,\
        -6891,  -7383,  -7858,  -8315,  -8753,  -9171,  -9567,  -9940,  -10290, -10614, -10913, -11184, -11427, -11640,\
        -11823, -11974, -12092, -12175, -12223, -12235, -12209, -12145, -12040, -11894, -11706, -11475, -11199, -10877,\
        -10509, -10092, -9627,  -9111, -8543,  -7923,  -7249,  -6520,  -5735,  -4893,  -3992,  -3032,  -2011,  -928,\
        218,    1428,   2703,   4045,   5454,   6933,   8481, 10100,  11792,  13557,  15396,  17312,  19304,  21374,\
        23523,  25753,
    },
};
...
{
    oal_int64 ll_result;
    if ((ll_b == 0) || (((oal_uint64)ll_b << 1) == 0)) {
        ll_result = (oal_int64)ll_a;
    } else {
        if (((ll_a >= 0) && (ll_b > 0)) || ((ll_a <= 0) && (ll_b < 0))) {
            ll_result = (oal_int64)((((oal_uint64)ll_a << 1) + ll_b) / ((oal_uint64)ll_b << 1));
        } else {
            ll_result = (oal_int64)((((oal_uint64)ll_a << 1) - ll_b) / ((oal_uint64)ll_b << 1));
        }
    }
...
void hmac_dpd_cordic(oal_int64 ll_real, oal_int64 ll_image, oal_int64* pll_amp, oal_int64* pll_theta1,
    oal_int64* pll_theta2)
{
    unsigned char  us_idx;
    oal_int64 ll_x[13] = {0};
    oal_int64 ll_y[13] = {0};
    oal_int64 ll_flag;

    oal_int64 ll_real_table[13] = {1447, 1891, 2008, 2037, 2045, 2046, 2047, 2047, 2047, 2047, 2047, 2047, 2047};
    oal_int64 ll_imag_table[13] = {1447, 783, 399, 201, 100, 50, 25, 13, 6, 3, 2, 1, 0};

    oal_int64 ll_theta1 = 2047;
    oal_int64 ll_theta2 = 0;
    oal_int64 ll_temp1 = 0;
    oal_int64 ll_temp2 = 0; /* real and image */
    oal_int64 ll_thetatmp1;
    oal_int64 ll_thetatmp2; /* real and image */
...
hi1102_complex_stru  hmac_dpd_complex_div(hi1102_complex_stru st_complex_num, hi1102_complex_stru st_complex_denom)
{
    ...
    if (ll_power == 0) {
        ...
        /* 实部 四舍五入 */
        ll_temp = ((oal_uint64)((st_mul_res.ll_real) % ll_power) << 1);
        st_mul_res.ll_real = ((st_mul_res.ll_real) / ll_power);
        if (ll_power < GET_ABS(ll_temp)) {
            if (ll_temp > 0)
        ...
        /* 虚部 四舍五入 */
        ll_temp = ((oal_uint64)((st_mul_res.ll_imag) % ll_power) << 1);
...
hi1102_complex_stru  hmac_dpd_complex_div_const(hi1102_complex_stru st_a, oal_int64 ll_b)
{
    ...
    st_return.ll_real = (oal_int64)((oal_uint64)(st_a.ll_real) >> (oal_uint64)ll_b);
    st_return.ll_imag = (oal_int64)((oal_uint64)(st_a.ll_imag) >> (oal_uint64)ll_b);

    return st_return;
}


OAL_STATIC OAL_INLINE oal_int64  hmac_dpd_complex_pow(hi1102_complex_stru st_a)
{
    return (oal_int64)(st_a.ll_real * st_a.ll_real + st_a.ll_imag * st_a.ll_imag);
}

oal_uint32 hmac_rf_cali_dpd_corr_calc(const oal_uint32* hi1102_dpd_cali_data_read,
                                      oal_uint32* hi1102_dpd_cali_data_calc)
{
    ...
    for (us_j = 0; us_j < 3; us_j++) {
        ...
        st_sum.ll_imag = 0;
        for (us_idx = 0; us_idx < DPD_CALI_LUT_LENGTH; us_idx++) {
            st_sum = hmac_dpd_complex_add(st_sum, hmac_dpd_complex_multiply_const(g_ll_pa_val[us_idx],
                g_ll_mk1_32[us_j][us_idx]));
        }
        st_ak_16[us_j] = hmac_cali_complex_div_round_closest(st_sum, 1 << 11);

        OAM_WARNING_LOG3(0, OAM_SF_CALIBRATE, "st_ak_16[%d] = %d + j%d\n", us_j, (int)st_ak_16[us_j].ll_real,
            (int)st_ak_16[us_j].ll_imag);
    }
    ...
    for (us_idx = 0; us_idx < DPD_CALI_LUT_LENGTH; us_idx++) {
        ...
        for (us_j = 0; us_j < 10; us_j++) {
            st_xll_32 = hmac_cali_complex_div_round_closest(hmac_dpd_complex_multiply_const(st_vv1_32, ul_vo_fixed),
                1 << 7);

            hmac_dpd_cordic(st_xll_32.ll_real, st_xll_32.ll_imag, &ll_amp, &ll_theta0, &ll_theta1);

            st_tmp1 = hmac_cali_complex_div_round_closest(hmac_dpd_complex_multiply_const(st_ak_16[1], ll_amp),
                1 << 17);
            ...
            st_y_opt_32 = hmac_dpd_complex_multiply_const(g_ll_pa_val[127], ((uint64_t)ul_vo_fixed << 2));
        ...
        st_dpd_lut_fixed = hmac_cali_complex_div_round_closest(hmac_dpd_complex_multiply_const(st_dpd_lut_fixed, 512),
            ll_dpd_base);
#endif
        *(hi1102_dpd_cali_data_calc + us_idx) = ((uint64_t)(SET2COMP(st_dpd_lut_fixed.ll_real))) |
                                                ((uint64_t)(SET2COMP(st_dpd_lut_fixed.ll_imag)) << 11);
