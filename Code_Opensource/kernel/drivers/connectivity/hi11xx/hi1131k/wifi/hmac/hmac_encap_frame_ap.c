...
OAL_STATIC oal_uint32 hmac_get_assoc_comeback_time(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user)
{
...
oal_uint32 hmac_mgmt_encap_asoc_rsp_ap(mac_vap_stru *pst_mac_ap, oal_uint8 *puc_sta_addr, oal_uint8 uc_sta_addr_len,
    oal_uint16 us_assoc_id, mac_status_code_enum_uint16 en_status_code, oal_uint8 *puc_asoc_rsp, oal_uint16 us_type)
{
    ...
    if ((pst_mac_ap == OAL_PTR_NULL) || (puc_sta_addr == OAL_PTR_NULL) || (puc_asoc_rsp == OAL_PTR_NULL) ||
        (uc_sta_addr_len == 0)) {
        OAM_ERROR_LOG1(0, OAM_SF_ASSOC, "{hmac_mgmt_encap_asoc_rsp_ap::uc_sta_addr_len[%d].}", uc_sta_addr_len);
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_mac_ap->uc_vap_id, OAM_SF_ASSOC,
            "{hmac_mgmt_encap_asoc_rsp_ap::pst_mac_device is null ptr}");
        return us_asoc_rsp_len;
    }
    ...
    if (pst_hmac_user == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(0, OAM_SF_ASSOC, "{hmac_mgmt_encap_asoc_rsp_ap::pst_hmac_user(idx=%d) is a null ptr.}",
            us_assoc_id);
        return us_asoc_rsp_len;
    }
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8  hmac_mgmt_is_challenge_txt_equal(uint8_t *puc_data,
    uint8_t *puc_chtxt, uint32_t chtxt_size)
{
    ...
    if (puc_data == OAL_PTR_NULL || puc_chtxt == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{hmac_mgmt_is_challenge_txt_equal::param null}");
        return OAL_FALSE;
    }
    ...
    *puc_auth_resend = OAL_FALSE;
    ...
    if (ul_ret != OAL_SUCC) {
        if (ul_ret == OAL_ERR_CODE_CONFIG_EXCEED_SPEC) {
            OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                "{hmac_encap_auth_rsp_get_user_idx::mac_vap_add_assoc_user fail, users exceed config spec.}");
            return OAL_ERR_CODE_CONFIG_EXCEED_SPEC;
        } else {
            OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
                "{hmac_encap_auth_rsp_get_user_idx::mac_vap_add_assoc_user fail,user_idx[%d]}", *pus_user_index);
            return OAL_FAIL;
        }
    }
    return OAL_SUCC;
...
hmac_ap_auth_process_code_enum_uint8 hmac_encap_auth_rsp_seq1(mac_vap_stru *mac_vap,
    hmac_auth_rsp_param_stru *pst_auth_rsp_param, oal_uint8 *puc_code, mac_user_asoc_state_enum_uint8 *pst_usr_ass_stat)
{
    ...
    *pst_usr_ass_stat = MAC_USER_STATE_BUTT;
    ...
    if ((pst_auth_rsp_param->en_user_asoc_state == MAC_USER_STATE_ASSOC) &&
        ...
        if (mac_mib_get_dot11RSNAMFPC(mac_vap) == OAL_TRUE) {
...
hmac_ap_auth_process_code_enum_uint8 hmac_encap_auth_rsp_seq3(mac_vap_stru *mac_vap,
    hmac_auth_rsp_param_stru *pst_auth_rsp_param, oal_uint8 *puc_code, mac_user_asoc_state_enum_uint8 *pst_usr_ass_stat)
{
    /* 如果不存在，返回错误 */
...
oal_uint32 hmac_encap_auth_rsp_support(hmac_vap_stru *pst_hmac_vap, oal_uint16 us_auth_type)
{
    ...
    if ((pst_hmac_vap->en_auth_mode) != us_auth_type && (pst_hmac_vap->en_auth_mode != WLAN_WITP_ALG_AUTH_BUTT)) {
...
OAL_STATIC uint16_t hmac_get_auth_rsp_timeout(mac_vap_stru *mac_vap)
{
    uint16_t auth_rsp_timeout;

    auth_rsp_timeout = ((g_mac_device_custom_cfg.cmd_auth_rsp_timeout != 0) ? \
                        g_mac_device_custom_cfg.cmd_auth_rsp_timeout : \
                        (uint16_t)mac_mib_get_AuthenticationResponseTimeOut(mac_vap));
    return auth_rsp_timeout;
}


oal_uint16  hmac_encap_auth_rsp(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *pst_auth_rsp, oal_netbuf_stru *pst_auth_req,
    oal_uint8 *puc_chtxt)
{
    ...
    oal_uint8        auc_addr2[WLAN_MAC_ADDR_LEN] = {0};
    hmac_vap_stru   *pst_hmac_vap           = OAL_PTR_NULL;
    ...
    if (OAL_ANY_NULL_PTR3(pst_mac_vap, pst_auth_rsp, pst_auth_req))
#else
    if (OAL_ANY_NULL_PTR4(pst_mac_vap, pst_auth_rsp, pst_auth_req, puc_chtxt))
#endif
    {
        OAM_ERROR_LOG0(0, OAM_SF_AUTH, "{hmac_encap_auth_rsp::param null}");
    ...
    /* 获取STA的地址 */
    mac_get_address2_host(oal_netbuf_header(pst_auth_req), auc_addr2, sizeof(auc_addr2));
    ...
    if (mac_addr_is_zero(auc_addr2)) {
        OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_AUTH, "{hmac_encap_auth_rsp::user mac:%02X:XX:XX:%02X:%02X:%02X"
            "is all 0 and invaild!}", auc_addr2[0], auc_addr2[3], auc_addr2[4], auc_addr2[5]);
    ...
    ul_ret = hmac_encap_auth_rsp_get_user_idx(pst_mac_vap,
        auc_addr2, sizeof(auc_addr2), uc_is_seq1, &st_auth_rsp_handle.st_auth_rsp_param.uc_auth_resend, &us_user_index);
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,
            "{hmac_encap_auth_rsp::hmac_ap_get_user_idx fail, reason[%d]}", ul_ret);
        puc_frame[4] = MAC_UNSPEC_FAIL;
        pst_tx_ctl->us_tx_user_idx = 0xffff;
        pst_tx_ctl->us_mpdu_len    = us_auth_rsp_len;
        return us_auth_rsp_len;
    ...
    if (ul_alg_suppt != OAL_SUCC) {
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,
            "{hmac_encap_auth_rsp::en_auth_mode[%d]!=auth type[%d]!}", pst_hmac_vap->en_auth_mode, us_auth_type);
        puc_frame[4] = MAC_UNSUPT_ALG;
        hmac_user_set_asoc_state(&(pst_hmac_vap->st_vap_base_info), &pst_hmac_user_sta->st_user_base_info,
            MAC_USER_STATE_BUTT);
        return us_auth_rsp_len;
    }
    ...
    if (st_auth_rsp_handle.st_auth_rsp_fun != OAL_PTR_NULL) {
        ul_auth_proc_rst = st_auth_rsp_handle.st_auth_rsp_fun(pst_mac_vap, &st_auth_rsp_handle.st_auth_rsp_param,
            &puc_frame[4], &pst_hmac_user_sta->st_user_base_info.en_user_asoc_state);
        /* 清空 HMAC层TID信息 */
    ...
    auth_rsp_timeout = hmac_get_auth_rsp_timeout(pst_mac_vap);
    OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_AUTH, "{hmac_encap_auth_rsp::auth_proc_rst:%d,auth_rsp_timeout:%d}",
                     ul_auth_proc_rst, auth_rsp_timeout);
    ...
    switch (ul_auth_proc_rst) {
        case HMAC_AP_AUTH_SEQ1_OPEN_ANY:
        {
            ...
            FRW_TIMER_CREATE_TIMER(&pst_hmac_user_sta->st_mgmt_timer, hmac_mgmt_timeout_ap, auth_rsp_timeout,
                pst_hmac_user_sta, OAL_FALSE, OAM_MODULE_ID_HMAC, pst_mac_vap->ul_core_id);
            break;
        }
        case HMAC_AP_AUTH_SEQ1_WEP_NOT_RESEND:
        {
            ...
            FRW_TIMER_CREATE_TIMER(&pst_hmac_user_sta->st_mgmt_timer, hmac_mgmt_timeout_ap, auth_rsp_timeout,
                                   pst_hmac_user_sta, OAL_FALSE, OAM_MODULE_ID_HMAC, pst_mac_vap->ul_core_id);
            pst_hmac_user_sta->st_user_base_info.st_key_info.en_cipher_type = mac_get_wep_type(pst_mac_vap,
                mac_get_wep_default_keyid(pst_mac_vap));
            break;
        }
        case HMAC_AP_AUTH_SEQ1_WEP_RESEND:
        {
            ...
            /* 重启超时定时器 */
            frw_timer_restart_timer(&pst_hmac_user_sta->st_mgmt_timer, pst_hmac_user_sta->st_mgmt_timer.ul_timeout,
                                    OAL_FALSE);
            break;
        }
        ...
        case HMAC_AP_AUTH_SEQ3_WEP_COMPLETE:
        {
            ...
            if (hmac_mgmt_is_challenge_txt_equal(puc_chtxt,
                pst_hmac_user_sta->auc_ch_text, OAL_SIZEOF(pst_hmac_user_sta->auc_ch_text)) == OAL_TRUE) {
