...
oal_void  hmac_sms4_crypt(const oal_uint8 *puc_input, oal_uint8 *puc_output, const oal_uint32 *puc_rk,
                          oal_uint32 uc_rk_len)
{
    oal_uint32         r;
    oal_uint32         mid;
    oal_uint32         x[4] = {0};
    ...
    if (memcpy_s(x, sizeof(x), puc_input, SMS4_PN_LEN) != EOK) {
        return ;
    }
...
oal_void  hmac_sms4_keyext(const oal_uint8 *puc_Key, oal_uint32 *puc_rk, oal_uint32 uc_rk_len)
{
    oal_uint32 r;
    oal_uint32 mid;
    oal_uint32 x[4] = {0};

    if (memcpy_s(x, sizeof(x), puc_Key, SMS4_PN_LEN) != EOK) { /* 注意输入参数地址对齐问题 */
        return ;
    }
#if (_PRE_LITTLE_CPU_ENDIAN == _PRE_CPU_ENDIAN)
