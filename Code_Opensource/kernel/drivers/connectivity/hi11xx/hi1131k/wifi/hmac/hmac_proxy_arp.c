...
oal_err_code_enum_uint32 hmac_proxy_get_mac_by_ipv4(mac_vap_stru *pst_mac_vap, oal_uint32 ul_ipv4, oal_uint8 *puc_mac)
{
    ...
    if (OAL_UNLIKELY((pst_mac_vap == OAL_PTR_NULL) || (puc_mac == OAL_PTR_NULL))) {
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_mac_vap->pst_vap_proxyarp->ast_ipv4_head[ul_user_hash_value]))
    {
        ...
        if (memcpy_s(puc_mac, WLAN_MAC_ADDR_LEN, pst_hash->auc_mac, WLAN_MAC_ADDR_LEN) != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_PROXYARP, "{mac_vap_find_user_by_macaddr::memcpy_s failed!.}");
            return OAL_FAIL;
        }
        return OAL_SUCC;
    }
...
oal_err_code_enum_uint32 hmac_proxy_remove_ipv4(mac_vap_stru *pst_mac_vap, oal_uint32 ul_ipv4)
{
    ...
    if (pst_mac_vap->pst_vap_proxyarp->uc_ipv4_num == 0) {
        return OAL_SUCC;
    }

    ul_user_hash_value = HMAC_PROXY_IPV4_HASH(ul_ipv4);

    OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_entry, pst_dlist_tmp,
        &(pst_mac_vap->pst_vap_proxyarp->ast_ipv4_head[ul_user_hash_value])) {
        pst_hash = (hmac_proxy_ipv4_hash_stru *)pst_entry;
...
oal_err_code_enum_uint32 hmac_proxy_add_ipv4(mac_vap_stru *pst_mac_vap, oal_uint32 ul_ipv4, oal_uint8 *puc_mac)
{
    ...
    if (memcpy_s(pst_hash->auc_mac, WLAN_MAC_ADDR_LEN, puc_mac, OAL_MAC_ADDR_LEN) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_PROXYARP, "hmac_proxy_add_ipv4 err! melloc err!");
        OAL_MEM_FREE(pst_hash, OAL_TRUE);
        return OAL_FAIL;
    }
...
oal_err_code_enum_uint32 hmac_proxy_get_mac_by_ipv6(mac_vap_stru *pst_mac_vap, oal_in6_addr *pst_ipv6,
    oal_uint8 *puc_mac, oal_uint32 puc_mac_len)
{
    oal_uint32 ul_user_hash_value;
    hmac_proxy_ipv6_hash_stru *pst_hash = OAL_PTR_NULL;
    oal_dlist_head_stru *pst_entry = OAL_PTR_NULL;

    if (OAL_UNLIKELY((pst_mac_vap == OAL_PTR_NULL) || (pst_ipv6 == OAL_PTR_NULL) || (puc_mac == OAL_PTR_NULL))) {
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_mac_vap->pst_vap_proxyarp->ast_ipv6_head[ul_user_hash_value]))
    {
        ...
        if (memcpy_s(puc_mac, puc_mac_len, pst_hash->auc_mac, WLAN_MAC_ADDR_LEN) != EOK)   {
            OAM_ERROR_LOG0(0, OAM_SF_PROXYARP, "{hmac_proxy_get_mac_by_ipv6::memcpy_s failed!.}");
            return OAL_FAIL;
        }
        return OAL_SUCC;
    }
...
oal_err_code_enum_uint32 hmac_proxy_remove_ipv6(mac_vap_stru *pst_mac_vap, oal_in6_addr *pst_ipv6)
{
    ...
    if (pst_mac_vap->pst_vap_proxyarp->uc_ipv6_num == 0) {
        return OAL_SUCC;
    }
    ...
    OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_entry, pst_dlist_tmp,
        &(pst_mac_vap->pst_vap_proxyarp->ast_ipv6_head[ul_user_hash_value])) {
        pst_hash = (hmac_proxy_ipv6_hash_stru *)pst_entry;
...
oal_err_code_enum_uint32 hmac_proxy_add_ipv6(mac_vap_stru *pst_mac_vap, oal_in6_addr *pst_ipv6, oal_uint8 *puc_mac)
{
    ...
    en_exist = hmac_proxy_get_mac_by_ipv6(pst_mac_vap, pst_ipv6, auc_mac, sizeof(auc_mac));
    if (en_exist == OAL_SUCC) {
        if (!oal_memcmp(auc_mac, puc_mac, OAL_MAC_ADDR_LEN)) {
            return OAL_ERR_CODE_PROXY_ND_LEARN_USR_SUCC_COMP;
        }
    ...
    if (pst_mac_vap->pst_vap_proxyarp->uc_ipv6_num >= MAC_VAP_USER_HASH_MAX_VALUE) {
        return OAL_ERR_CODE_PROXY_ND_LEARN_USR_FAIL_INCOMP;
    }
    ...
    if (memcpy_s(pst_hash->auc_mac, WLAN_MAC_ADDR_LEN, puc_mac, OAL_MAC_ADDR_LEN) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_PROXYARP, "{hmac_proxy_add_ipv6::memcpy_s fail!.}");
        OAL_MEM_FREE(pst_hash, OAL_TRUE);
        return OAL_FAIL;
    }
...
oal_err_code_enum_uint32 hmac_proxy_remove_mac(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_mac)
{
    ...
    if (OAL_LIKELY(pst_mac_vap->pst_vap_proxyarp == OAL_PTR_NULL)) {
    ...
    if (pst_mac_vap->pst_vap_proxyarp->uc_ipv6_num > 0) {
        for (ul_loop = 0; ul_loop < MAC_VAP_USER_HASH_MAX_VALUE; ul_loop++) {
            OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_entry, pst_dlist_tmp,
                &(pst_mac_vap->pst_vap_proxyarp->ast_ipv6_head[ul_loop])) {
    ...
    if (pst_mac_vap->pst_vap_proxyarp->uc_ipv4_num > 0) {
        for (ul_loop = 0; ul_loop < MAC_VAP_USER_HASH_MAX_VALUE; ul_loop++) {
            OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_entry, pst_dlist_tmp,
                &(pst_mac_vap->pst_vap_proxyarp->ast_ipv4_head[ul_loop])) {
...
oal_void hmac_proxy_debug_record(mac_vap_stru *pst_mac_vap, mac_ether_header_stru *pst_eth_hdr, oal_uint16 us_type,
    oal_err_code_enum_uint32 en_rst)
{
    mac_vap_proxyarp_stru *pst_vap_proxyarp = OAL_PTR_NULL;
    oal_uint32 ul_idx;
    if (en_rst == OAL_ERR_CODE_PROXY_OTHER_INCOMP) {
        return;
    }
...
oal_err_code_enum_uint32 hmac_proxy_arp_rcv_req(mac_vap_stru *pst_mac_vap, oal_net_device_stru *pst_dev,
    oal_eth_arphdr_stru *pst_arp_hdr)
{
    ...
    l_ret += memcpy_s(auc_src_mac, ETHER_ADDR_LEN, pst_arp_hdr->auc_ar_sha, ETHER_ADDR_LEN);
    l_ret += memcpy_s(&ul_src_ipv4, ETH_SENDER_IP_ADDR_LEN, pst_arp_hdr->auc_ar_sip, ETH_SENDER_IP_ADDR_LEN);
    l_ret += memcpy_s(&ul_tgt_ipv4, ETH_TARGET_IP_ADDR_LEN, pst_arp_hdr->auc_ar_tip, ETH_TARGET_IP_ADDR_LEN);
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_PROXYARP, "{hmac_proxy_arp_rcv_req::memcpy_s failed!}");
        return OAL_FAIL;
    }

    /* GARP,进行地址学习 */
    if (ul_src_ipv4 == ul_tgt_ipv4) {
        if (mac_vap_find_user_by_macaddr(pst_mac_vap, auc_src_mac, &us_usr_idx) != OAL_SUCC) {
    ...
    pst_netbuff = oal_arp_create(OAL_ARPOP_REPLY, ETHER_TYPE_ARP, ul_src_ipv4, pst_dev, ul_tgt_ipv4, auc_src_mac,
                                 auc_tgt_mac, auc_src_mac);
    if (pst_netbuff == NULL) {
    ...
    if (mac_vap_find_user_by_macaddr(pst_mac_vap, auc_src_mac, &us_usr_idx) == OAL_SUCC) {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_PROXYARP, "{hmac_proxy_arp_rcv_req 2wlan Err[%u]!!}",
                ul_ret);
...
oal_err_code_enum_uint32 hmac_proxy_arp_rcv_reply(mac_vap_stru *pst_mac_vap, oal_eth_arphdr_stru *pst_arp_hdr,
    oal_uint8 uc_is_mcst)
{
    ...
    if (mac_vap_find_user_by_macaddr(pst_mac_vap, pst_arp_hdr->auc_ar_sha, &us_usr_idx) == OAL_SUCC) {
...
oal_err_code_enum_uint32 hmac_proxy_arp(mac_vap_stru *pst_mac_vap, oal_net_device_stru *pst_dev,
    oal_netbuf_stru *pst_buf)
{
    mac_ether_header_stru *pst_ether_hdr; /* 以太网头 */
    oal_uint32 ul_arp_hdr_len;
    oal_eth_arphdr_stru *pst_arp_hdr;
    oal_uint8 uc_is_mcst;
    mac_ether_header_stru st_eth_hdr;
...
oal_err_code_enum_uint32 hmac_proxy_nd_learn_from_ns(mac_vap_stru *pst_mac_vap, oal_in6_addr *pst_ipv6_src,
    oal_uint8 *puc_mac_src)
{
    oal_uint16 us_usr_idx = 0;
    oal_uint8 auc_mac[ETHER_ADDR_LEN];
    oal_err_code_enum_uint32 en_err;

    if (mac_vap_find_user_by_macaddr(pst_mac_vap, puc_mac_src, &us_usr_idx) != OAL_SUCC) {
        return OAL_ERR_CODE_PROXY_ND_LEARN_USR_NOTEXIST_COMP;
    }

    /* 检查ipv6的地址已经存在,并且mac地址不相等，则将此帧发送出去，进行DAD检测 */
    en_err = hmac_proxy_get_mac_by_ipv6(pst_mac_vap, pst_ipv6_src, auc_mac, sizeof(auc_mac));
    if ((en_err == OAL_SUCC) && (oal_memcmp(puc_mac_src, auc_mac, OAL_MAC_ADDR_LEN))) {
...
oal_netbuf_stru *hmac_proxy_create_na(oal_net_device_stru *pst_net_dev, oal_uint8 *puc_src_mac, oal_uint8 *puc_dst_mac,
    oal_uint8 *puc_src_ipv6, oal_uint8 *puc_dst_ipv6)
{
    oal_netbuf_stru *pst_netbuff = OAL_PTR_NULL;
    oal_uint32 ul_size;
    ...
    *(oal_uint32 *)pst_ipv6 = OAL_HOST2NET_LONG(0x60000000);
    ...
    /*lint -e415*/ /*lint -e416*/
    pst_nd->opt[1] = 1;
    if (memcpy_s(&pst_nd->opt[2], OAL_MAC_ADDR_LEN, puc_src_mac, OAL_MAC_ADDR_LEN) != EOK) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_PROXYARP, "{hmac_proxy_create_na::memcpy_s failed!}");
        oal_netbuf_free(pst_netbuff);
        return OAL_PTR_NULL;
    }
    /*lint +e415*/ /*lint +e416*/
    pst_nd->icmph.icmp6_cksum = oal_csum_ipv6_magic(&pst_ipv6->saddr, &pst_ipv6->daddr, (OAL_SIZEOF(*pst_nd) - 4) + 8,
        OAL_IPPROTO_ICMPV6, oal_csum_partial(&pst_nd->icmph, (OAL_SIZEOF(*pst_nd) - 4) + 8, 0));
...
oal_uint8 *hmac_proxy_get_mac_from_opt(oal_uint8 *puc_opt, oal_int32 l_optlen, oal_bool_enum_uint8 en_src)
{
    ...
    while (l_optlen_tmp > 0) {
        ...
        switch (pst_ndopt->nd_opt_type) {
            ...
            default: {
                OAM_WARNING_LOG1(0, OAM_SF_PROXYARP, "{hmac_proxy_get_mac_from_opt::type [%d].}",
                    pst_ndopt->nd_opt_type);
                break;
            }
        }
...
oal_err_code_enum_uint32 hmac_proxy_nd_rcv_ns(mac_vap_stru *pst_mac_vap, oal_net_device_stru *pst_dev,
    mac_ether_header_stru *pst_ether_hdr, oal_int32 l_opt_len)
{
    ...
    if (pst_src_mac == OAL_PTR_NULL) {
        return OAL_ERR_CODE_PROXY_ND_NS_OPT_INVLD_COMP;
    }

    if (hmac_proxy_get_mac_by_ipv6(pst_mac_vap, &pst_msg->target, ast_dst_mac, sizeof(ast_dst_mac) != OAL_SUCC)) {
        return OAL_ERR_CODE_PROXY_ND_NS_FINDUSR_ERR_COMP;
    }

    /* 构造NA */
    pst_netbuff = hmac_proxy_create_na(pst_dev, ast_dst_mac, pst_src_mac, (oal_uint8 *)&pst_msg->target,
        (oal_uint8 *)&pst_ipv6_hdr->saddr);
    if (pst_netbuff == OAL_PTR_NULL) {
    ...
    if (mac_vap_find_user_by_macaddr(pst_mac_vap, pst_src_mac, &us_idx) == OAL_SUCC) {
        ul_ret = hmac_tx_lan_to_wlan(pst_mac_vap, pst_netbuff);
        if (ul_ret != OAL_SUCC) {
        ...
        if (l_netifrx_rst != OAL_SUCC) {
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_PROXYARP, "{hmac_proxy_nd_rcv_ns 2br Err[%d]!!}",
                l_netifrx_rst);
        }
...
oal_err_code_enum_uint32 hmac_proxy_nd_rcv_na(mac_vap_stru *pst_mac_vap, mac_ether_header_stru *pst_ether_hdr,
    oal_int32 l_opt_len)
{
    oal_ipv6hdr_stru         *pst_ipv6_hdr;
    ...
    /* S在响应组播ns时不应该置位 */
    if ((pst_nd->icmph.icmp6_dataun.u_nd_advt.solicited) && hmac_proxy_ipv6_addr_is_mcast(&pst_nd->target)) {
        return OAL_ERR_CODE_PROXY_ND_NA_INVLD_COMP;
    }

    en_solicited =
        (pst_nd->icmph.icmp6_dataun.u_nd_advt.solicited) && !(hmac_proxy_ipv6_addr_is_mcast(&pst_nd->target));

    puc_src_mac = hmac_proxy_get_mac_from_opt(pst_nd->opt, l_opt_len, OAL_FALSE);
    if (puc_src_mac == OAL_PTR_NULL) {
        if (en_solicited) {
    ...
    /* 从NA中学习一下地址 */
    if (mac_vap_find_user_by_macaddr(pst_mac_vap, puc_src_mac, &us_usr_idx) == OAL_SUCC) {
        en_err = hmac_proxy_add_ipv6(pst_mac_vap, &pst_nd->target, puc_src_mac);
        if (en_err != OAL_ERR_CODE_PROXY_ND_LEARN_USR_SUCC_COMP) {
            return en_err;
        }
    }
...
oal_err_code_enum_uint32 hmac_proxy_nd(mac_vap_stru *pst_mac_vap, oal_net_device_stru *pst_dev,
    oal_netbuf_stru *pst_buf)
{
    mac_ether_header_stru *pst_ether_hdr; /* 以太网头 */
    oal_ipv6hdr_stru      *pst_ipv6_hdr;
    ...
    if (pst_ipv6_hdr->nexthdr != OAL_IPPROTO_ICMPV6) {
    ...
    if ((pst_icmp6->icmp6_type == OAL_NDISC_NEIGHBOUR_SOLICITATION)) {
        return hmac_proxy_nd_rcv_ns(pst_mac_vap, pst_dev, pst_ether_hdr, (oal_int32)ul_opt_len);
    } else if (pst_icmp6->icmp6_type == OAL_NDISC_NEIGHBOUR_ADVERTISEMENT) {
...
oal_bool_enum_uint8 hmac_proxy_arp_proc(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_buf)
{
    ...
    OAM_INFO_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_PROXYARP, "hmac_proxy_arp_proc! s:%x->d:%x type %x, result %u!",
        st_eth_hdr.auc_ether_shost[ETHER_ADDR_LEN - 1], st_eth_hdr.auc_ether_dhost[ETHER_ADDR_LEN - 1], us_ether_type,
        en_rst);
#endif
...
oal_void hmac_proxy_arp_init(mac_vap_stru *pst_mac_vap)
{
    ...
    if ((pst_mac_vap->en_vap_mode != WLAN_VAP_MODE_BSS_AP) || (pst_mac_vap->pst_vap_proxyarp != OAL_PTR_NULL)) {
        return;
    }
...
oal_uint32 hmac_proxy_display_info(mac_vap_stru *pst_mac_vap)
{
    ...
    for (ul_loop = 0; ul_loop < MAC_VAP_USER_HASH_MAX_VALUE; ul_loop++) {
        ...
        OAL_IO_PRINT("%u\t%08x%04x\t%08x%04x\t%04x\t\t%u\t0x%x\n", pst_dbg->us_proxy_num, \
            ...
            pst_dbg->st_eth_hdr.us_ether_type, pst_dbg->en_rst, pst_dbg->us_type);
    }
    ...
    for (ul_loop = 0; ul_loop < MAC_VAP_USER_HASH_MAX_VALUE; ul_loop++) {
        OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_entry, pst_dlist_tmp,
            &(pst_mac_vap->pst_vap_proxyarp->ast_ipv4_head[ul_loop])) {
            pst_hash_ipv4 = (hmac_proxy_ipv4_hash_stru *)pst_entry;
            OAL_IO_PRINT("%u\t%08x\t%08x%04x\n", ul_idx++, OAL_HOST2NET_LONG(pst_hash_ipv4->ul_ipv4), \
