...
oal_uint32 hmac_mgmt_encap_asoc_req_sta(hmac_vap_stru *pst_hmac_sta, oal_uint8 *puc_req_frame)
{
    ...
    if ((pst_hmac_sta == OAL_PTR_NULL) || (puc_req_frame == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_ASSOC, "{hmac_mgmt_encap_asoc_req_sta::null param}");
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
            "{hmac_mgmt_encap_asoc_req_sta::pst_mac_device null.}");
        return us_asoc_rsq_len;
    }
    ...
    if (pst_hmac_sta->bit_reassoc_flag) {
        mac_hdr_set_frame_control(puc_req_frame,
...
#ifdef _PRE_WLAN_FEATURE_TXBF
    if ((pst_mac_vap->bit_ap_11ntxbf == OAL_TRUE) && (pst_mac_vap->st_cap_flag.bit_11ntxbf == OAL_TRUE) &&
        (uc_ie_len != 0)) {
...
#endif // PRE_WLAN_FEATURE_HISTREAM
...
oal_uint16  hmac_mgmt_encap_auth_req(hmac_vap_stru *pst_hmac_sta, oal_uint8 *puc_mgmt_frame)
{
    ...
    /*                                                                       */
    /*************************************************************************/
    if (pst_hmac_sta->st_vap_base_info.pst_mib_info->st_wlan_mib_privacy.en_dot11PrivacyInvoked == OAL_FALSE) {
        /* Open System */
    ...
    /* If there is a mapping in dot11WEPKeyMappings matching the address of
       ...
       No error condition detected */
        ...
        if (memcpy_s(&puc_mgmt_frame[us_index + 8], uc_ch_text_len, puc_ch_text, uc_ch_text_len) != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_mgmt_encap_auth_req_seq3::memcpy_s failed!");
        }
...
oal_uint16  hmac_encap_2040_coext_mgmt(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_buffer, oal_uint8 uc_coext_info,
    oal_uint32 ul_chan_report)
{
    ...
    oal_set_mac_addr(puc_buffer + WLAN_HDR_ADDR2_OFFSET,
...
oal_uint32 hmac_mgmt_encap_smps_sta(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_buffer)
{
    ...
    if ((pst_mac_device->en_smps == OAL_FALSE) || (uc_smps_mode == WLAN_MIB_MIMO_POWER_SAVE_MIMO)) {
