...
oal_uint16 hmac_encap_sa_query_req(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_data, const oal_uint8 *puc_da,
...
oal_uint16  hmac_mgmt_encap_deauth(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_data, const oal_uint8 *puc_da,
    oal_uint16 us_err_code)
{
    ...
    if (us_err_code & MAC_SEND_TWO_DEAUTH_FLAG) {
        ...
        if (OAL_UNLIKELY(pst_mac_device == OAL_PTR_NULL)) {
            us_deauth_len = 0;
            OAM_ERROR_LOG1(0, OAM_SF_ANY, "{hmac_mgmt_encap_deauth::pst_mac_device[%d] null!}",
        ...
        if (ul_ret == OAL_SUCC) {
            ...
            oal_set_mac_addr(((mac_ieee80211_frame_stru *)puc_data)->auc_address2,
                pst_up_vap2->pst_mib_info->st_wlan_mib_sta_config.auc_dot11StationID);
            oal_set_mac_addr(((mac_ieee80211_frame_stru *)puc_data)->auc_address3, pst_up_vap2->auc_bssid);
        }

        OAM_WARNING_LOG1(0, OAM_SF_AUTH, "hmac_mgmt_encap_deauth:send second deauth frame.error code:%d", us_err_code);
    } else
#endif
    {
...
oal_uint16 hmac_mgmt_encap_disassoc(mac_vap_stru  *pst_mac_vap,
                                    oal_uint8     *puc_data,
                                    const uint8_t *puc_da,
                                    oal_uint16    us_err_code)
{
...
oal_uint16 hmac_encap_notify_chan_width(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_data, const oal_uint8 *puc_da)
{
    oal_uint16 us_len;
