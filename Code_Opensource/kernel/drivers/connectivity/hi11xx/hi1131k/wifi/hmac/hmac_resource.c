...
oal_uint32 hmac_res_free_mac_dev(oal_uint32 ul_dev_idx)
{
    ...
    /* 入队索引值需要加1操作 */
    oal_queue_enqueue(&(g_st_hmac_res.st_hmac_dev_res.st_queue), (oal_void *)((uintptr_t)ul_dev_idx + 1));
...
oal_uint32 hmac_res_init(oal_void)
{
    ...
    ***************************************************************************/
    ...
    for (ul_loop = 0; ul_loop < MAC_RES_MAX_DEV_NUM; ul_loop++) {
        /* 初始值保存的是对应数组下标值加1 */
        oal_queue_enqueue(&(g_st_hmac_res.st_hmac_dev_res.st_queue), (oal_void *)((uintptr_t)ul_loop + 1));
...
oal_uint32 hmac_res_exit(mac_board_stru *pst_hmac_board)
{
    ...
    for (uc_chip = 0; uc_chip < ul_chip_max_num; uc_chip++) {
        ...
        for (uc_device = 0; (uc_device < uc_device_max) && (uc_device < WLAN_DEVICE_MAX_NUM_PER_CHIP); uc_device++) {
            /* 释放hmac res资源 */
