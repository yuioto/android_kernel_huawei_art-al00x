...
oal_void wal_long_mac_test(oal_void)
{
    ...
    for (ul_loop = 0; ul_loop < 1; ul_loop++) {
        WITP_REG_WRITE32(pst_device, WITP_PA_AP0_MACADDR_L_REG, 0x12345678,
            IDX_WITP_PA_AP0_MACADDR_L_REG, HAL_SOFT_REG_TBL_MAC, OAL_FALSE);
    }
...
oal_uint32 wal_test_5115_reg_read(oal_int8 *pc_param)
{
    ...
    /* 参数检查 */
    /*lint -e960*/
    if ((oal_strcmp(pc_token, "sys") != 0) && (oal_strcmp(pc_token, "pci0") != 0)) {
        return OAL_FAIL;
    }
    /*lint +e960*/
...
int32_t test_hipriv_proc_write(oal_file_stru *pst_file, const oal_int8 *pc_buffer, uint32_t ul_len, oal_void *p_data)
{
    ...
    if (ul_len > TEST_HIPRIV_CMD_MAX_LEN) {
        OAL_IO_PRINT("test_hipriv_proc_write:ul_len > WAL_HIPRIV_CMD_MAX_LEN, ul_len:%d.\n", ul_len);
        return -OAL_EINVAL;
    }
