...
#if defined(_PRE_WLAN_FEATURE_IP_FILTER) && !defined(_PRE_WLAN_ASIC_LITEOS_FEATURE)
extern mac_rx_ip_filter_stru g_rx_ip_filter;
extern uint8_t g_ip_filter_btable[MAC_MAX_IP_FILTER_BTABLE_SIZE];
...
#endif
    DMAC_WLAN_CTX_EVENT_SUB_TYPE_CALI_HMAC2DMAC = 25,
    DMAC_WLAN_CTX_EVENT_SUB_TYPE_DSCR_OPT,
#ifdef _PRE_WLAN_FEATURE_TCP_UDP_KEEPALIVE
    DMAC_TX_HOST_DATA_FRAME_FILTER = 27,
...
typedef struct dmac_to_hmac_btcoex_auto_freq {
    oal_bool_enum_uint8 btcoex_auto_freq_enable;
    uint8_t             resv[3];
} dmac_to_hmac_btcoex_auto_freq_stru;
#endif
#endif

typedef struct
{
    uint32_t arpoffload_drop_frame_cnt;
    uint32_t arpoffload_send_rsp_cnt;
    uint32_t apf_flt_drop_cnt;
    uint32_t icmp_flt_drop_cnt;
} dmac_to_hmac_psm_txrx_pkts_stru;

typedef struct
{
    uint32_t pm_msg_psm_beacon_cnt;
    uint32_t pm_msg_tbtt_cnt;
    uint32_t psm_err_tim_set_cnt;
    uint32_t pm_msg_tim_awake;
} dmac_to_hmac_psm_beacon_pkts_stru;
...
}dmac_query_station_info_response_event_ex;
...
#endif
#if (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1131C_HOST)
extern uint32_t mac_vap_set_cb_tx_user_idx(mac_vap_stru *mac_vap, mac_tx_ctl_stru *tx_ctl, const unsigned char *data);
#endif
