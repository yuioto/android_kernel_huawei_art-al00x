...
/* 指定filter配置命令格式 */
typedef struct {
    mac_assigned_filter_id_enum_uint8 filter_id;
    oal_bool_enum_uint8 enable;  /* 下发过滤功能使能开关命令 */
    uint16_t rx_icmp_pkgs_num; /* 扩展用于统计过滤的icmp报文数量 */
...
typedef struct
{
    uint16_t port;    /* 目的端口号，以主机字节序格式存储 */
    uint8_t  protocol;
    uint8_t  resv;
...
{
    uint8_t                 item_count;
    oal_bool_enum_uint8     enable;    /* 下发功能使能标志 */
    uint8_t                 cmd;
    uint8_t                 resv;
    mac_ip_filter_item_stru filter_items_items[1];
...
typedef struct
{
    ...
    uint32_t            install_timestamp;
    uint8_t             program_buff[APF_PROGRAM_MAX_LEN];
} mac_apf_stru;
...
typedef struct {
    uint8_t chip_oui : 4;                    /* 扫描到chip oui识别有兼容性问题ap chip */
    uint8_t resv : 3;
    uint8_t btcoex_blacklist_chip_oui : 1;   /* 扫描到的AP 识别chip oui加入共存黑名单 */
} mac_scan_chip_oui_stru;
...
typedef struct
{
    ...
    mac_scan_chip_oui_stru              chip_oui_info;                      /* 扫描到chip oui识别有兼容性问题ap chip */
    oal_int8                            c_rssi;                             /* bss的信号强度 */
...
typedef struct
{
    uint8_t                 state;           //功能状态：过滤、非过滤等
    uint8_t                 table_items_num; //黑名单中目前存储的items个数
    uint8_t                 table_size;      //黑名单大小，表示最多存储的items个数
    uint8_t                 resv;
    mac_ip_filter_item_stru *filter_btable;  //黑名单指针
} mac_rx_ip_filter_stru;
#endif //_PRE_WLAN_FEATURE_IP_FILTER

typedef struct {
    uint16_t cmd_auth_rsp_timeout;
    uint8_t  forbit_open_auth : 1;
    uint8_t  rsv              : 7;
...
extern mac_device_custom_cfg_stru g_mac_device_custom_cfg;
#endif
/*****************************************************************************
  10.4 待移除
*****************************************************************************/
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8  mac_is_hide_ssid(const oal_uint8 *puc_ssid_ie, oal_uint8 uc_ssid_len)
{
...
#ifdef _PRE_WLAN_FEATURE_1131K_BTCOEX
extern mac_fcs_err_enum_uint8 mac_fcs_start_same_channel(mac_fcs_mgr_stru *fcs_mgr,
                                                         mac_fcs_cfg_stru *fcs_cfg,
                                                         hal_one_packet_status_stru *status,
                                                         oal_uint8 fake_tx_q_id);

extern mac_fcs_err_enum_uint8 mac_fcs_start_enhanced_same_channel(mac_fcs_mgr_stru *fcs_mgr,
                                                                  mac_fcs_cfg_stru *fcs_cfg);
