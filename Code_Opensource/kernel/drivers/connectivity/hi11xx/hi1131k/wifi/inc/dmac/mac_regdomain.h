...
extern uint32_t mac_get_valid_channel_idx(uint8_t band, uint8_t vap_id, uint8_t channel_num, uint8_t *channel_idx);
#endif
...
#if (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1131C_HOST)
extern oal_int32 mac_regdomain_enable_channel_14(oal_void);
extern oal_int32 mac_regdomain_disable_channel_14(oal_void);
#endif
...
{
#ifdef _PRE_WLAN_FEATURE_5G
    return (wlan_channel_band_enum_uint8)((uc_channel_num > MAX_CHANNEL_NUM_FREQ_2G) ? WLAN_BAND_5G : WLAN_BAND_2G);
#else
    return (wlan_channel_band_enum_uint8)((uc_channel_num <= MAX_CHANNEL_NUM_FREQ_2G) ? WLAN_BAND_2G : WLAN_BAND_BUTT);
#endif /* _PRE_WLAN_FEATURE_5G */
}
