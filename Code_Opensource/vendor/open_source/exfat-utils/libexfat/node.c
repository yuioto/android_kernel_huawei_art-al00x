...
static struct exfat_node* allocate_node(void)
{
	...
	memset_s(node, sizeof(struct exfat_node), 0, sizeof(struct exfat_node));
	return node;
}
...
int exfat_set_label(struct exfat* ef, const char* label)
{
	...
	memset_s(label_utf16, sizeof(label_utf16), 0, sizeof(label_utf16));
	...
	memcpy_s(entry.name, sizeof(entry.name), label_utf16, sizeof(entry.name));
	if (entry.length == 0)
	...
	strcpy_s(ef->label, sizeof(ef->label), label);
	return 0;
}
