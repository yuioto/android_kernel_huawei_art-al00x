...
int exfat_mount(struct exfat* ef, const char* spec, const char* options)
{
	...
	exfat_tzset();
retry:
	memset_s(ef, sizeof(struct exfat), 0, sizeof(struct exfat));
	...
	memset_s(ef->sb, sizeof(struct exfat_super_block), 0, sizeof(struct exfat_super_block));
	...
	if (memcmp(ef->sb->oem_name, "EXFAT   ", 8) && memcmp(ef->sb->oem_name, "AXFAT   ", 8))
	{
	...
	memset_s(ef->zero_cluster, CLUSTER_SIZE(*ef->sb), 0, CLUSTER_SIZE(*ef->sb));
	...
	memset_s(ef->root, sizeof(struct exfat_node), 0, sizeof(struct exfat_node));
