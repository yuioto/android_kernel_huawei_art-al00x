...
#define EXFAT_FSCK_TIME_THRESHOLD   (5)       //5s
...
static int nodeck(struct exfat* ef, struct exfat_node* node)
{
	...
	while (clusters--)
	{
		if (CLUSTER_INVALID(*ef->sb, c))
		{
			...
			rc = 1;
			exfat_error("exfat.fsck found error, exit");
			exit(1);
		}
...
static void dirck(struct exfat* ef, const char* path)
{
	...
	strcpy_s(entry_path, path_length + 1 + EXFAT_UTF8_NAME_BUFFER_MAX, path);
	strcat_s(entry_path, path_length + 1 + EXFAT_UTF8_NAME_BUFFER_MAX, "/");
...
{
	exfat_error("exfat.fsck running timeout, signo(%d)", signo);
	exit(1);
}
...
void init_time()
{
	...
	value.it_value.tv_sec = EXFAT_FSCK_TIME_THRESHOLD;
