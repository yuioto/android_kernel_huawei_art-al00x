...
{
	memset_s(label_entry, sizeof(struct exfat_entry_label),
	...
	memcpy_s(label_entry->name, EXFAT_ENAME_MAX * sizeof(le16_t),
		 get_volume_label(), EXFAT_ENAME_MAX * sizeof(le16_t));
...
{
	memset_s(bitmap_entry, sizeof(struct exfat_entry_bitmap),
...
static void init_upcase_entry(struct exfat_entry_upcase* upcase_entry)
{
	...
	memset_s(upcase_entry, sizeof(struct exfat_entry_upcase),
