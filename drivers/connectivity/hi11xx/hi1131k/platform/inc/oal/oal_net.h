...
#define OAL_IPV6_IS_UNSPECIFIED_ADDR(_a)   \
    ((_a)[0]  == 0x00 && (_a)[1]  == 0x00 &&          \
     ...
     (_a)[6]  == 0x00 && (_a)[7]  == 0x00 &&          \
     (_a)[8]  == 0x00 && (_a)[9]  == 0x00 &&          \
     (_a)[10] == 0x00 && (_a)[11] == 0x00 &&          \
     (_a)[12] == 0x00 && (_a)[13] == 0x00 &&          \
     (_a)[14] == 0x00 && (_a)[15] == 0x00)
...
typedef struct oal_tcp_header oal_tcp_header_stru;
struct oal_udp_header {
    oal_uint16 source;
    oal_uint16 dest;
    oal_uint16 len;
    oal_uint16 check;
} __OAL_DECLARE_PACKED;
typedef struct oal_udp_header oal_udp_header_stru;

/* WIN32和linux共用结构体  */
struct oal_eth_icmp6_lladdr {
    oal_uint8 uc_type;
    oal_uint8 uc_len;
    oal_uint8 uc_addr[6];  /* hardware address */
} __OAL_DECLARE_PACKED;
typedef struct oal_eth_icmp6_lladdr oal_eth_icmp6_lladdr_stru;
struct oal_dhcp_packet {
...
typedef struct oal_dhcp_packet oal_dhcp_packet_stru;
/* 不分平台通用结构体 */
typedef struct {
...
extern oal_int32 dev_netlink_send (const oal_uint8 *data, oal_long data_len);
extern oal_int32 init_dev_excp_handler(oal_void);
extern oal_void deinit_dev_excp_handler(oal_void);
extern oal_long genl_msg_send_to_user(oal_void *data, oal_long i_len);
