...
OAL_STATIC OAL_INLINE oal_void  oal_irq_save(oal_ulong *pui_flags, oal_uint32 ul_type)
{
...
OAL_STATIC OAL_INLINE oal_void  oal_irq_restore(const oal_ulong *pui_flags, oal_uint32 ul_type)
{
#ifdef _PRE_DEBUG_MODE
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC != _PRE_MULTI_CORE_MODE)
    ...
    if (g_aul_irq_save_time[ul_core_id][254] != 0) {
        ...
        g_aul_irq_save_time[ul_core_id][254] = 0; // 254下标存储标志位
        g_aul_irq_save_time[ul_core_id][253] = -1; // 253下标存储标志位
    } else {
        /* 重复restore */
...
OAL_STATIC  OAL_INLINE oal_hi_timerx_reg_stru  *hi_timerx_request(oal_hi_timerx_index_enum_uint8  en_timer_idx)
{
    ...
    if (en_timer_idx < HI5115_TIMER_INDEX_BUTT) {
        pst_timer = (oal_hi_timerx_reg_stru *)oal_ioremap(
            OAL_HI_TIMER_REG_BASE + sizeof(oal_hi_timerx_reg_stru) * en_timer_idx, sizeof(oal_hi_timerx_reg_stru));
    }
...
{
    if ((pst_timer != NULL) && (pst_config != NULL)) {
        pst_timer->ul_timerx_control = pst_config->u_timerx_config.ul_value;
    }
}

extern OAL_INLINE oal_int32  oal_irq_setup(oal_irq_dev_stru *st_osdev);

extern OAL_INLINE oal_void  oal_irq_free(oal_irq_dev_stru *st_osdev);
