...
#define FRW_TIMER_CREATE_TIMER(_pst_timeout, _p_timeout_func, _ul_timeout, _p_timeout_arg, _en_is_periodic, _en_module_id, _ul_core_id) \
    frw_timer_create_timer(THIS_FILE_ID, __LINE__, _pst_timeout, _p_timeout_func, _ul_timeout, _p_timeout_arg, \
    _en_is_periodic, _en_module_id, _ul_core_id)
...
typedef struct {
    ...
    oal_uint8             auc_event_data[4];      /* payload 4 */
} frw_event_stru;
...
{
    oal_uint32       ul_ret;
    frw_event_stru  *pst_frw_event = OAL_PTR_NULL;
...
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
oal_void frw_event_sub_rx_adapt_table_init(frw_event_sub_table_item_stru *pst_sub_table,
    oal_uint32 ul_table_nums, frw_event_mem_stru *(*p_rx_adapt_func)(frw_event_mem_stru *));
#endif
#endif
