...
oal_int32 get_board_gpio(const char *gpio_node, const char *gpio_prop, oal_int32 *physical_gpio)
{
    ...
    if (ret != BOARD_SUCC) {
        PS_PRINT_ERR("get node[%s] gpio_prop[%s] failed\n", gpio_node, gpio_prop);
        return BOARD_FAIL;
    }
...
int32_t get_ini_file_name_from_dts(int8_t *dts_prop, int8_t *prop_value, uint32_t size)
{
    ...
    ret = get_board_custmize(DTS_NODE_HISI_CUST, dts_prop, &device_version);
    if (ret != BOARD_SUCC) {
        PS_PRINT_ERR("get_board_custmize failed. dts_prop=%s.", dts_prop);
        return BOARD_FAIL;
    }

    len = strlen(device_version);
    if (memcpy_s(prop_value, (size_t)size, device_version, len) != EOK) {
        PS_PRINT_ERR("memcpy_s error, destlen=%d, srclen=%d.", (int)size, (int)len);
        return BOARD_FAIL;
    }
    PS_PRINT_DBG("get_ini_file_name_from_dts: dts prop %s value is:%s.", dts_prop, prop_value);
#endif
    return BOARD_SUCC;
}
...
oal_int32 board_get_power_pinctrl(struct platform_device *pdev)
{
    ...
    if (get_board_dts_node(&np, DTS_NODE_HI110X)) {
        PS_PRINT_ERR("DTS read node %s fail!!!\n", DTS_NODE_HI110X);
        return BOARD_FAIL;
    }
    ...
    if (IS_ERR_OR_NULL(pinctrl)) {
        chr_exception(chr_plat_drv(CHR_PLT_DRV_EVENT_INIT, CHR_PLAT_DRV_ERROR_BOARD_GPIO_INIT));
        return BOARD_FAIL;
    }
    ...
    if (IS_ERR_OR_NULL(pinctrl_idle)) {
        PS_PRINT_ERR("pinctrl_lookup_state idel failed, and the value of pinctrl_idle is NULL");
        devm_pinctrl_put(g_board_info.pctrl);
        return BOARD_FAIL;
    }
...
oal_int32 board_gpio_init(void)
{
    ...
    if (get_board_gpio(DTS_NODE_HI110X, DTS_PROP_GPIO_POWEN_ON, &g_board_info.power_on_enable) != BOARD_SUCC) {
        chr_exception(chr_plat_drv(CHR_PLT_DRV_EVENT_INIT, CHR_PLAT_DRV_ERROR_BOARD_DRV_PROB));
    ...
    ret = get_board_gpio(DTS_NODE_HI110X, DTS_DROP_GPIO_WLAN_POWER_ON, &g_board_info.wlan_en);
    if (ret != BOARD_SUCC) {
        goto err_get_power_on_gpio;
    }
    ...
    ret = get_board_gpio(DTS_NODE_HI110X_WIFI, DTS_PROP_GPIO_WLAN_WAKEUP_HOST, &g_board_info.wlan_wakeup_host);
    if (ret != BOARD_SUCC) {
    ...
    ret = get_board_gpio(DTS_NODE_HI110X_WIFI, DTS_DROP_GPIO_HOST_TO_DEVICE, &g_board_info.wlan_host_to_device);
    if (ret != BOARD_SUCC) {
        goto err_get_wlan_wkup_host_gpio;
    }
    ...
    err_get_power_on_gpio:
        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT,
                             CHR_LAYER_DRV, CHR_PLT_DRV_EVENT_INIT, CHR_PLAT_DRV_ERROR_BOARD_GPIO_INIT);
    return BOARD_FAIL;
}
...
void board_power_on(void)
{
    prepare_to_power_on();
    SET_CLOCK_40M();
    gpio_direction_output(g_board_info.wlan_en, GPIO_HIGHLEVEL); // wlan_rst pull high
    mdelay(WL_RST_DELAY);
...
{
    gpio_direction_output(g_board_info.power_on_enable, GPIO_LOWLEVEL);
    gpio_direction_output(g_board_info.wlan_en, GPIO_LOWLEVEL); // wlan_rst pull down
    oal_sdio_mmc_power_off(); // sdio data clk cmd pin pull down
}
