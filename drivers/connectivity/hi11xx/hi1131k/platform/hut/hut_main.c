...
typedef struct {
    ...
    oal_uint8                   auc_resv[3];  // 预留3字节，同时用于字节对齐
...
OAL_STATIC oal_int32  hut_read_reg(oal_uint8 *puc_data)
{
    ...
    --------
    ************/
    /* data3 高32 - 24位， data2 高24 - 16位 data1 16 - 8 */
...
{
    hal_to_dmac_device_stru   *pst_hal_device = OAL_PTR_NULL;
    hut_cmd_fmt_stru          *pst_cmd_fmt = OAL_PTR_NULL;
...
OAL_STATIC oal_int32  hut_write_mem(oal_uint8 *puc_data)
{
    ...
    **********************/
    ...
    l_ret = memcpy_s((oal_uint8 *)ul_virt_addr, pst_cmd_fmt->ul_val + 1,
...
{
    hal_to_dmac_device_stru   *pst_hal_device = OAL_PTR_NULL;
    hut_rx_node               *pst_rx_node = OAL_PTR_NULL;
    oal_ulong                   ul_flag;
    oal_uint32                 ul_base_addr;
    oal_uint32                *pul_tx_dscr = OAL_PTR_NULL;
    oal_uint32                *pul_rx_dscr = OAL_PTR_NULL;
    ...
    if (pst_rx_node->ul_data1 & (1 << 0)) { /* 接收完成中断 */
        hal_reg_info(pst_hal_device, 0x20002420, &pst_rx_node->ul_data3); // 配置寄存地址 0x20002420
        hal_reg_info(pst_hal_device, 0x20002418, &pst_rx_node->ul_data4); // 配置寄存地址 0x20002418
        ...
        pst_rx_node->ul_data5 = pul_rx_dscr[4]; // data1为1时，从desr 4中获取数据
    } else if (pst_rx_node->ul_data1 & (1 << 1)) { /* 发送完成中断 */
        hal_reg_info(pst_hal_device, 0x20002424, &pst_rx_node->ul_data3); // 配置寄存地址 0x20002424
        hal_reg_info(pst_hal_device, 0x20002428, &pst_rx_node->ul_data4); // 配置寄存地址 0x20002428
        ...
        pst_rx_node->ul_data5 = pul_tx_dscr[7]; // data1为2时，从desr 7中获取数据
    }
    ...
    /* 清错误状态寄存器 */
    hal_reg_write(pst_hal_device, 0x20002410, pst_rx_node->ul_data2); // 配置寄存地址 0x20002410

    /* 清中断状态寄存器 */
    hal_reg_write(pst_hal_device, 0x20002404, pst_rx_node->ul_data1); // 配置寄存地址 0x20002404
...
oal_void  hut_report_in(hut_rx_node *pst_rx_node)
{
    hal_to_dmac_device_stru   *pst_hal_device = OAL_PTR_NULL;
    hut_frag_hdr_stru          st_frag_hdr;
    oal_uint8                  auc_buf[22]; // buffsize 22字节
    ...
    *********************************************************/
    ...
    auc_buf[2] = ul_intr_status & 0xFF; // 2下标用于存储低8位
    auc_buf[3] = (ul_intr_status >> 8) & 0xFF; // 3下标用于存储8位开始高8位
    auc_buf[4] = (ul_intr_status >> 16) & 0xFF; // 4下标用于存储低16位开始高8位
    auc_buf[5] = (ul_intr_status >> 24) & 0xFF; // 5下标用于存储低24位开始高8位

    /* 读取错误状态寄存器 */
    ul_err_intr_stat = pst_rx_node->ul_data2;
    auc_buf[6] = ul_err_intr_stat & 0xFF; // 6下标用于存储低8位
    auc_buf[7] = (ul_err_intr_stat >> 8) & 0xFF; // 7下标用于存储8位开始高8位
    auc_buf[8] = (ul_err_intr_stat >> 16) & 0xFF; // 8下标用于存储低16位开始高8位
    auc_buf[9] = (ul_err_intr_stat >> 24) & 0xFF; // 9下标用于存储低24位开始高8位
    /* 读取tx/rx mpdu count */
    ul_mpdu_cnt = pst_rx_node->ul_data4;
    auc_buf[10] = ul_mpdu_cnt & 0xFF; // 10下标用于存储低8位
    auc_buf[11] = (ul_mpdu_cnt >> 8) & 0xFF; // 11下标用于存储8位开始高8位
    auc_buf[12] = (ul_mpdu_cnt >> 16) & 0xFF; // 12下标用于存储低16位开始高8位
    auc_buf[13] = (ul_mpdu_cnt >> 24) & 0xFF; // 13下标用于存储低24位开始高8位
    /* 读取tx/rx frame ptr */
    ul_dscr_ptr = pst_rx_node->ul_data3;
    auc_buf[14] = ul_dscr_ptr & 0xFF; // 14下标用于存储低8位
    auc_buf[15] = (ul_dscr_ptr >> 8) & 0xFF; // 15下标用于存储8位开始高8位
    auc_buf[16] = (ul_dscr_ptr >> 16) & 0xFF; // 16下标用于存储低16位开始高8位
    auc_buf[17] = (ul_dscr_ptr >> 24) & 0xFF; // 17下标用于存储低24位开始高8位
    /* 读取status */
    ul_status = ul_dscr_ptr = pst_rx_node->ul_data5;
    auc_buf[18] = ul_status & 0xFF; // 18下标用于存储低8位
    auc_buf[19] = (ul_status >> 8) & 0xFF; // 19下标用于存储8位开始高8位
    auc_buf[20] = (ul_status >> 16) & 0xFF; // 20下标用于存储低16位开始高8位
    auc_buf[21] = (ul_status >> 24) & 0xFF; // 21下标用于存储低24位开始高8位
    /* 中断上半部会申请事件，所以需要关中断 */
...
oal_void  hut_tasklet_handler(oal_ulong l_data)
{
...
OAL_STATIC oal_void hut_get_cmd_name(oal_int8 *pc_dst, oal_int8 *pc_src, oal_uint32 ul_name_len)
{
    if (ul_name_len < HUT_CMD_NAME_MAX_LEN) {
        HUT_ERR_LOG1(0, "hut_get_cmd_name::ul_name_len is not enough %d", ul_name_len);
        return;
    }
    pc_dst[0] = pc_src[0];
    pc_dst[1] = pc_src[1];
    pc_dst[2] = '\0'; // 最后一个字节2写字符串结束符
}
...
OAL_STATIC oal_uint32  hut_get_cmd_id(oal_uint8 *puc_data, oal_uint8 *puc_cmd_id)
{
    ...
    /* 获取命令字 */
    hut_get_cmd_name(ac_cmd, (oal_int8 *)puc_data, HUT_CMD_NAME_MAX_LEN);
...
OAL_STATIC oal_uint32  hut_non_frag_msg_recv(oal_uint8 *puc_data, oal_uint32 ul_len)
{
    ...
    ***********************/
    ...
    return (oal_uint32)g_ast_hut_cmd_ops[uc_cmd_id].p_hut_cmd_func(puc_data + 2); // 跳过前2字节命令字
}
...
OAL_STATIC oal_int32  hut_1st_frag_write_mem(oal_uint8 *puc_data, oal_void *p_param)
{
    ...
    puc_data += OAL_SIZEOF(hut_frag_hdr_stru) + 2; // 偏移至头数据后2字节
    pst_cmd_fmt = (hut_cmd_fmt_stru *)puc_data;
    ...
    l_ret = memcpy_s((oal_uint8 *)ul_virt_addr, ul_write_len + 1,
                     puc_data + OAL_SIZEOF(hut_cmd_fmt_stru), ul_write_len);
    if (l_ret != EOK) {
        return OAL_FAIL;
    }
...
OAL_STATIC oal_int32  hut_2nd_frag_write_mem(oal_uint8 *puc_data, oal_void *p_param)
{
    ...
    l_ret = memcpy_s((oal_uint8 *)ul_virt_addr, ul_len + 1,
                     puc_data + OAL_SIZEOF(hut_frag_hdr_stru), ul_len);
    if (l_ret != EOK) {
        return OAL_FAIL;
    }
...
{
    hut_frag_hdr_stru   *pst_frag_hdr = OAL_PTR_NULL;
...
oal_void hut_report_mem_loop(hut_frag_hdr_stru st_frag_hdr, oal_uint32 ul_virt_addr, oal_uint32 ul_send_len,
...
oal_void  hut_report_mem(oal_uint8 *puc_data)
{
    ...
    *******************************************************************/
    ...
    hut_report_mem_loop(ul_virt_addr, ul_send_len, ul_remain_len);
}
...
{
    hal_to_dmac_device_stru         *pst_hal_device = OAL_PTR_NULL;
    hal_rx_dscr_queue_header_stru   *pst_rx_dscr_queue = OAL_PTR_NULL;
    hut_rx_buffer_addr_stru         *pst_rx_dscr_addr = OAL_PTR_NULL;
...
oal_int32 hut_main_init(oal_void)
{
    ...
    puc_base_addr = (oal_uint8 *)oal_memalloc(HUT_MEM_MAX_LEN + 3); // 申请最大长度+3空间用以存储控制信息
    ...
    HUT_INFO_VAR(0, "hut_main_init, mem start addr is 0x%x.", 
                 OAL_VIRT_TO_PHY_ADDR((oal_void *)g_st_base_addr.puc_base_addr_align));

    /* 创建工作队列，用于内存数据上报 */
    g_st_hut_workqueue.pst_rx_workqueue = OAL_CREATE_SINGLETHREAD_WORKQUEUE("hut_rx");
    if (g_st_hut_workqueue.pst_rx_workqueue == OAL_PTR_NULL) {
...
{
    if (g_st_base_addr.puc_base_addr_origin != OAL_PTR_NULL) {
        /* 释放申请的内存 */
        oal_free(g_st_base_addr.puc_base_addr_origin);
    }
