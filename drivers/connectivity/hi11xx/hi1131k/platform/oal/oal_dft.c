...
OAL_STATIC oal_int32 oal_dft_dump_key_info(char* buf, oal_int32 key_type, oal_uint32 print_tag)
{
    ...
    struct rtc_time first_tm = {0};
    struct rtc_time last_tm = {0};
    oal_list_entry_stru *pst_pos = OAL_PTR_NULL;
    oal_dft_trace_item  *pst_dft_item = OAL_PTR_NULL;
    ...
    OAL_LIST_SEARCH_FOR_EACH(pst_pos, &g_dft_head)
    {
        ...
        if (memcpy_s(&first_tv, OAL_SIZEOF(first_tv),
                     &pst_dft_item->first_timestamp, OAL_SIZEOF(first_tv)) != EOK) {
            OAL_IO_PRINT("{frw_sysfs_entry_init::memcpy_s failed!}\n");
            return 0;
        }

        first_tv.tv_sec -= (oal_long)sys_tz.tz_minuteswest * 60;
        oal_rtc_time_to_tm(first_tv.tv_sec, &first_tm);

        if (memcpy_s(&last_tv, OAL_SIZEOF(last_tv),
                     &pst_dft_item->last_timestamp, OAL_SIZEOF(last_tv)) != EOK) {
            OAL_IO_PRINT("{frw_sysfs_entry_init::memcpy_s failed!}\n");
            return 0;
        }
        if (last_tv.tv_sec) {
            last_tv.tv_sec -= (oal_long)sys_tz.tz_minuteswest * 60;
        ...
        if (!last_time_flag) {
            if (print_tag & KERN_PRINT)
                printk(KERN_DEBUG"%s,1st at %02d:%02d:%02d:%02d:%02d,total:%u\n",
                        pst_dft_item->name, first_tm.tm_mon+1, first_tm.tm_mday,
            ...
            if (print_tag & BUFF_PRINT) {
                if (buf != NULL) {
                    ret +=  snprintf_s(buf + ret, PAGE_SIZE - ret, PAGE_SIZE - ret - 1,
                                       ...
                                       pst_dft_item->name, first_tm.tm_mon + 1, first_tm.tm_mday,
                                       first_tm.tm_hour, first_tm.tm_min, first_tm.tm_sec, pst_dft_item->trace_count);
                }
            }
        } else {
            if (print_tag & KERN_PRINT)
                printk(KERN_DEBUG"%s,1st at %02d:%02d:%02d:%02d:%02d,last at %02d:%02d:%02d:%02d:%02d,total:%u\n",
                       ...
                       last_tm.tm_hour, last_tm.tm_min, last_tm.tm_sec, pst_dft_item->trace_count);
            if (print_tag & BUFF_PRINT) {
                if (buf != NULL) {
                    ret += snprintf_s(buf + ret, PAGE_SIZE - ret, PAGE_SIZE - ret - 1, 
    ...
    if (ret < EOK) {
        OAL_IO_PRINT("{oal_dft_dump_key_info::snprintf_s failed!}\n");
        oal_spin_unlock_irq_restore(&g_dft_head_lock, &flags);
        return ret;
    }
    oal_spin_unlock_irq_restore(&g_dft_head_lock, &flags);
...
OAL_STATIC ssize_t  oal_dft_get_key_info(struct kobject *dev, struct kobj_attribute *attr, char* buf)
{
    ...
    if (oal_list_is_empty(&g_dft_head)) {
        ...
        ret +=  snprintf_s(buf + ret, PAGE_SIZE - ret, PAGE_SIZE - ret - 1, "key info empty\n");
        if (ret < EOK) {
            OAL_IO_PRINT("{oal_dft_get_key_info::snprintf_s failed!}\n");
            return ret;
        }
        return ret;
    }
...
OAL_STATIC oal_int32 oal_dft_print_version_info(char* buf)
{
    ...
    ret +=  snprintf_s(buf + ret, PAGE_SIZE - ret, PAGE_SIZE - ret - 1, "%s\n", HI110X_DRV_VERSION);  //lint !e122 !e146
    if (ret < EOK) {
        OAL_IO_PRINT("{oal_dft_print_version_info::snprintf_s failed!}\n");
        return ret;
    }
    return ret;
}
