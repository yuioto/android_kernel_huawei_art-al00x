...
#define HAL_MAX_VAP_NUM                    (HAL_MAX_AP_NUM + HAL_MAX_STA_NUM)  /* HAL VAP个数 */

#define HAL_VAP_ID_IS_VALID(_vap_id)      (((_vap_id) == 0) || ((_vap_id) == 1) || ((_vap_id) == 4) || \
    ((_vap_id) == 5) || ((_vap_id) == 6))

#ifdef _PRE_WLAN_FEATURE_1131K_BTCOEX
#define BTCOEX_UART_HEADER_MASK            0x3
#define BTCOEX_UART_HEADER_TYPE_STATUS     0x1
#define BTCOEX_UART_HEADER_TYPE_REQUEST    0x2
#endif
...
#define HAL_MAX_VAP_NUM                    (HAL_MAX_AP_NUM + HAL_MAX_STA_NUM)  /* HAL VAP个数  */
#define HAL_VAP_ID_IS_VALID(_vap_id)       (((_vap_id) == 0) || ((_vap_id) == 1) || ((_vap_id) == 2) || \
    ((_vap_id) == 3) || ((_vap_id) == 4))
...
#ifdef _PRE_WLAN_FEATURE_1131K_BTCOEX
    HAL_EVENT_DMAC_BTCOEX_PS,           /* 共存ACL状态变化触发PS机制事件 */
#else
...
#ifdef _PRE_WLAN_FEATURE_1131K_BTCOEX
    HAL_EVENT_DMAC_MIST_BTCOEX,             /* 共存事件 */
#else
...
typedef enum {
    BTCOEX_UART_BURST1 = 0,
    BTCOEX_UART_BURST2 = 1,

    BTCOEX_UART_BURST_BUTT
} uart_burst_enum;
#endif
...
#ifdef _PRE_WLAN_FEATURE_1131K_BTCOEX
    uint16_t                            wait_timeout;     /* 软件定时器超时时间 */
#else
...
typedef struct {
    ...
    oal_uint8     uc_pn_key_type;     /*
                                       * 1151 0:multicast,1:unicast
...
typedef struct hal_btcoex_btble_status {
    uint32_t bit_bt_on           : 1,
             ...
             bit_ble_con         : 1,
             bit_ble_adv         : 1,
             bit_bt_transfer     : 1, /* wifi use only, bit_bt_data_send || bit_bt_data_recv */
             bit_bt_6slot        : 2,
             bit_ble_init        : 1,
             bit_bt_acl          : 1,
             bit_bt_ldac         : 2,
             bit_reserved2       : 4,
             bit_bt_ba           : 1; /* wifi use only */
} hal_btcoex_btble_status_stru;

typedef struct {
    uint32_t abort_start_cnt;
    uint32_t abort_done_cnt;
    uint32_t abort_end_cnt;
    uint32_t preempt_cnt;
    uint32_t post_preempt_cnt;
    uint32_t post_premmpt_fail_cnt;
    uint32_t abort_duration_on;
    uint32_t abort_duration_start_us;
    uint32_t abort_duration_us;
    uint32_t abort_duration_s;
} hal_btcoex_statistics_stru;

typedef struct hal_btcoex_wifi_uart_status {
    uint32_t bit_wifi_freq                 : 1,
             bit_wifi_channel              : 5,
             bit_wifi_bandwith             : 3,
             bit_wifi_join_state           : 1,
             bit_wifi_key_frame            : 3,
             bit_wifi_vap_state            : 1,
             bit_wifi_p2p_scan_state       : 1,
             bit_wifi_p2p_connect_state    : 1,
             bit_wifi_on                   : 1,
             bit_wifi_scan_state           : 1,
             bit_wifi_connecting_state     : 1,
             bit_wifi_connect_state        : 1,
             bit_wifi_low_rate             : 2,
             bit_wifi_low_power            : 1,
             bit_wifi_ps_enable            : 1,
             bit_wifi_ps_state             : 1,
             bit_reserved1                 : 1,
             bit_wifi_slot_level           : 4,
             bit_reserved2                 : 1,
             bit_wifi_cali_state           : 1;
} hal_btcoex_wifi_uart_status_stru;

typedef struct {
    uint8_t bit_ps_on         : 1,  /* ps软件机制: 0=关闭, 1=打开 */
...
typedef struct {
#ifdef _PRE_WLAN_FEATURE_BTCOEX
    oal_uint64                          ps_cur_time;      /* 用于ps中断上下半部执行时间统计 */
#endif
    oal_atomic                          ps_event_num;     /* ps中断event数目 */
    oal_atomic                          acl_en_cnt;       /* 连续acl cnt的计数，达到一定次数时，可能是蓝牙长时间为恢复 */
    uint16_t                            timeout_ms;       /* ps超时时间，page扫描190slot 音乐和数传30slot */
    hal_coex_sw_preempt_mode_stru       sw_preempt_mode;
    uint8_t                             sw_preempt_type;
    uint8_t                             sw_preempt_subtype;
    uint8_t                             protect_coex_pri; /* one pkt帧发送优先级 */
    oal_bool_enum_uint8                 last_acl_status;  /* 保存上一次acl状态 */
    oal_bool_enum_uint8                 ps_stop;          /* 特定业务下，不需要开启ps，通知蓝牙不要发送ps中断 */
    oal_bool_enum_uint8                 ps_pause;         /* 特定业务下需暂停ps，不影响ps中断处理，防止和wifi特定业务冲突 */
    oal_bool_enum_uint8                 coex_pri_forbit;  /* coex pri控制开关，ldac下需要关闭该功能 */
    uint8_t                             ps_frame_type;
    uint8_t                             resv[1];
} hal_device_btcoex_sw_preempt_stru;

typedef struct hal_device_btcoex_prio_occupied {
    frw_timeout_stru btcoex_prio_occupied_timer;
    frw_timeout_stru btcoex_wl_active_timer;
    uint32_t         ap_beacon_count;
    uint32_t         prio_timestamp;
    uint8_t          ap_beacon_miss;
    uint8_t          occupied_times;
    uint8_t          prio_occupied_state;
    uint8_t          wl_active_state;
    uint8_t          linkloss_index;
    uint8_t          linkloss_occupied_times;
    uint8_t          resv[2];
} hal_device_btcoex_prio_occupied_stru;

typedef struct hal_btcoex_wifi_status_burst1 {
    uint8_t bit_header        : 2,
            bit_burst_num     : 1,
            bit_wifi_bandwith : 1,
            bit_wifi_channel  : 4;
} hal_btcoex_wifi_status_burst1_stru;

typedef struct hal_btcoex_wifi_status_burst2 {
    uint8_t bit_header             : 2,
            bit_burst_num          : 1,
            bit_wifi_connect_state : 1,
            bit_wifi_on            : 1,
            ...
            bit_wifi_scan_state    : 1,
            bit_reserved           : 1;
} hal_btcoex_wifi_status_burst2_stru;

typedef struct hal_btcoex_wifi_status {
    hal_btcoex_wifi_status_burst1_stru btcoex_wifi_status1;
    hal_btcoex_wifi_status_burst2_stru btcoex_wifi_status2;
    uint8_t                            resv[2];
} hal_btcoex_wifi_status_stru;

typedef enum dmac_btcoex_bt_protect {
    DMAC_BTCOEX_NO_PROTECT,       /* 保护恢复，或者正常无保护状态 */
    DMAC_BTCOEX_PRE_PROTECT,      /* 防抖动进入保护前的预保护状态 */
    DMAC_BTCOEX_PROTECT,          /* 已经进入保护状态 */
} dmac_btcoex_bt_protect_enum;

typedef struct hal_device_btcoex {
    frw_timeout_stru                     btcoex_powersave_timer;
    frw_timeout_stru                     btcoex_uart_timer;
    hal_btcoex_btble_status_stru         btcoex_btble_status;
    hal_btcoex_statistics_stru           btcoex_statistics;
    hal_device_btcoex_sw_preempt_stru    btcoex_sw_preempt;
    hal_btcoex_wifi_uart_status_stru     btcoex_wifi_uart_status;
    hal_device_btcoex_prio_occupied_stru btcoex_pri_occupied;
#ifdef _PRE_WLAN_FEATURE_BTCOEX
    oal_atomic                           uart_msg_num;
#endif
    hal_btcoex_wifi_status_stru          btcoex_wifi_status;
    uint32_t                             bt_protect_timestamp;
    uint32_t                             high_prio_timestamp;
    uint8_t                              bt_protect_status;
    uint8_t                              btcoex_uart_recieve;
    uint8_t                              high_pri_timeout;         /* 拉高wlan优先级 */
    uint8_t                              wl_active_timeout;        /* 拉wl_active */
    uint8_t                              btcoex_sco_disable_cts_state;
    uint8_t                              btcoex_ps_channel;
    uint8_t                              resv[3];
} hal_device_btcoex_stru;
#endif
...
#elif ((_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1131C_DEV) || (_PRE_TEST_MODE != _PRE_TEST_MODE_OFF) || \
    (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1131C_HOST))
typedef struct {
    /* TX POWER (第14行) */
...
#else
/* 针对接收，提供读取接口 frame_len长度 802.11帧头长度(uc_mac_hdr_len) */
typedef struct {
    /* word 0 */
...
typedef struct tag_hal_to_dmac_device_stru {
    ...
    wlan_p2p_mode_enum_uint8        en_p2p_mode;   /* 指示当前创建的P2P 是处于CL/GO 模式。由于P2P 中断产生后只能通过创建
                                                    * 的P2P 设备是CL 还是GO 来获取对应的hal_vap_id，所以增加该变量 */
...
extern oal_uint32 hal_chip_get_device_impl(oal_bus_dev_stru *pst_bus_stru,
                                           oal_uint8 uc_device_id,
...
extern oal_void hi1151_rx_add_dscr(hal_to_dmac_device_stru *pst_hal_device,
                                   hal_rx_dscr_queue_id_enum_uint8 en_queue_num,
                                   oal_uint16 us_rx_buff);
...
extern oal_void hi1151_get_tx_dscr_queue_total_ppdu_num(hal_to_dmac_device_stru *pst_hal_device,
                                                        oal_uint16 *pus_ppdu_cnt);
...
extern oal_void hi1151_tx_non_ucast_data_set_dscr(hal_to_dmac_device_stru *pst_hal_device,
                                                  ...
                                                  hal_tx_txop_alg_stru *pst_txop_alg,
                                                  hal_tx_ppdu_feature_stru *pst_ppdu_feature);
extern oal_void hi1151_tx_set_dscr_modify_mac_header_length(hal_tx_dscr_stru *pst_tx_dscr,
...
extern oal_void hi1151_tx_get_size_dscr(oal_uint8 us_msdu_num,
                                        oal_uint32 *pul_dscr_one_size,
                                        oal_uint32 *pul_dscr_two_size);
extern oal_void hi1151_tx_get_vap_id(hal_tx_dscr_stru *pst_tx_dscr,
                                     oal_uint8 *puc_vap_id,
                                     oal_int32 l_proxysta_feature);
extern oal_void hi1151_tx_get_dscr_ctrl_one_param(hal_tx_dscr_stru *pst_tx_dscr,
...
extern oal_void hi1151_tx_get_dscr_chiper_type(hal_tx_dscr_stru *pst_tx_dscr,
...
extern oal_void hi1151_tx_put_dscr(hal_to_dmac_device_stru *pst_hal_device,
                                   hal_tx_queue_type_enum_uint8 en_tx_queue_type,
                                   hal_tx_dscr_stru *past_tx_dscr);
extern oal_void hi1151_get_tx_q_status(hal_to_dmac_device_stru *pst_hal_device,
                                       oal_uint32 *pul_status,
                                       oal_uint8 uc_qnum);
extern oal_void hi1151_tx_get_ampdu_len(hal_to_dmac_device_stru *pst_hal_device,
                                        hal_tx_dscr_stru *pst_dscr,
                                        oal_uint32 *pul_ampdu_len);

#ifdef _PRE_DEBUG_MODE_USER_TRACK
extern oal_void hi1151_tx_get_protocol_mode(hal_to_dmac_device_stru *pst_hal_device,
                                            hal_tx_dscr_stru *pst_dscr,
                                            oal_uint8 *puc_protocol_mode);
#endif
extern oal_void hi1151_rx_get_size_dscr(oal_uint32 *pul_dscr_size);
extern oal_void hi1151_rx_get_info_dscr(oal_uint32 *pul_rx_dscr,
                                        hal_rx_ctl_stru *pst_rx_ctl,
                                        hal_rx_status_stru *pst_rx_status,
                                        hal_rx_statistic_stru *pst_rx_statistics);
extern oal_void hi1151_get_hal_vap(hal_to_dmac_device_stru *pst_hal_device,
                                   oal_uint8 uc_vap_id,
...
extern oal_void hi1151_rx_show_dscr_queue_info(hal_to_dmac_device_stru *pst_hal_device,
                                               hal_rx_dscr_queue_id_enum_uint8 en_rx_dscr_type);
extern oal_void hi1151_rx_sync_invalid_dscr(hal_to_dmac_device_stru *pst_hal_device,
                                            oal_uint32 *pul_dscr,
                                            oal_uint8 en_queue_num);
extern oal_void hi1151_rx_free_dscr_list(hal_to_dmac_device_stru *pst_hal_device,
                                         hal_rx_dscr_queue_id_enum_uint8 en_queue_num,
...
extern oal_void hi1151_set_machw_rx_buff_addr(hal_to_dmac_device_stru *pst_hal_device,
                                              oal_uint32 ul_rx_dscr,
                                              hal_rx_dscr_queue_id_enum_uint8 en_queue_num);
extern oal_void hi1151_resume_mac_rx_dscr_addr(hal_to_dmac_device_stru *pst_hal_device,
                                               oal_uint32 ul_rx_dscr,
                                               hal_rx_dscr_queue_id_enum_uint8 en_queue_num);
...
extern oal_void hi1151_reset_phy_machw(hal_to_dmac_device_stru *pst_hal_device,
...
extern oal_void hi1151_set_bandwidth_mode(hal_to_dmac_device_stru *pst_hal_device,
                                          wlan_channel_bandwidth_enum_uint8 en_bandwidth);

#ifdef _PRE_WLAN_FEATURE_TPC
extern oal_void hi1151_set_upc_data(hal_to_dmac_device_stru *pst_hal_device,
                                    oal_uint8 uc_band,
                                    oal_uint8 uc_subband_idx);
extern oal_void hi1151_set_tpc_params(hal_to_dmac_device_stru *pst_hal_device,
                                      oal_uint8 uc_band,
                                      oal_uint8 uc_channel_num);
#endif

extern oal_void hi1151_process_phy_freq(hal_to_dmac_device_stru *pst_hal_device);
extern oal_void hi1151_set_primary_channel(hal_to_dmac_device_stru *pst_hal_device,
                                           oal_uint8 uc_channel_num,
                                           oal_uint8 uc_band,
...
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC != _PRE_MULTI_CORE_MODE)
extern oal_void hi1151_freq_adjust(hal_to_dmac_device_stru *pst_hal_device,
...
extern oal_void hi1151_add_machw_ba_lut_entry(hal_to_dmac_device_stru *pst_hal_device, oal_uint8 uc_lut_index,
    oal_uint8 *puc_dst_addr, oal_uint8 uc_tid, oal_uint16 uc_seq_no, oal_uint8 uc_win_size);
extern oal_void hi1151_remove_machw_ba_lut_entry(hal_to_dmac_device_stru *pst_hal_device, oal_uint8 uc_lut_index);
extern oal_void hi1151_get_machw_ba_params(hal_to_dmac_device_stru *pst_hal_device,
                                           oal_uint8 uc_index,
                                           oal_uint32 *pst_addr_h,
                                           oal_uint32 *pst_addr_l,
                                           oal_uint32 *pst_ba_para);
extern oal_void hi1151_restore_machw_ba_params(hal_to_dmac_device_stru *pst_hal_device,
                                               oal_uint8 uc_index,
                                               oal_uint32 ul_addr_h,
                                               oal_uint32 ul_addr_l,
                                               oal_uint32 ul_ba_para);
extern oal_void hi1151_machw_seq_num_index_update_per_tid(hal_to_dmac_device_stru *pst_hal_device,
                                                          oal_uint8 uc_lut_index,
                                                          oal_uint8 uc_qos_flag);
extern oal_void hi1151_set_tx_sequence_num(hal_to_dmac_device_stru *pst_hal_device,
                                           oal_uint8 uc_lut_index,
                                           oal_uint8 uc_tid,
                                           oal_uint8 uc_qos_flag,
                                           oal_uint32 ul_val_write);
extern oal_void hi1151_get_tx_sequence_num(hal_to_dmac_device_stru *pst_hal_device,
                                           oal_uint8 uc_lut_index,
                                           oal_uint8 uc_tid,
                                           oal_uint8 uc_qos_flag,
...
extern oal_void hi1151_reset_reg_dma_save(hal_to_dmac_device_stru *pst_hal,
                                          oal_uint8 *uc_dmach0,
                                          oal_uint8 *uc_dmach1,
                                          oal_uint8 *uc_dmach2);
extern oal_void hi1151_reset_reg_dma_restore(hal_to_dmac_device_stru *pst_hal,
                                             oal_uint8 *uc_dmach0,
                                             oal_uint8 *uc_dmach1,
                                             oal_uint8 *uc_dmach2);
...
extern oal_void hi1151_radar_config_reg(hal_to_dmac_device_stru *pst_hal_device,
...
extern oal_void hi1151_get_mac_error_int_status(hal_to_dmac_device_stru *pst_hal_device,
                                                hal_error_state_stru *pst_state);
extern oal_void hi1151_clear_mac_error_int_status(hal_to_dmac_device_stru *pst_hal_device,
                                                  hal_error_state_stru *pst_status);
extern oal_void hi1151_unmask_mac_error_init_status(hal_to_dmac_device_stru *pst_hal_device,
                                                    hal_error_state_stru *pst_status);
...
extern oal_void hi1151_get_irq_stat(hal_to_dmac_device_stru *pst_hal_device,
                                    oal_uint8 *puc_param,
                                    oal_uint32 ul_len,
                                    oal_uint8 uc_type);
extern oal_void hi1151_get_vap(hal_to_dmac_device_stru *pst_hal_device,
                               wlan_vap_mode_enum_uint8 vap_mode,
                               oal_uint8 vap_id,
                               hal_to_dmac_vap_stru **ppst_hal_vap);
extern oal_void hi1151_add_vap(hal_to_dmac_device_stru *pst_hal_device,
                               wlan_vap_mode_enum_uint8 vap_mode,
                               oal_uint8 uc_mac_vap_id,
                               hal_to_dmac_vap_stru **ppst_hal_vap);
extern oal_void hi1151_del_vap(hal_to_dmac_device_stru *pst_hal_device,
                               wlan_vap_mode_enum_uint8 vap_mode,
                               oal_uint8 vap_id);

extern oal_void hi1151_set_proxysta_enable(hal_to_dmac_device_stru *pst_hal_device, oal_int32 l_enable);
#ifdef _PRE_WLAN_FEATURE_PROXYSTA
extern oal_void hi1151_set_proxysta_vap_addr(hal_to_dmac_device_stru *pst_hal_device,
                                             oal_uint8 vap_id,
                                             oal_uint8 *puc_mac_addr);
#endif
extern oal_void hi1151_config_eifs_time(hal_to_dmac_device_stru *pst_hal_device, wlan_protocol_enum_uint8 en_protocol);
extern oal_void hi1151_register_alg_isr_hook(hal_to_dmac_device_stru *pst_hal_device,
                                             hal_isr_type_enum_uint8 en_isr_type,
                                             hal_alg_noify_enum_uint8 en_alg_notify,
                                             p_hal_alg_isr_func p_func);
extern oal_void hi1151_unregister_alg_isr_hook(hal_to_dmac_device_stru *pst_hal_device,
                                               hal_isr_type_enum_uint8 en_isr_type,
                                               hal_alg_noify_enum_uint8 en_alg_notify);
extern oal_void hi1151_one_packet_start(struct tag_hal_to_dmac_device_stru *pst_hal_device,
                                        hal_one_packet_cfg_stru *pst_cfg);
extern oal_void hi1151_one_packet_stop(struct tag_hal_to_dmac_device_stru *pst_hal_device);
extern oal_void hi1151_one_packet_get_status(struct tag_hal_to_dmac_device_stru *pst_hal_device,
...
extern oal_void hi1151_get_ampdu_bytes(hal_to_dmac_device_stru *pst_hal_device,
...
extern oal_void hi1151_mac_error_msg_report(hal_to_dmac_device_stru *pst_hal_device,
...
extern oal_void hi1151_ce_add_key(hal_to_dmac_device_stru *pst_hal_device,
...
extern oal_void hi1151_ce_add_peer_macaddr(hal_to_dmac_device_stru *pst_hal_device,
                                           oal_uint8 uc_lut_idx,
                                           oal_uint8 *puc_addr);
...
extern oal_void hi1151_tx_get_ra_lut_index(hal_to_dmac_device_stru *pst_hal_device,
                                           hal_tx_dscr_stru *pst_dscr,
...
extern oal_void hi1151_get_ch_statics_result(hal_to_dmac_device_stru *pst_hal_device,
                                             hal_ch_statics_irq_event_stru *pst_ch_statics);
extern oal_void hi1151_get_ch_measurement_result(hal_to_dmac_device_stru *pst_hal_device,
                                                 hal_ch_statics_irq_event_stru *pst_ch_statics);
...
extern oal_void hi1151_get_radar_det_result(hal_to_dmac_device_stru *pst_hal_device,
                                            hal_radar_det_event_stru *pst_radar_info);
extern oal_void hi1151_update_rts_rate_params(hal_to_dmac_device_stru *pst_hal_device,
                                              wlan_channel_band_enum_uint8 en_band);
extern oal_void hi1151_set_rts_rate_params(hal_to_dmac_device_stru *pst_hal_device,
                                           hal_cfg_rts_tx_param_stru *pst_hal_rts_tx_param);
extern oal_void hi1151_set_rts_rate_selection_mode(hal_to_dmac_device_stru *pst_hal_device,
...
extern oal_void hi1151_set_tpc_init_rate_dac_lpf_table(oal_uint8 *pauc_rate_pow_table_2G,
                                                       oal_uint8 *pauc_rate_pow_table_5G,
                                                       oal_uint8 *pauc_mode_len,
                                                       oal_uint8 uc_pow_mode);
extern oal_void hi1151_set_dac_lpc_pa_upc_level(oal_uint8 uc_dac_lpf_code,
                                                oal_int8 *pac_tpc_level_table,
                                                oal_uint8 uc_tpc_level_num,
...
extern oal_void hi1151_get_bcn_rate(hal_to_dmac_device_stru *pst_hal_device,
                                    oal_uint8 uc_vap_id,
                                    oal_uint8 *puc_data_rate);
extern oal_void hi1151_set_bcn_phy_tx_mode(hal_to_dmac_device_stru *pst_hal_device,
                                           oal_uint8 uc_tpc_code,
                                           oal_uint8 uc_vap_id);
extern oal_void hi1151_get_spec_frm_rate(hal_to_dmac_device_stru *pst_hal_device);
extern oal_void hi1151_set_spec_frm_phy_tx_mode(hal_to_dmac_device_stru *pst_hal_device,
                                                oal_uint8 uc_band,
                                                oal_uint8 uc_subband_idx);
...
extern oal_void hi1151_set_txbf_lut_info(hal_to_dmac_device_stru *pst_hal_device,
                                         oal_uint8 uc_lut_index,
                                         oal_uint16 ul_reg_value);
extern oal_void hi1151_get_txbf_lut_info(hal_to_dmac_device_stru *pst_hal_device,
                                         oal_uint8 uc_lut_index,
...
extern oal_void hi1151_set_peer_lut_info(hal_to_dmac_device_stru *pst_hal_device,
                                         oal_uint8 *puc_mac_addr,
                                         oal_uint8 uc_lut_index);
extern oal_void hi1151_set_mu_aid_matrix_info(hal_to_dmac_device_stru *pst_hal_device,
                                              oal_uint8 uc_vap_id,
                                              oal_uint16 us_aid,
                                              oal_uint8 *p_matrix);
...
extern oal_void hi1151_set_agc_unlock_min_th(hal_to_dmac_device_stru *pst_hal_device,
                                             oal_int32 l_tx_reg_val,
                                             oal_int32 l_rx_reg_val);
extern oal_void hi1151_set_nav_max_duration(hal_to_dmac_device_stru *pst_hal_device,
                                            oal_uint16 us_bss_dur,
                                            oal_uint16 us_obss_dur);
#endif
...
#endif
extern oal_void hi1151_get_mac_statistics_data(hal_to_dmac_device_stru *pst_hal_device,
                                               hal_mac_key_statis_info_stru *pst_mac_key_statis);
#ifdef _PRE_WLAN_FEATURE_CCA_OPT
extern oal_void hi1151_get_ed_high_th(hal_to_dmac_device_stru *pst_hal_device,
                                      oal_int8 *pl_ed_high_reg_val);
extern oal_void hi1151_set_ed_high_th(hal_to_dmac_device_stru *pst_hal_device,
                                      oal_int32 l_ed_high_20_reg_val,
                                      oal_int32 l_ed_high_40_reg_val);
extern oal_void hi1151_enable_sync_error_counter(hal_to_dmac_device_stru *pst_hal_device,
...
#endif
extern oal_void hi1151_set_soc_lpm(hal_to_dmac_device_stru *pst_hal_device,
...
#endif
extern oal_void hi1151_set_wow_en(hal_to_dmac_device_stru *pst_hal_device,
                                  oal_uint32 ul_set_bitmap,
                                  hal_wow_param_stru *pst_para);
extern oal_void hi1151_set_lpm_state(hal_to_dmac_device_stru *pst_hal_device,
...
extern oal_void hi1151_set_coex_ctrl(hal_to_dmac_device_stru *pst_hal_device,
                                     oal_uint32 ul_mac_ctrl,
                                     oal_uint32 ul_rf_ctrl);
extern oal_void hi1151_get_hw_version(hal_to_dmac_device_stru *pst_hal_device,
...
extern oal_void hi1151_set_tx_dscr_field(hal_to_dmac_device_stru *pst_hal_device,
                                         oal_uint32 ul_data,
...
#endif
extern oal_void hi1151_rf_adjust_ppm(hal_to_dmac_device_stru *pst_hal_device,
                                     oal_int8 c_ppm,
                                     wlan_channel_bandwidth_enum_uint8  en_bandwidth,
                                     oal_uint8 uc_clock);
extern oal_void hi1151_set_daq_mac_reg(hal_to_dmac_device_stru *pst_hal_device,
                                       oal_uint32* pul_addr,
                                       oal_uint16 us_unit_len,
                                       oal_uint16 us_unit_num,
                                       oal_uint16 us_depth);
...
extern oal_void hi1151_vap_tsf_get_64bit(hal_to_dmac_vap_stru *pst_hal_vap,
...
extern oal_void hi1151_vap_set_machw_prot_params(hal_to_dmac_vap_stru *pst_hal_vap,
...
extern oal_void hi1151_vap_set_machw_aifsn_all_ac(hal_to_dmac_vap_stru *pst_hal_vap,
                                                  oal_uint8 uc_bk,
                                                  oal_uint8 uc_be,
                                                  oal_uint8 uc_vi,
                                                  oal_uint8 uc_vo);
extern oal_void hi1151_vap_set_machw_aifsn_ac(hal_to_dmac_vap_stru *pst_hal_vap,
                                              ...
                                              oal_uint8 uc_aifs);
extern oal_void hi1151_vap_set_edca_machw_cw(hal_to_dmac_vap_stru *pst_hal_vap,
                                             oal_uint8 uc_cwmax,
                                             oal_uint8 uc_cwmin,
                                             oal_uint8 uc_ac_type);
extern oal_void hi1151_vap_get_edca_machw_cw(hal_to_dmac_vap_stru *pst_hal_vap,
                                             oal_uint8 *puc_cwmax,
                                             oal_uint8 *puc_cwmin,
                                             oal_uint8 uc_ac_type);

extern oal_void hi1151_vap_set_machw_txop_limit_bkbe(hal_to_dmac_vap_stru *pst_hal_vap,
                                                     oal_uint16 us_be,
                                                     oal_uint16 us_bk);
extern oal_void hi1151_vap_get_machw_txop_limit_bkbe(hal_to_dmac_vap_stru *pst_hal_vap,
                                                     oal_uint16 *pus_be,
                                                     oal_uint16 *pus_bk);
extern oal_void hi1151_vap_set_machw_txop_limit_vivo(hal_to_dmac_vap_stru *pst_hal_vap,
                                                     oal_uint16 us_vo,
                                                     oal_uint16 us_vi);
extern oal_void hi1151_vap_get_machw_txop_limit_vivo(hal_to_dmac_vap_stru *pst_hal_vap,
                                                     oal_uint16 *pus_vo,
                                                     oal_uint16 *pus_vi);
extern oal_void hi1151_vap_set_machw_edca_bkbe_lifetime(hal_to_dmac_vap_stru *pst_hal_vap,
                                                        oal_uint16 us_be,
                                                        oal_uint16 us_bk);
extern oal_void hi1151_vap_get_machw_edca_bkbe_lifetime(hal_to_dmac_vap_stru *pst_hal_vap,
                                                        oal_uint16 *pus_be,
                                                        oal_uint16 *pus_bk);
extern oal_void hi1151_vap_set_machw_edca_vivo_lifetime(hal_to_dmac_vap_stru *pst_hal_vap,
                                                        oal_uint16 us_vo,
                                                        oal_uint16 us_vi);
extern oal_void hi1151_vap_get_machw_edca_vivo_lifetime(hal_to_dmac_vap_stru *pst_hal_vap,
                                                        oal_uint16 *pus_vo,
                                                        oal_uint16 *pus_vi);
...
extern oal_void hi1151_vap_set_noa(hal_to_dmac_vap_stru *pst_hal_vap,
                                   oal_uint32 ul_start_tsf,
                                   oal_uint32 ul_duration,
                                   oal_uint32 ul_interval,
                                   oal_uint8 uc_count);
#ifdef _PRE_WLAN_FEATURE_P2P
extern oal_void hi1151_vap_set_ops(hal_to_dmac_vap_stru *pst_hal_vap,
                                   oal_uint8 en_ops_ctrl,
                                   oal_uint8 uc_ct_window);
extern oal_void hi1151_vap_enable_p2p_absent_suspend(hal_to_dmac_vap_stru *pst_hal_vap,
                                                     oal_bool_enum_uint8 en_suspend_enable);
#endif
extern oal_void hi1151_set_sta_bssid(hal_to_dmac_vap_stru *pst_hal_vap,
                                     oal_uint8 *puc_byte);
extern oal_void hi1151_set_psm_dtim_period(hal_to_dmac_vap_stru *pst_hal_vap,
                                           oal_uint8 uc_dtim_period,
...
extern oal_void hi1151_tx_get_dscr_iv_word(hal_tx_dscr_stru *pst_dscr,
                                           oal_uint32 *pul_iv_ms_word,
                                           oal_uint32 *pul_iv_ls_word,
                                           oal_uint8 uc_chiper_type,
...
extern oal_void hi1102_tx_get_size_dscr(oal_uint8 us_msdu_num,
                                        oal_uint32 *pul_dscr_one_size,
                                        oal_uint32 *pul_dscr_two_size);
extern oal_void hi1102_tx_get_vap_id(hal_tx_dscr_stru *pst_tx_dscr,
                                     oal_uint8 *puc_vap_id,
                                     oal_int32 l_proxysta_feature);
...
extern oal_void hi1102_tx_get_dscr_chiper_type(hal_tx_dscr_stru *pst_tx_dscr,
                                               oal_uint8 *puc_chiper_type,
                                               oal_uint8 *puc_chiper_key_id);
...
extern oal_void hi1102_tx_put_dscr(hal_to_dmac_device_stru *pst_hal_device,
                                   hal_tx_queue_type_enum_uint8 en_tx_queue_type,
                                   hal_tx_dscr_stru *past_tx_dscr);
extern oal_void hi1102_get_tx_q_status(hal_to_dmac_device_stru *pst_hal_device,
                                       oal_uint32 *pul_status,
                                       oal_uint8 uc_qnum);
extern oal_void hi1102_tx_get_ampdu_len(hal_to_dmac_device_stru *pst_hal_device,
                                        hal_tx_dscr_stru *pstdscr,
                                        oal_uint32 *pul_ampdulen);
#ifdef _PRE_DEBUG_MODE_USER_TRACK
extern oal_void hi1102_tx_get_protocol_mode(hal_to_dmac_device_stru *pst_hal_device,
                                            hal_tx_dscr_stru *pst_dscr,
                                            oal_uint8 *puc_protocol_mode);
#endif
extern oal_void hi1102_rx_get_info_dscr(oal_uint32 *pul_rx_dscr,
                                        hal_rx_ctl_stru *pst_rx_ctl,
                                        hal_rx_status_stru *pst_rx_status,
                                        hal_rx_statistic_stru *pst_rx_statistics);
extern oal_void hi1102_get_hal_vap(hal_to_dmac_device_stru *pst_hal_device,
                                   oal_uint8 uc_vap_id,
                                   hal_to_dmac_vap_stru **ppst_hal_vap);
...
extern oal_void hi1102_rx_show_dscr_queue_info(hal_to_dmac_device_stru *pst_hal_device,
                                               hal_rx_dscr_queue_id_enum_uint8 en_rx_dscr_type);
extern oal_void hi1102_rx_sync_invalid_dscr(hal_to_dmac_device_stru *pst_hal_device,
                                            oal_uint32 *pul_dscr,
                                            oal_uint8 en_queue_num);
...
extern oal_void hi1102_set_machw_rx_buff_addr(hal_to_dmac_device_stru *pst_hal_device,
                                              oal_uint32 ul_rx_dscr,
                                              hal_rx_dscr_queue_id_enum_uint8 en_queue_num);
...
extern oal_void hi1102_set_bandwidth_mode(hal_to_dmac_device_stru *pst_hal_device,
                                          wlan_channel_bandwidth_enum_uint8 en_bandwidth);
#ifdef _PRE_WLAN_FEATURE_TPC
extern oal_void hi1102_set_upc_data(hal_to_dmac_device_stru *pst_hal_device,
                                    oal_uint8 uc_band,
                                    oal_uint8 uc_subband_idx);
extern oal_void hi1102_set_tpc_params(hal_to_dmac_device_stru *pst_hal_device,
                                      oal_uint8 uc_band,
                                      oal_uint8 uc_channel_num);
#endif
...
extern oal_void hi1102_add_machw_ba_lut_entry(hal_to_dmac_device_stru *pst_hal_device,
                                              oal_uint8 uc_lut_index,
                                              oal_uint8 *puc_dst_addr,
                                              oal_uint8 uc_tid,
                                              oal_uint16 uc_seq_no,
                                              oal_uint8 uc_win_size);
...
extern oal_void hi1102_restore_machw_ba_params(hal_to_dmac_device_stru *pst_hal_device, oal_uint8 uc_index,
                                               ...
                                               oal_uint32 ul_addr_l,
                                               oal_uint32 ul_ba_para);
extern oal_void hi1102_machw_seq_num_index_update_per_tid(hal_to_dmac_device_stru *pst_hal_device,
                                                          oal_uint8 uc_lut_index,
                                                          oal_uint8 uc_qos_flag);
extern oal_void hi1102_set_tx_sequence_num(hal_to_dmac_device_stru *pst_hal_device,
                                           oal_uint8 uc_lut_index,
                                           oal_uint8 uc_tid,
                                           oal_uint8 uc_qos_flag,
                                           oal_uint32 ul_val_write);
extern oal_void hi1102_get_tx_sequence_num(hal_to_dmac_device_stru *pst_hal_device,
                                           oal_uint8 uc_lut_index,
                                           oal_uint8 uc_tid,
                                           oal_uint8 uc_qos_flag,
                                           oal_uint32 *pst_val_read);
...
extern oal_void hi1102_reset_reg_dma_save(hal_to_dmac_device_stru *pst_hal,
                                          oal_uint8 *uc_dmach0,
                                          oal_uint8 *uc_dmach1,
                                          oal_uint8 *uc_dmach2);
extern oal_void hi1102_reset_reg_dma_restore(hal_to_dmac_device_stru *pst_hal,
                                             oal_uint8 *uc_dmach0,
                                             oal_uint8 *uc_dmach1,
                                             oal_uint8 *uc_dmach2);
...
extern oal_void hi1102_get_vap(hal_to_dmac_device_stru *pst_hal_device,
                               wlan_vap_mode_enum_uint8 vap_mode,
                               oal_uint8 vap_id,
                               hal_to_dmac_vap_stru **ppst_hal_vap);
extern oal_void hi1102_add_vap(hal_to_dmac_device_stru *pst_hal_device,
                               wlan_vap_mode_enum_uint8 vap_mode,
                               oal_uint8 uc_mac_vap_id,
                               hal_to_dmac_vap_stru **ppst_hal_vap);
extern oal_void hi1102_del_vap(hal_to_dmac_device_stru *pst_hal_device,
                               wlan_vap_mode_enum_uint8 vap_mode,
                               oal_uint8 vap_id);

#ifdef _PRE_WLAN_RF_110X_CALI_DPD
extern oal_void hi1102_dpd_cali_func(oal_uint8 u_functype,
                                     oal_uint32 *pul_status,
                                     oal_uint32 *hi1102_dpd_cal_data_read,
                                     oal_uint32 *hi1102_dpd_cal_data_write);
extern oal_void hi1102_dpd_cali_func_1131(oal_uint8 u_functype,
                                          oal_uint32 *pul_status,
                                          oal_uint32 *hi1102_dpd_cal_data_read,
                                          oal_uint32 *hi1102_dpd_cal_data_write,
                                          oal_uint8 uc_ch_idx,
...
#ifdef _PRE_WLAN_FEATURE_PROXYSTA
extern oal_void hi1102_set_proxysta_vap_addr(hal_to_dmac_device_stru *pst_hal_device,
                                             oal_uint8 vap_id,
                                             oal_uint8 *puc_mac_addr);
#endif
...
extern oal_void hi1102_get_ampdu_bytes(hal_to_dmac_device_stru *pst_hal_device,
                                       oal_uint32 *pul_tx_bytes,
                                       oal_uint32 *pul_rx_bytes);
...
extern oal_void hi1102_ce_add_key(hal_to_dmac_device_stru *pst_hal_device,
                                  hal_security_key_stru *pst_security_key,
                                  oal_uint8 *puc_addr);
...
extern oal_void hi1102_ce_add_peer_macaddr(hal_to_dmac_device_stru *pst_hal_device,
                                           oal_uint8 uc_lut_idx,
                                           oal_uint8 *puc_addr);
...
extern oal_void hi1102_tx_get_ra_lut_index(hal_to_dmac_device_stru *pst_hal_device,
                                           hal_tx_dscr_stru *pst_dscr,
                                           oal_uint8 *puc_ra_lut_index);
...
extern oal_void hi1102_get_bcn_rate(hal_to_dmac_device_stru *pst_hal_device,
                                    oal_uint8 uc_vap_id,
                                    oal_uint8 *puc_data_rate);
extern oal_void  hi1102_get_spec_frm_rate(hal_to_dmac_device_stru *pst_hal_device);
extern oal_void hi1102_set_bcn_phy_tx_mode(hal_to_dmac_device_stru *pst_hal_device,
                                           oal_uint8 uc_tpc_code, oal_uint8 uc_vap_id);
extern oal_void hi1102_set_spec_frm_phy_tx_mode(hal_to_dmac_device_stru *pst_hal_device,
...
extern oal_void hi1102_get_subband_index(wlan_channel_band_enum_uint8 en_band,
                                         oal_uint8 uc_channel_num,
                                         oal_uint8 *puc_subband_idx);
...
extern oal_void hi1102_set_txbf_lut_info(hal_to_dmac_device_stru *pst_hal_device,
                                         oal_uint8 uc_lut_index,
                                         oal_uint16 ul_reg_value);
extern oal_void hi1102_get_txbf_lut_info(hal_to_dmac_device_stru *pst_hal_device,
                                         oal_uint8 uc_lut_index,
                                         oal_uint32 *pst_reg_value);
extern oal_void hi1102_set_h_matrix_timeout(hal_to_dmac_device_stru *pst_hal_device, oal_uint32 ul_reg_value);
extern oal_void hi1102_set_peer_lut_info(hal_to_dmac_device_stru *pst_hal_device,
                                         oal_uint8 *puc_mac_addr,
                                         oal_uint8 uc_lut_index);
extern oal_void hi1102_set_dl_mumimo_ctrl(hal_to_dmac_device_stru *pst_hal_device, oal_uint32 value);
extern oal_void hi1102_set_mu_aid_matrix_info(hal_to_dmac_device_stru *pst_hal_device,
                                              oal_uint8 uc_vap_id,
                                              oal_uint16 us_aid,
                                              oal_uint8 *p_matrix);
...
extern oal_void hi1102_set_agc_unlock_min_th(hal_to_dmac_device_stru *pst_hal_device,
                                             oal_int32 l_tx_reg_val,
                                             oal_int32 l_rx_reg_val);
extern oal_void hi1102_set_nav_max_duration(hal_to_dmac_device_stru *pst_hal_device,
                                            oal_uint16 us_bss_dur,
...
extern oal_void hi1102_set_ed_high_th(hal_to_dmac_device_stru *pst_hal_device,
                                      oal_int32 l_ed_high_20_reg_val,
                                      oal_int32 l_ed_high_40_reg_val);
...
extern oal_void hi1102_set_wow_en(hal_to_dmac_device_stru *pst_hal_device,
                                  oal_uint32 ul_set_bitmap,
                                  hal_wow_param_stru* pst_para);
...
extern oal_void hi1102_set_coex_ctrl(hal_to_dmac_device_stru *pst_hal_device,
                                     oal_uint32 ul_mac_ctrl,
                                     oal_uint32 ul_rf_ctrl);
extern oal_void hi1102_get_hw_version(hal_to_dmac_device_stru *pst_hal_device,
                                      oal_uint32 *pul_hw_vsn,
                                      oal_uint32 *pul_hw_vsn_data,
                                      oal_uint32 *pul_hw_vsn_num);
...
#endif
extern oal_void hi1102_set_tx_dscr_field(hal_to_dmac_device_stru *pst_hal_device,
                                         oal_uint32 ul_data,
                                         hal_rf_test_sect_enum_uint8 en_sect);
...
#endif
extern oal_void hi1102_set_daq_mac_reg(hal_to_dmac_device_stru *pst_hal_device,
                                       oal_uint32 *pul_addr,
                                       oal_uint16 us_unit_len,
                                       oal_uint16 us_unit_num,
                                       oal_uint16 us_depth);
...
extern oal_void hi1102_set_dac_lpf_gain(hal_to_dmac_device_stru *pst_hal_device,
                                        ...
                                        oal_uint8 en_protocol_mode,
                                        oal_uint8 en_rate);
extern oal_void hi1102_get_pwr_comp_val(hal_to_dmac_device_stru *pst_hal_device,
...
extern oal_void hi1102_vap_tsf_get_64bit(hal_to_dmac_vap_stru *pst_hal_vap,
                                         oal_uint32 *pul_tsf_hi,
                                         oal_uint32 *pul_tsf_lo);
...
extern oal_void hi1102_vap_set_edca_machw_cw(hal_to_dmac_vap_stru *pst_hal_vap,
                                             oal_uint8 uc_cwmax,
                                             oal_uint8 uc_cwmin,
                                             oal_uint8 uc_ac_type);
extern oal_void hi1102_vap_set_edca_machw_cw_wfa(hal_to_dmac_vap_stru *pst_hal_vap,
                                                 oal_uint8 uc_cwmaxmin,
                                                 oal_uint8 uc_ac_type,
                                                 wlan_wme_ac_type_enum_uint8 en_wfa_lock);
extern oal_void hi1102_vap_get_edca_machw_cw(hal_to_dmac_vap_stru *pst_hal_vap,
                                             oal_uint8 *puc_cwmax,
                                             oal_uint8 *puc_cwmin,
                                             oal_uint8 uc_ac_type);
extern oal_void hi1102_vap_set_machw_txop_limit_bkbe(hal_to_dmac_vap_stru *pst_hal_vap,
                                                     oal_uint16 us_be,
                                                     oal_uint16 us_bk);
extern oal_void hi1102_vap_get_machw_txop_limit_bkbe(hal_to_dmac_vap_stru *pst_hal_vap,
                                                     oal_uint16 *pus_be,
                                                     oal_uint16 *pus_bk);
extern oal_void hi1102_vap_set_machw_txop_limit_vivo(hal_to_dmac_vap_stru *pst_hal_vap,
                                                     oal_uint16 us_vo,
                                                     oal_uint16 us_vi);
extern oal_void hi1102_vap_get_machw_txop_limit_vivo(hal_to_dmac_vap_stru *pst_hal_vap,
                                                     oal_uint16 *pus_vo,
                                                     oal_uint16 *pus_vi);
extern oal_void hi1102_vap_set_machw_edca_bkbe_lifetime(hal_to_dmac_vap_stru *pst_hal_vap,
                                                        oal_uint16 us_be,
                                                        oal_uint16 us_bk);
extern oal_void hi1102_vap_get_machw_edca_bkbe_lifetime(hal_to_dmac_vap_stru *pst_hal_vap,
                                                        oal_uint16 *pus_be,
                                                        oal_uint16 *pus_bk);
extern oal_void hi1102_vap_set_machw_edca_vivo_lifetime(hal_to_dmac_vap_stru *pst_hal_vap,
                                                        oal_uint16 us_vo,
                                                        oal_uint16 us_vi);
extern oal_void hi1102_vap_get_machw_edca_vivo_lifetime(hal_to_dmac_vap_stru *pst_hal_vap,
                                                        oal_uint16 *pus_vo,
                                                        oal_uint16 *pus_vi);
...
#ifdef _PRE_WLAN_FEATURE_P2P
extern oal_void hi1102_vap_set_ops(hal_to_dmac_vap_stru *pst_hal_vap, oal_uint8 en_ops_ctrl, oal_uint8 uc_ct_window);
...
extern oal_void hi1102_set_adc_sample_rate_80m(hal_to_dmac_device_stru *pst_hal_device);
extern oal_void hi1102_set_psm_dtim_count(hal_to_dmac_vap_stru *pst_hal_vap, oal_uint8 uc_dtim_count);
extern oal_void hi1102_set_psm_dtim_period(hal_to_dmac_vap_stru *pst_hal_vap,
                                           oal_uint8 uc_dtim_period,
                                           oal_uint8 uc_listen_intvl_to_dtim_times,
                                           oal_bool_enum_uint8 en_receive_dtim);
...
#if defined(_PRE_WLAN_FEATURE_BTCOEX) || defined(_PRE_WLAN_FEATURE_1131K_BTCOEX)
extern void hal_set_btcoex_channel_switch_on(void);
extern void hi1102_get_btcoex_abort_qos_null_seq_num(uint32_t *qosnull_seq_num);
extern void hi1102_get_btcoex_pa_status(uint32_t *pa_status);
extern uint32_t hi1102_btcoex_init(void *arg);
extern void hi1102_get_btcoex_statistic(oal_bool_enum_uint8 enable_abort_stat);
extern uint32_t hi1102_mpw_soc_write_reg(uint32_t ulQuryRegAddrTemp, uint16_t usQuryRegValueTemp);
extern void hi1102_btcoex_update_ap_beacon_count(uint32_t *beacon_count);
extern void hi1102_btcoex_have_small_ampdu(hal_to_dmac_device_stru *hal_base_device, uint32_t *have_ampdu);
#ifdef _PRE_WLAN_FEATURE_LTECOEX
extern void  hi1102_ltecoex_req_mask_ctrl(uint16_t req_mask_ctrl);
#endif
extern void hi1102_set_btcoex_abort_null_buff_addr(uint32_t abort_null_buff_addr);
extern void hi1102_set_btcoex_abort_qos_null_seq_num(uint32_t qosnull_seq_num);
extern void hi1102_set_btcoex_hw_rx_priority_dis(uint8_t hw_rx_prio_dis);
extern void hi1102_set_btcoex_hw_priority_en(uint8_t hw_prio_en);
extern void hi1102_set_btcoex_sw_all_abort_ctrl(uint8_t sw_abort_ctrl);
extern void hi1102_set_btcoex_sw_priority_flag(uint8_t sw_prio_flag);
extern void hi1102_btcoex_get_rf_control(uint16_t occupied_period,
                                         uint32_t *wlbt_mode_sel,
                                         uint16_t wait_cnt);
extern oal_bool_enum_uint8 hal_btcoex_check_sw_preempt_on(void);
extern oal_bool_enum_uint8 hal_btcoex_check_sw_preempt_reply_cts_on(void);
extern oal_bool_enum_uint8 get_hal_btcoex_sw_preempt_ps_stop(void);
extern oal_bool_enum_uint8 get_hal_btcoex_sw_preempt_ps_pause(void);
extern uint8_t get_hal_btcoex_sw_preempt_type(void);
extern uint8_t get_hal_btcoex_sw_preempt_subtype(void);
extern hal_device_btcoex_stru* hal_get_device_btcoex(void);
extern void hi1131K_btcoex_uart_process(void);
extern void hi1131k_btcoex_wifi_status_uart_send(uint8_t uart_type, uint8_t burst_num);
extern void hi1131k_btcoex_btble_status_uart_request(hal_to_dmac_device_stru *h2d_device);
extern void hal_btcoex_wlan_priority_set(uint8_t value, uint32_t timeout_ms);
extern void hi1131K_btcoex_get_ps_service_status(hal_to_dmac_device_stru *hal_device,
                                                 hal_btcoex_ps_status_enum_uint8 *ps_status);
extern void hi1131K_btcoex_assoc_gpio_send(void);
void hal_btcoex_set_wl_active_enable(uint32_t timeout_ms);
void hal_btcoex_set_wl_active_disable(void);
void hal_btcoex_enable_machw_tx(hal_to_dmac_device_stru *hal_device);
void hal_btcoex_disable_machw_tx(hal_to_dmac_device_stru *hal_device);
void hi1102_btcoex_cfg_tx_abort_auto_resp_en(uint8_t auto_resp_en);
#endif

extern oal_void hi1102_tx_get_dscr_iv_word(hal_tx_dscr_stru *pst_dscr,
                                           oal_uint32 *pul_iv_ms_word,
                                           oal_uint32 *pul_iv_ls_word,
                                           oal_uint8 uc_chiper_type,
                                           oal_uint8 uc_chiper_keyid);
...
extern oal_void hi1102_dft_get_machw_stat_info(hal_to_dmac_device_stru *pst_hal_device,
                                               oal_uint32 *pst_machw_stat,
                                               oal_uint8 us_bank_select,
                                               oal_uint32 *pul_len);
...
extern oal_void hi1102_dft_get_phyhw_stat_info(hal_to_dmac_device_stru *pst_hal_device,
                                               oal_uint32 *pst_phyhw_stat,
                                               oal_uint8 us_bank_select,
                                               oal_uint32 *pul_len);
extern oal_void hi1102_dft_get_rfhw_stat_info(hal_to_dmac_device_stru *pst_hal_device,
                                              oal_uint32 *pst_rfhw_stat,
                                              oal_uint32 *pul_len);
extern oal_void hi1102_dft_get_sochw_stat_info(hal_to_dmac_device_stru *pst_hal_device,
                                               oal_uint16 *pst_sochw_stat,
                                               oal_uint32 *pul_len);
