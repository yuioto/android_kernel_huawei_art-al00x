...
/* DEV/VAP/USER 操作封装 */
#define MAC_GET_DEVICE_STRU(_ul_device_num)   \
    mac_res_get_dev(_ul_device_num)
#define MAC_GET_VAP_STRU(_ul_vap_num)       \
    mac_res_get_mac_vap(_ul_vap_num)
#define MAC_FOR_EACH_DEV(_ul_dev_num)   \
    for (_ul_dev_num = 0; _ul_dev_num < MAC_RES_MAX_DEV_NUM; _ul_dev_num++)
#define MAC_IS_DEV_INIT_DONE(_pst_device)     \
    ((_pst_device)->en_device_state)
...
#define MAC_TCP_CONNECT_NUM  4   /* 保活TCP连接条数 */
#define MAC_UDP_CONNECT_NUM  4   /* 保活UDP连接条数 */
#endif
#endif
...
typedef enum
{
    MAC_SUB_LDPC_EVENT,     /*设置ldpc能力*/
    MAC_SUB_HIGN_POWER_SWITCH_EVENT, /* 设置高功耗开关 */
#ifdef _PRE_WLAN_FEATURE_IP_FILTER
    MAC_SUB_ASSIGNED_FILTER = 3, /* ICMP包过滤 */
#endif
#ifdef _PRE_WLAN_FEATURE_1131K_BTCOEX
    MAC_SUB_BTCOEX_CHANNEL_SWITCH_EVENT = 4,
#endif
    MAC_SUB_WLAN_CFGID_RX_FILTER_FRAG = 5,
    MAC_SUB_WLAN_CFGID_CHR_GET_CHIP_INFO = 6,
    MAC_SUB_WLAN_CFGID_CHR_GET_BEACON_INFO = 7,
    MAC_SUB_BUTT
}mac_sub_event_enum;
typedef oal_uint8 mac_sub_event_enum_uint8;

typedef enum
{
    MAC_APF_SET_FILTER_CMD,
    MAC_APF_GET_FILTER_CMD,
    MAC_APF_ENABLE_FILTER_CMD,
    MAC_APF_DISABLE_FILTER_CMD,
    MAC_IP_FILTER_CMD,   /*设置IP过滤相关参数*/
    MAC_SUB_TYPE_BUTT
} mac_sub_event_type_enum;
...
*****************************************************************************/
/*
 * WLAN_SUB_EVENT_MAX_LEN + sizeof(hmac_to_dmac_cfg_msg_stru)- 4 + OAL_MEM_INFO_SIZE + FRW_EVENT_HDR_LEN + OAL_DOG_TAG_SIZE = TOTAL_WLAN_MEM_EVENT_SIZE2
 * 8 = sizeof(hmac_to_dmac_cfg_msg_stru)
 */

#define WLAN_SUB_EVENT_MAX_LEN (WLAN_MEM_EVENT_SIZE2 - OAL_DOG_TAG_SIZE - FRW_EVENT_HDR_LEN - OAL_MEM_INFO_SIZE + 4 - 8)

/* ROM代码事件下发结构体 */
typedef struct {
    mac_sub_event_enum_uint8    en_sub_type;    /* 复用子事件类型 */
    oal_uint8                   auc_recv[1];    /* 保留位 */
    oal_uint16                  us_buf_len;     /* 定制化request帧 buf长度 */
    oal_uint8                   auc_buf[WLAN_SUB_EVENT_MAX_LEN];    /* 定制化request帧 buf信息 */
}sub_type_load_stru;
...
typedef struct
{
   ...
   // oal_bool_enum_uint8                  en_bw_change;             /* STA是否需要进行带宽切换 */
    ...
    oal_uint8                            uc_ch_swt_cnt;                  /* ap上一次发送的切换个数 */
...
typedef struct
{
    oal_uint32                          uc_data_blk_cnt;    /* 需要传输的数据块个数 */
    oal_uint32                          ul_wakeup_reason;
}mac_d2h_syn_info_hdr_stru;
...
typedef oal_uint8 mac_wow_msg_enum_uint8;

typedef struct
{
    mac_wow_msg_enum_uint8              en_msg_type;        /* 传输的消息类型 */
    oal_uint8                           uc_notify_param;    /* 睡眠通知参数 */
    oal_uint8                           auc_resv[2];
    oal_uint32                          uc_data_blk_cnt;    /* 需要传输的数据块个数 */
...
typedef struct
{
    ...
    /* 打包RSNE是否删除group management chiper suite */
    oal_uint8                           bit_del_group_mgmt_cs   :   1;
...
typedef struct
{
    ...
    uint8_t                  bit_del_group_mgmt_cs : 1;    /* 是否RSNE不携带group mgmt chiper suite参数 */
    uint8_t                  bit_resv              : 7;
    uint8_t                  auc_resv1;
...
typedef struct
{
    ...
    oal_uint32      ul_src_ipaddr;
    oal_uint16      us_src_port;
    oal_uint16      us_des_port;
    oal_uint32      ul_des_ipaddr;
}wow_pattern_stru;
...
typedef struct
{
    oal_uint8                     uc_ssid_set_flag;
    oal_int8                      ac_ssid[WLAN_SSID_MAX_LEN]; /*32+1*/
    oal_int8                      c_rssi;
    oal_uint8                     auc_res[1];
}wow_ssid_cfg_stru;
...
{
    oal_uint8                   uc_wow_en;
    oal_uint8                   auc_res[3];//复用为pno配置auc_res[0]为扫描信道,auc_res[1]和auc_res[2]为pno周期
    oal_uint32                  ul_wow_event;
    wow_pattern_cfg_stru        st_wow_pattern;
    wow_ssid_cfg_stru           st_wow_ssid_cfg;
    oal_uint8                   uc_wow_complete_flag;
    oal_uint8                   auc_res1[3]; //复用为是否使能14信道，DTS:使能14后sleep pno无法扫描14，res[0]配置标志，res1[1]使能标志
}mac_wow_cfg_stu;
...
typedef struct
{
    oal_uint32       ul_dbb_scale[9];       /* DBB幅值 */
}dbb_scaling_stru;
...
/* 1131 wiphy Vendor CMD参数 对应cfgid: WLAN_CFGID_VENDOR_CMD */
...
OAL_STATIC OAL_INLINE oal_void mac_mib_set_DeauthenticateStation(mac_vap_stru *pst_mac_vap, const oal_uint8 *puc_da)
{
    int32_t ret;
    ret = memcpy_s(pst_mac_vap->pst_mib_info->st_wlan_mib_sta_config.auc_dot11DeauthenticateStation, 6, puc_da, 6);
    if (ret != EOK) {
        OAM_WARNING_LOG1(0, 0, "mac_mib_set_DeauthenticateStation memcpy_s faile", ret);
    }
}


OAL_STATIC OAL_INLINE oal_void  mac_mib_set_AuthenticateFailStation(mac_vap_stru *pst_mac_vap, const oal_uint8 *puc_da, oal_uint8 uc_da_len)
{
    int32_t ret;
    ret = memcpy_s(pst_mac_vap->pst_mib_info->st_wlan_mib_sta_config.auc_dot11AuthenticateFailStation, 6, puc_da, 6);
    if (ret != EOK) {
        OAM_WARNING_LOG1(0, 0, "mac_mib_set_AuthenticateFailStation memcpy_s faile", ret);
    }
}
...
OAL_STATIC OAL_INLINE oal_void  mac_mib_set_DisassocStation(mac_vap_stru *pst_mac_vap, const oal_uint8 *puc_da)
{
    int32_t ret;
    ret = memcpy_s(pst_mac_vap->pst_mib_info->st_wlan_mib_sta_config.auc_dot11DisassociateStation, 6, puc_da, 6);
    if (ret != EOK) {
        OAM_WARNING_LOG1(0, 0, "mac_mib_set_DisassocStation memcpy_s faile", ret);
    }
}
#endif
...
OAL_STATIC OAL_INLINE oal_void  mac_mib_set_StationID(mac_vap_stru *pst_mac_vap, const oal_uint8 *puc_sta_id)
{
...
{
    return (oal_bool_enum_uint8)((WLAN_BAND_2G == pst_mac_vap->st_channel.en_band) ?
...
{
    return mac_vap->pst_mib_info->st_wlan_mib_sta_config.ul_dot11AuthenticationResponseTimeOut;
}
...
extern oal_uint32  mac_vap_find_user_by_macaddr(
                mac_vap_stru        *pst_vap,
                const oal_uint8     *puc_sta_mac_addr,
...
extern uint8_t mac_mib_get_rsn_akm_suites(mac_vap_stru *mac_vap, uint8_t *suites);
#endif
#ifdef __cplusplus
    #if __cplusplus
        }
