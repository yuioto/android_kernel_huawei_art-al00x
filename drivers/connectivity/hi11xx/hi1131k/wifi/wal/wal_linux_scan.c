...
OAL_STATIC oal_void wal_inform_bss_frame(wal_scanned_bss_info_stru *pst_scanned_bss_info, oal_void *p_data)
{
    ...
    if ((pst_scanned_bss_info == OAL_PTR_NULL) || (p_data == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_SCAN, "{wal_inform_bss_frame::input param pointer is null}");
        return;
    }
...
/* 由于驱动缓存扫描结果，导致cts认证2次扫描的bss的timestamp时间一致(后一次没有扫描到) */
    ...
    pst_scanned_bss_info->pst_mgmt->u.probe_resp.timestamp = ((u64)ts.tv_sec * 1000000) + ts.tv_nsec / 1000;
#else
    do_gettimeofday(&tv);
    pst_scanned_bss_info->pst_mgmt->u.probe_resp.timestamp = ((u64)tv.tv_sec * 1000000) + tv.tv_usec;
#endif
#endif

    /* 逐个上报内核bss 信息 */
    pst_cfg80211_bss = oal_cfg80211_inform_bss_frame(pst_wiphy, pst_ieee80211_channel, pst_scanned_bss_info->pst_mgmt,
        pst_scanned_bss_info->ul_mgmt_len, pst_scanned_bss_info->l_signal, GFP_ATOMIC);
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
OAL_STATIC oal_bool_enum_uint8 wal_if_need_update_bss(oal_wiphy_stru *wiphy,
                                                      mac_bss_dscr_stru *bss_dscr,
                                                      uint8_t *bssid)
{
    oal_cfg80211_bss_stru     *cfg80211_bss = NULL;
    int32_t                   channel;
    oal_bool_enum_uint8       is_inform_bss = OAL_FALSE;

    /* 查找是否在内核中，如果没有该bss，则添加bss，否则更新bss 时间 */
    cfg80211_bss = oal_cfg80211_get_bss(wiphy, NULL, bssid,
                                        (uint8_t *)(bss_dscr->ac_ssid), OAL_STRLEN(bss_dscr->ac_ssid));
    if (cfg80211_bss != NULL) {
        ...
        if (bss_dscr->st_channel.uc_chan_number != (uint8_t)channel) {
            OAM_WARNING_LOG2(0, OAM_SF_ASSOC,
                             "{wal_if_need_update_bss::current kernel bss channel[%d] need to update to channel[%d].",
                             (uint8_t)channel, bss_dscr->st_channel.uc_chan_number);

            oal_cfg80211_unlink_bss(wiphy, cfg80211_bss);
            is_inform_bss = OAL_TRUE;
        } else {
            oal_cfg80211_put_bss(wiphy, cfg80211_bss);
        }
    } else {
        OAM_WARNING_LOG0(0, OAM_SF_ASSOC, "{wal_if_need_update_bss::not find bss in kernel.");
        is_inform_bss = OAL_TRUE;
    }
    OAM_WARNING_LOG1(0, OAM_SF_ASSOC, "{wal_if_need_update_bss::is_inform_bss[%d].", is_inform_bss);
    return is_inform_bss;
}


void wal_update_bss(oal_wiphy_stru *wiphy, hmac_bss_mgmt_stru *bss_mgmt, uint8_t *bssid)
{
    wal_scanned_bss_info_stru scanned_bss_info = {0};
    ...
    if (OAL_ANY_NULL_PTR3(wiphy, bss_mgmt, bssid)) {
        OAM_WARNING_LOG0(0, OAM_SF_ASSOC, "{wal_update_bss_etc::null pointer}");
        return;
    }
    ...
    if (bss_dscr == NULL) {
        OAM_WARNING_LOG4(0, OAM_SF_ASSOC, "{wal_update_bss::do not find correspond bss from scan result."
                         "%02X:XX:XX:%02X:%02X:%02X}", bssid[0], bssid[3], bssid[4], bssid[5]);
        return;
    }

    /* 需要inform bss到内核 */
    if (wal_if_need_update_bss(wiphy, bss_dscr, bssid)) {
        chan_number = bss_dscr->st_channel.uc_chan_number;
        band = (enum ieee80211_band)bss_dscr->st_channel.en_band;
        ...
        scanned_bss_info.ul_mgmt_len = bss_dscr->ul_mgmt_len;
        /* 获取上报的扫描结果的管理帧的帧头 */
        ...
        OAM_WARNING_LOG3(0, OAM_SF_ASSOC, "{wal_update_bss::update %02X:XX:XX:XX:%02X:%02X info}",
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_bss_mgmt->st_bss_list_head))
    {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG2(uc_vap_id, OAM_SF_SCAN,
                "{wal_inform_all_bss::curr channel[%d] and band[%d] is not in regdomain.}", uc_chan_number, en_band);
...

{
    ...
    if (pst_request == OAL_PTR_NULL || pst_scan_param == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY,
            "{wal_get_scan_channel_num::set scan channel from scan request failed, param null}");
    ...
    if ((l_ssid_num > 0) && (pst_request->ssids != OAL_PTR_NULL)) {
        ...
        for (l_loop = 0; l_loop < l_ssid_num; l_loop++) {
            ...
            if (memcpy_s(pst_scan_param->st_ssids[l_loop].auc_ssid, OAL_IEEE80211_MAX_SSID_LEN,
                ...
                OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_set_scan_ssid::memcpy_s failed!");
                return;
            }
        }
    }
...
oal_uint32 wal_scan_work_func(hmac_scan_stru *pst_scan_mgmt, oal_net_device_stru *pst_netdev,
    oal_cfg80211_scan_request_stru *pst_request)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(0, OAM_SF_CFG, "{wal_scan_work_func::wal_cfg80211_start_scan proc failed, err_code[%d]!}",
            ul_ret);
        /* 正常情况下，在hmac释放 */
...
oal_int32 wal_send_scan_abort_msg(oal_net_device_stru *pst_net_dev)
{
    ...
    if (memcpy_s(st_write_msg.auc_value, WAL_MSG_WRITE_MAX_LEN, (oal_int8 *)&ul_pedding_data,
        OAL_SIZEOF(ul_pedding_data)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_send_scan_abort_msg::memcpy_s failed!");
        return OAL_FAIL;
    }
...
oal_int32 wal_force_scan_complete(oal_net_device_stru *pst_net_dev, oal_bool_enum en_is_aborted)
{
    ...
    pst_hmac_device = hmac_res_get_mac_dev(pst_mac_vap->uc_device_id);
    if (pst_hmac_device == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN, "{wal_force_scan_complete::pst_hmac_device[%d] is null!}",
            pst_mac_vap->uc_device_id);
        return -OAL_EINVAL;
    }
    ...
    if ((pst_scan_mgmt->pst_request != OAL_PTR_NULL) &&
        ...
        wal_inform_all_bss(pst_hmac_device->pst_device_base_info->pst_wiphy,
            &(pst_hmac_device->st_scan_mgmt.st_scan_record_mgmt.st_bss_mgmt), pst_mac_vap->uc_vap_id);
        ...
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
            "{wal_force_scan_complete::vap_id[%d] notify kernel scan abort!}", pst_mac_vap->uc_vap_id);
...
oal_int32 wal_stop_sched_scan(oal_net_device_stru *pst_netdev)
{
    ...
    OAM_WARNING_LOG2(0, OAM_SF_SCAN, "{wal_stop_sched_scan::sched scan req[%p],sched scan complete[%d]}",
        (uintptr_t)(pst_scan_mgmt->pst_sched_scan_req), pst_scan_mgmt->en_sched_scan_complete);

    if ((pst_scan_mgmt->pst_sched_scan_req != OAL_PTR_NULL) && (pst_scan_mgmt->en_sched_scan_complete != OAL_TRUE)) {
        ...
        if (memcpy_s(st_write_msg.auc_value, WAL_MSG_WRITE_MAX_LEN, (oal_int8 *)&ul_pedding_data,
            OAL_SIZEOF(ul_pedding_data)) != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "wal_stop_sched_scan::memcpy_s failed!");
            return OAL_FAIL;
        }
