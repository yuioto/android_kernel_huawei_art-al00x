...
typedef enum {
    CMD_STR_AUTH_RSP_TIME    = 0, /* AP模式配置auth rsp超时时间 */
    CMD_STR_FORBIT_OPEN_AUTH = 1, /* 拒绝open模式关联 */
    CMD_STR_BUTT
} cmd_str_config_enum;
...
/* rx ip数据包过滤功能和上层协定(格式)结构体，上层接口尚未明确，需澄清后修改 */
#ifdef CONFIG_DOZE_FILTER
...
typedef struct {
    ...
    oal_uint8 auc_resv[BYTE_ALIGNED_3]; /* 字节对齐 */
