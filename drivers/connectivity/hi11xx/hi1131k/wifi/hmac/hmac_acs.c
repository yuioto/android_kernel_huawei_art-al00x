                                                           ...
                                                           hmac_scan_record_stru            *pst_scan_record)
{
    ...
    for (ul_idx = 0; ul_idx < ul_chan_numbers; ul_idx++) {
        ...
        pst_simple_channel_stats[ul_idx].uc_bandwidth_mode =
            st_scan_record->ast_chan_results[ul_idx].uc_bandwidth_mode;
        ...
        pst_simple_channel_stats[ul_idx].uc_free_time_20M_rate =
            (oal_uint8)ACS_DIV(pst_scan_record->ast_chan_results[ul_idx].ul_total_free_time_20M_us << 8,
            pst_scan_record->ast_chan_results[ul_idx].ul_total_stats_time_us);
        pst_simple_channel_stats[ul_idx].uc_free_time_40M_rate =
            (oal_uint8)ACS_DIV(pst_scan_record->ast_chan_results[ul_idx].ul_total_free_time_40M_us << 8,
            pst_scan_record->ast_chan_results[ul_idx].ul_total_stats_time_us);
        pst_simple_channel_stats[ul_idx].uc_free_time_80M_rate =
            (oal_uint8)ACS_DIV(pst_scan_record->ast_chan_results[ul_idx].ul_total_free_time_80M_us << 8,
            pst_scan_record->ast_chan_results[ul_idx].ul_total_stats_time_us);
        pst_simple_channel_stats[ul_idx].uc_total_send_time_rate =
            (oal_uint8)ACS_DIV(pst_scan_record->ast_chan_results[ul_idx].ul_total_send_time_us << 8,
            pst_scan_record->ast_chan_results[ul_idx].ul_total_stats_time_us);
        pst_simple_channel_stats[ul_idx].uc_total_recv_time_rate =
            (oal_uint8)ACS_DIV(pst_scan_record->ast_chan_results[ul_idx].ul_total_recv_time_us << 8,
            pst_scan_record->ast_chan_results[ul_idx].ul_total_stats_time_us);
    }
                                               ...
                                               oal_bool_enum_uint8      en_need_rank)
{
    ...
    *(oal_uint32 *)(pst_msg_hdr + 1) = (oal_uint32)puc_result;
    ...
    /* 结果包首先是一个重复的response */
    if (memcpy_s(puc_result, ul_size, pst_msg_hdr, OAL_SIZEOF(mac_acs_response_hdr_stru)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ACS, "hmac_acs_setup_scan_result::memcpy_s failed!");
        FRW_EVENT_FREE(pst_event_mem);
...
oal_uint32  hmac_acs_got_init_rank(hmac_device_stru *pst_hmac_device, mac_vap_stru *pst_mac_vap,
    mac_acs_cmd_stru *pst_cmd)
{
    ...
    if (mac_get_channel_idx_from_num(pst_ch->en_band, pst_ch->uc_chan_number, &pst_ch->uc_idx) != OAL_SUCC) {
        OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_ACS,
            "{hmac_acs_got_init_rank::invalid 2G channel band=%d bw=%d ch=%d", pst_ch->en_band, pst_ch->en_bandwidth,
    ...
    if (mac_get_channel_idx_from_num(pst_ch->en_band, pst_ch->uc_chan_number, &pst_ch->uc_idx) != OAL_SUCC) {
        OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_ACS,
            "{hmac_acs_got_init_rank::invalid 5G channel band=%d bw=%d ch=%d", pst_ch->en_band, pst_ch->en_bandwidth,
            pst_ch->uc_chan_number);
        memset_s(pst_ch, OAL_SIZEOF(mac_channel_stru), 0, OAL_SIZEOF(mac_channel_stru));
    }
    ...
    if (pst_hmac_device->en_init_scan == OAL_TRUE) {
        ...
        if (mac_dfs_get_dfs_enable(pst_hmac_device->pst_device_base_info)) {
            if (hmac_dfs_init_scan_hook(&pst_hmac_device->st_scan_mgmt.st_scan_record_mgmt, pst_hmac_device) !=
                OAL_SUCC) {
...
oal_uint32  hmac_acs_rescan_handler(void *p_arg)
{
    ...
    if ((pst_dev == NULL) || (pst_dev->pst_device_base_info == NULL) ||
        (mac_get_acs_switch(pst_dev->pst_device_base_info) == MAC_ACS_SW_NONE) || pst_dev->en_init_scan) {
        return OAL_SUCC;
    }
...
oal_uint32  hmac_acs_exit(oal_void)
{
    ...
    for (ul_dev_id = 0; ul_dev_id < MAC_RES_MAX_DEV_NUM; ul_dev_id++) {
        pst_dev = hmac_res_get_mac_dev(ul_dev_id);
        if ((pst_dev != NULL) && pst_dev->st_rescan_timer.en_is_enabled) {
