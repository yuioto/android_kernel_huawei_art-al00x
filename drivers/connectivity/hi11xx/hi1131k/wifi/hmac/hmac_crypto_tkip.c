    ...
    oal_uint8 uc_hdr_size)
{
    ...
    switch (uc_frame_dir) {
        case IEEE80211_FC1_DIR_NODS:
            l_ret += memcpy_s(auc_hdr, uc_hdr_size, pst_frame_4addr_hdr->auc_address1, WLAN_MAC_ADDR_LEN);
            l_ret += memcpy_s(auc_hdr + WLAN_MAC_ADDR_LEN, uc_hdr_size - WLAN_MAC_ADDR_LEN,
                              pst_frame_4addr_hdr->auc_address2, WLAN_MAC_ADDR_LEN);
            break;
        case IEEE80211_FC1_DIR_TODS:
            l_ret += memcpy_s(auc_hdr, uc_hdr_size, pst_frame_4addr_hdr->auc_address3, WLAN_MAC_ADDR_LEN);
            l_ret += memcpy_s(auc_hdr + WLAN_MAC_ADDR_LEN, uc_hdr_size - WLAN_MAC_ADDR_LEN,
                              pst_frame_4addr_hdr->auc_address2, WLAN_MAC_ADDR_LEN);
            break;
    ...
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_crypto_tkip_michael_hdr::memcpy_s fail!");
        return;
    }
    auc_hdr[12] = 0;
...
OAL_STATIC oal_uint32 hmac_crypto_tkip_michael_mic(const oal_uint8 *puc_key,
                                                   ...
                                                   oal_uint8  uc_mic_len)
{
    ...
    ul_msb = get_le32(puc_key, WLAN_WPA_KEY_LEN - WLAN_TEMPORAL_KEY_LENGTH);
    ul_lsb = get_le32(puc_key + 4, WLAN_WPA_KEY_LEN - WLAN_TEMPORAL_KEY_LENGTH - 4);

    /* Michael MIC pseudo header: DA, SA, 3 x 0, Priority */
    ul_msb ^= get_le32(auc_hdr, sizeof(auc_hdr));
    michael_block(ul_msb, ul_lsb);
    ul_msb ^= get_le32(&auc_hdr[4], sizeof(auc_hdr));
    michael_block(ul_msb, ul_lsb);
    ul_msb ^= get_le32(&auc_hdr[8], sizeof(auc_hdr));
    michael_block(ul_msb, ul_lsb);
    ul_msb ^= get_le32(&auc_hdr[12], sizeof(auc_hdr));
    michael_block(ul_msb, ul_lsb);
    ...
    for (;;) {
        ...
        while (ul_space >= sizeof(oal_uint32)) {
            ul_msb ^= get_le32(puc_data, OAL_NETBUF_LEN(pst_netbuf));
            michael_block(ul_msb, ul_lsb);
...
oal_uint32 hmac_crypto_tkip_enmic(wlan_priv_key_param_stru *pst_key, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (memcpy_s(puc_mic_tail, ul_tailroom, auc_mic, IEEE80211_WEP_MICLEN) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_crypto_tkip_enmic::memcpy_s failed!");
        return OAL_FAIL;
    }
...
oal_uint32 hmac_crypto_tkip_demic(wlan_priv_key_param_stru *pst_key, oal_netbuf_stru *pst_netbuf)
{
    ...
    ul_ret = hmac_crypto_tkip_michael_mic(puc_rx_mic_key, pst_netbuf,
                                          (mac_ieee80211_frame_stru *)pst_cb->pul_mac_hdr_start_addr, ul_hdrlen,
                                          ul_pktlen - (ul_hdrlen + IEEE80211_WEP_MICLEN), auc_mic, sizeof(auc_mic));
    if (ul_ret != OAL_SUCC) {
        return ul_ret;
    }
