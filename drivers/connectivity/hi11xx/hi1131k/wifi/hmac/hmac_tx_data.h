...
extern oal_void hmac_tx_ba_setup(hmac_vap_stru *pst_vap, hmac_user_stru *pst_user, oal_uint8 uc_tidno);

extern oal_void hmac_tx_ba_cnt_vary(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user, oal_uint8 uc_tidno,
    oal_netbuf_stru *pst_buf);
...
OAL_STATIC OAL_INLINE oal_void hmac_tx_netbuf_list_enqueue(oal_netbuf_head_stru *pst_head, oal_netbuf_stru *pst_buf,
...
OAL_STATIC OAL_INLINE oal_uint32 hmac_tx_set_addresses(hmac_vap_stru                          *pst_vap,
                                                       hmac_user_stru                         *pst_user,
                                                       mac_tx_ctl_stru                        *pst_tx_ctl,
                                                       const oal_uint8                        *puc_saddr,
                                                       oal_uint8                              uc_saddr_len,
                                                       const oal_uint8                        *puc_daddr,
                                                       oal_uint8                              uc_daddr_len,
                                                                   ...
                                                                   oal_netbuf_stru *pst_buf)
{
    ...
    oal_spin_lock_bh(&(pst_hmac_tid_info->st_ba_tx_info.ba_status_lock));
    if ((pst_hmac_tid_info->st_ba_tx_info.en_ba_status == DMAC_BA_INIT) &&
        ...
        oal_spin_unlock_bh(&(pst_hmac_tid_info->st_ba_tx_info.ba_status_lock));
        return OAL_FALSE;
    }
    oal_spin_unlock_bh(&(pst_hmac_tid_info->st_ba_tx_info.ba_status_lock));
