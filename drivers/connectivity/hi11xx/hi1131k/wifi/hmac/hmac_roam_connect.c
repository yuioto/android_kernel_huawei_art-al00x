...
OAL_STATIC oal_uint32  hmac_roam_start_join(hmac_roam_info_stru *roam_info, oal_void *p_param);
...
oal_void hmac_roam_connect_fsm_init(oal_void)
{
    ...
    g_hmac_roam_connect_fsm_func[ROAM_CONNECT_STATE_WAIT_JOIN][ROAM_CONNECT_FSM_EVENT_MGMT_RX] =
        hmac_roam_process_beacon;
    g_hmac_roam_connect_fsm_func[ROAM_CONNECT_STATE_JOIN_COMP][ROAM_CONNECT_FSM_EVENT_TBTT] =
        hmac_roam_send_auth_seq1;
    g_hmac_roam_connect_fsm_func[ROAM_CONNECT_STATE_WAIT_JOIN][ROAM_CONNECT_FSM_EVENT_TIMEOUT] =
        hmac_roam_connect_fail;
    g_hmac_roam_connect_fsm_func[ROAM_CONNECT_STATE_JOIN_COMP][ROAM_CONNECT_FSM_EVENT_TIMEOUT] =
        hmac_roam_connect_fail;
    g_hmac_roam_connect_fsm_func[ROAM_CONNECT_STATE_WAIT_AUTH_COMP][ROAM_CONNECT_FSM_EVENT_MGMT_RX] =
        hmac_roam_process_auth_seq2;
    g_hmac_roam_connect_fsm_func[ROAM_CONNECT_STATE_WAIT_AUTH_COMP][ROAM_CONNECT_FSM_EVENT_MGMT_FAIL_RX] =
    ...
    g_hmac_roam_connect_fsm_func[ROAM_CONNECT_STATE_WAIT_ASSOC_COMP][ROAM_CONNECT_FSM_EVENT_MGMT_FAIL_RX] =
        hmac_roam_connect_terminate;
    ...
    g_hmac_roam_connect_fsm_func[ROAM_CONNECT_STATE_HANDSHAKING][ROAM_CONNECT_FSM_EVENT_TIMEOUT] =
        hmac_roam_connect_fail;
#ifdef _PRE_WLAN_FEATURE_11R
    g_hmac_roam_connect_fsm_func[ROAM_CONNECT_STATE_INIT][ROAM_CONNECT_FSM_EVENT_FT_OVER_DS] =
...
oal_uint32 hmac_roam_connect_fsm_action(hmac_roam_info_stru *pst_roam_info, roam_connect_fsm_event_type_enum en_event,
    oal_void *p_param)
{
...
{
    if (pst_roam_info) {
        OAM_WARNING_LOG2(0, OAM_SF_ROAM, "{hmac_roam_connect_change_state::[%d]->[%d]}",
...
oal_uint32 hmac_roam_connect_set_join_reg(mac_vap_stru *pst_mac_vap)
{
    ...
    dmac_ctx_join_req_set_reg_stru *pst_reg_params = NULL;
    ...
    /* 设置dtim period信息 */
    pst_reg_params->us_beacon_period = (uint16_t)pst_mac_vap->pst_mib_info->st_wlan_mib_sta_config.ul_dot11BeaconPeriod;
    ...
    oal_uint32 ul_dtim_period, oal_uint16 us_tsf_bit0)
{
    ...
    frw_event_stru     *pst_event = NULL;
    dmac_ctx_set_dtim_tsf_reg_stru *pst_set_dtim_tsf_reg_params = NULL;
    ...
    if (memcpy_s(pst_set_dtim_tsf_reg_params->auc_bssid, WLAN_MAC_ADDR_LEN, pst_mac_vap->auc_bssid,
        WLAN_MAC_ADDR_LEN) != EOK) {
        OAM_WARNING_LOG0(0, OAM_SF_ROAM, "{hmac_roam_connect_set_dtim_param::memcpt_s failed!}");
        FRW_EVENT_FREE(pst_event_mem);
        return OAL_FAIL;
    }
...
OAL_STATIC oal_uint32 hmac_roam_connect_set_join_params(hmac_vap_stru *pst_hmac_vap, mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    if ((pst_mac_device == OAL_PTR_NULL) || (pst_mib_info == OAL_PTR_NULL)) {
    ...
    if (memcpy_s(pst_mib_info->st_wlan_mib_sta_config.auc_dot11DesiredSSID, WLAN_SSID_MAX_LEN,
        pst_bss_dscr->ac_ssid, WLAN_SSID_MAX_LEN) != EOK) {
        OAM_WARNING_LOG0(0, OAM_SF_ROAM, "{hmac_roam_connect_set_join_params::memcpt_s failed!}");
        return OAL_FAIL;
    }
    ...
    ul_ret = mac_vap_set_current_channel(pst_mac_vap, pst_bss_dscr->st_channel.en_band,
    ...
    /* 更新LDPC、STBC能力，取最小能力 */
    pst_mib_info->st_phy_ht.en_dot11LDPCCodingOptionImplemented =
        (pst_bss_dscr->en_ht_ldpc && pst_mib_info->st_phy_ht.en_dot11LDPCCodingOptionActivated);
    pst_mib_info->st_phy_ht.en_dot11TxSTBCOptionImplemented =
        (pst_bss_dscr->en_ht_stbc && pst_mib_info->st_phy_ht.en_dot11TxSTBCOptionActivated);
    ...
    if (!(pst_mac_vap->st_channel.en_band == WLAN_BAND_2G && pst_mac_vap->st_cap_flag.bit_disable_2ght40)) {
        if (pst_bss_dscr->en_bw_cap == WLAN_BW_CAP_20M) {
    ...
    if (pst_mib_info->st_wlan_mib_sta_config.en_dot11DesiredBSSType == WLAN_MIB_DESIRED_BSSTYPE_INFRA) {
    ...
    /* 入网优化，不同频段下的能力不一样 */
    if (pst_vap_channel->en_band == WLAN_BAND_2G) {
    ...
    oal_uint16 us_msg_len)
{
    ...
    if (memset_s(&st_roam_rsp, OAL_SIZEOF(hmac_asoc_rsp_stru), 0, OAL_SIZEOF(hmac_asoc_rsp_stru)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ROAM, "{hmac_handle_asoc_rsp_sta::memset fail!}");
        return OAL_FAIL;
    }

    /* 获取AP的mac地址 */
    mac_get_address3_host(puc_mac_hdr, st_roam_rsp.auc_bssid, WLAN_MAC_ADDR_LEN);
    ...
    if (us_msg_len < OAL_ASSOC_RSP_IE_OFFSET) {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
            "{hmac_handle_asoc_rsp_sta::us_msg_len is too short, %d.}", us_msg_len);
        FRW_EVENT_FREE(pst_event_mem);
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }
    ...
    if (puc_mgmt_data == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
            "{hmac_handle_asoc_rsp_sta::pst_mgmt_data alloc null,size %d.}", (us_msg_len - OAL_ASSOC_RSP_IE_OFFSET));
        FRW_EVENT_FREE(pst_event_mem);
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }
    ...
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ROAM, "hmac_roam_connect_notify_wpas::memcpy_s fail!");
        FRW_EVENT_FREE(pst_event_mem);
    ...
    oal_uint16 us_msg_len)
{
    ...
    mac_get_address3_host(puc_mac_hdr, pst_ft_event->auc_bssid, WLAN_MAC_ADDR_LEN);
...
OAL_STATIC oal_uint32 hmac_roam_send_ft_req(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    oal_netbuf_stru *pst_ft_frame = NULL;
    oal_uint8 *puc_ft_buff = NULL;
    mac_tx_ctl_stru *pst_tx_ctl = NULL;
    oal_uint8 *puc_my_mac_addr = NULL;
    oal_uint8 *puc_current_bssid = NULL;
    ...
    if (pst_roam_info->en_main_state != ROAM_MAIN_STATE_CONNECTING) {
        OAM_ERROR_LOG1(uc_vap_id, OAM_SF_ROAM, "{hmac_roam_send_ft_req::main state[%d] error.}",
    ...
    if (pst_roam_info->st_connect.en_state != ROAM_CONNECT_STATE_INIT) {
        OAM_ERROR_LOG1(uc_vap_id, OAM_SF_ROAM, "{hmac_roam_send_ft_req::connect state[%d] error.}",
    ...
    memset_s(oal_netbuf_cb(pst_ft_frame), 48, 0, 48);
...
OAL_STATIC oal_uint32 hmac_roam_process_ft_rsp(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    hmac_rx_ctl_stru         *pst_rx_ctrl = NULL;
    oal_uint8                *puc_mac_hdr = NULL;
    oal_uint8                *puc_ft_frame_body = NULL;
    ...
    /* 漫游开关没有开时，不处理 */
    if (pst_roam_info->uc_enable == 0) {
        OAM_ERROR_LOG0(0, OAM_SF_ROAM, "{hmac_roam_process_ft_rsp::roam disabled.}");
        return OAL_ERR_CODE_ROAM_DISABLED;
    }
...
OAL_STATIC oal_uint32 hmac_roam_start_join(hmac_roam_info_stru *roam_info, oal_void *p_param)
{
    ...
    pst_hmac_vap = roam_info->pst_hmac_vap;
    if (pst_hmac_vap == OAL_PTR_NULL) {
    ...
    if (roam_info->en_main_state != ROAM_MAIN_STATE_CONNECTING) {
        OAM_ERROR_LOG1(0, OAM_SF_ROAM, "{hmac_roam_start_join::ROAM main status[%d] ERROR.}", roam_info->en_main_state);
        return OAL_ERR_CODE_ROAM_STATE_UNEXPECT;
    }

    /* CONNECT状态机为非INIT状态，失败 */
    if (roam_info->st_connect.en_state != ROAM_CONNECT_STATE_INIT) {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
                       "{hmac_roam_start_join::ROAM connect status[%d] ERROR.}", roam_info->st_connect.en_state);
        return OAL_ERR_CODE_ROAM_STATE_UNEXPECT;
    }
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(0, OAM_SF_SCAN, "{hmac_roam_start_join::hmac_roam_connect_set_join_params fail[%d].}", ul_ret);
        return ul_ret;
    }

#if defined(_PRE_WLAN_FEATURE_WPA)
    if (pst_hmac_vap->uc_80211i_mode == DMAC_WPA_802_11I) {
    ...
    if (pst_hmac_vap->uc_80211i_mode == DMAC_RSNA_802_11I) {
    ...
    /* 启动超时定时器 */
    hmac_roam_connect_start_timer(roam_info, ROAM_JOIN_TIME_MAX);
...
OAL_STATIC oal_uint32 hmac_roam_process_beacon(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    /* beacon帧校验 */
    if (mac_get_frame_sub_type(puc_mac_hdr) != WLAN_FC0_SUBTYPE_BEACON) {
        return OAL_SUCC;
    }

    /* bssid校验 */
    mac_get_bssid(puc_mac_hdr, auc_bssid);
    if (oal_memcmp(auc_bssid, pst_hmac_vap->st_vap_base_info.auc_bssid, WLAN_MAC_ADDR_LEN) != 0) {
        return OAL_SUCC;
    }
    OAM_WARNING_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
        "{hmac_roam_process_beacon:: [%02X:XX:XX:%02X:%02X:%02X]}", auc_bssid[0], auc_bssid[3], auc_bssid[4],
        auc_bssid[5]);
    ...
    if ((puc_tim_ie == OAL_PTR_NULL) || (puc_tim_ie[1] < MAC_MIN_TIM_LEN)) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_process_beacon::Do not find Tim ie.}");
        return OAL_SUCC;
    }
    ...
    hmac_roam_connect_set_dtim_param(&pst_hmac_vap->st_vap_base_info, (oal_uint32)puc_tim_ie[2],
        (oal_uint32)puc_tim_ie[3], BIT0);
...
OAL_STATIC oal_uint32 hmac_roam_send_auth_seq1(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    hmac_vap_stru   *pst_hmac_vap  = pst_roam_info->pst_hmac_vap;
    hmac_user_stru  *pst_hmac_user = pst_roam_info->pst_hmac_user;
    oal_netbuf_stru *pst_auth_frame = NULL;
    oal_uint8       *puc_auth_buff = NULL;
    mac_tx_ctl_stru *pst_tx_ctl = NULL;
    oal_uint8       *puc_my_mac_addr = NULL;
    ...
    /* 主状态机为非CONNECTING状态，失败 */
    if (pst_roam_info->en_main_state != ROAM_MAIN_STATE_CONNECTING) {
        OAM_ERROR_LOG1(uc_vap_id, OAM_SF_ROAM, "{hmac_roam_send_auth_seq1::main state[%d] error.}",
            pst_roam_info->en_main_state);
        return OAL_ERR_CODE_ROAM_STATE_UNEXPECT;
    }
    ...
    if (pst_roam_info->st_connect.en_state != ROAM_CONNECT_STATE_JOIN_COMP) {
        OAM_ERROR_LOG1(uc_vap_id, OAM_SF_ROAM, "{hmac_roam_send_auth_seq1::connect state[%d] error.}",
            pst_roam_info->st_connect.en_state);
        return OAL_ERR_CODE_ROAM_STATE_UNEXPECT;
    }
    ...
    memset_s(oal_netbuf_cb(pst_auth_frame), 48, 0, 48);
    memset_s(puc_auth_buff, MAC_80211_FRAME_LEN, 0, MAC_80211_FRAME_LEN);
...
#ifdef _PRE_WLAN_FEATURE_11R
    if (pst_hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_fast_bss_trans_cfg.en_dot11FastBSSTransitionActivated == OAL_TRUE) {
        /* FT System */
    ...
    if (ul_ret != OAL_SUCC) {
        ...
        OAM_ERROR_LOG1(uc_vap_id, OAM_SF_ROAM, "{hmac_roam_send_auth_seq1::hmac_tx_mgmt_send_event failed[%d].}",
            ul_ret);
        return ul_ret;
    }
...
OAL_STATIC oal_uint32 hmac_roam_send_reassoc_req(hmac_roam_info_stru *pst_roam_info)
{
    ...
    oal_netbuf_stru      *pst_assoc_req_frame = NULL;
    oal_uint8            *puc_assoc_buff = NULL;
    mac_tx_ctl_stru      *pst_tx_ctl = NULL;
    oal_uint8            *puc_my_mac_addr = NULL;
    ...
    mac_txbf_cap_stru    *pst_txbf_cap = NULL;
#endif
    if ((pst_hmac_vap == OAL_PTR_NULL) || (pst_hmac_user == OAL_PTR_NULL)) {
    ...
    memset_s(oal_netbuf_cb(pst_assoc_req_frame), 48, 0, 48);
    memset_s(puc_assoc_buff, MAC_80211_FRAME_LEN, 0, MAC_80211_FRAME_LEN);
...
#ifdef _PRE_WLAN_FEATURE_TXBF
    if ((pst_hmac_user->st_hmac_cap_info.bit_11ntxbf == OAL_TRUE) &&
        (pst_mac_vap->st_cap_flag.bit_11ntxbf == OAL_TRUE) && (uc_ie_len != 0)) {
    ...
    if (us_assoc_len <= OAL_ASSOC_REQ_IE_OFFSET) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ROAM, "{hmac_roam_send_assoc_req::unexpected assoc len[%d].}",
            us_assoc_len);
    ...
    pst_hmac_vap->puc_asoc_req_ie_buff = OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL,
        (oal_uint16)(us_assoc_len - OAL_ASSOC_REQ_IE_OFFSET - 6), OAL_TRUE);
    if (pst_hmac_vap->puc_asoc_req_ie_buff == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ROAM, "{hmac_roam_send_assoc_req::OAL_MEM_ALLOC len[%d] fail.}",
            us_assoc_len);
        oal_netbuf_free(pst_assoc_req_frame);
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }
    ...
    if (memcpy_s(pst_hmac_vap->puc_asoc_req_ie_buff, pst_hmac_vap->ul_asoc_req_ie_len,
                 OAL_NETBUF_HEADER(pst_assoc_req_frame) + OAL_ASSOC_REQ_IE_OFFSET + 6,
                 pst_hmac_vap->ul_asoc_req_ie_len) != EOK) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ROAM, "{hmac_roam_send_assoc_req::memcpy_s failed!");
        oal_netbuf_free(pst_assoc_req_frame);
        OAL_MEM_FREE(pst_hmac_vap->puc_asoc_req_ie_buff, OAL_TRUE);
        pst_hmac_vap->puc_asoc_req_ie_buff = OAL_PTR_NULL;
        return OAL_FAIL;
    ...
    if (ul_ret != OAL_SUCC) {
        ...
        OAL_MEM_FREE(pst_hmac_vap->puc_asoc_req_ie_buff, OAL_TRUE);
        pst_hmac_vap->puc_asoc_req_ie_buff = OAL_PTR_NULL;
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ROAM, "{hmac_roam_send_assoc_req::send_event fail[%d]}", ul_ret);
        return ul_ret;
    }
    return OAL_SUCC;
}
...
OAL_STATIC oal_uint32 hmac_roam_process_auth_seq2(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    if (pst_roam_info->uc_enable == 0) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_process_auth_seq2::roam disabled.}");
        return OAL_ERR_CODE_ROAM_DISABLED;
    }
    ...
    if (uc_frame_sub_type == WLAN_FC0_SUBTYPE_AUTH) {
        if (us_auth_seq_num == WLAN_AUTH_TRASACTION_NUM_TWO) {
            if (us_auth_state != MAC_SUCCESSFUL_STATUSCODE) {
                ul_ret = hmac_roam_connect_fsm_action(pst_roam_info,
                    ROAM_CONNECT_FSM_EVENT_MGMT_FAIL_RX, (oal_void *)OAL_PTR_NULL);
                if (ul_ret != OAL_SUCC) {
                    OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                        "{hmac_roam_process_auth_seq2::fsm_action FAIL[%d]!}", ul_ret);
                }
                return OAL_FAIL;
            }
        }
    } else if(uc_frame_sub_type == WLAN_FC0_SUBTYPE_DEAUTH ||
            uc_frame_sub_type == WLAN_FC0_SUBTYPE_DISASSOC) {
        /* 增加接收到去认证帧或者去关联帧时的维测信息 */
        mac_rx_get_sa((mac_ieee80211_frame_stru *)puc_mac_hdr, &puc_sa);
        OAM_WARNING_LOG4(0, OAM_SF_ROAM,
            "{hmac_roam_process_auth_seq2::Because of error_code[%d], received deauth or disassoc frame frome \
            source addr, sa [%02X:XX:XX:XX:%02X:%02X]}", *((oal_uint16 *)(puc_mac_hdr + MAC_80211_FRAME_LEN)),
            puc_sa[0], puc_sa[4], puc_sa[5]);

        ul_ret = hmac_roam_connect_fsm_action(pst_roam_info, ROAM_CONNECT_FSM_EVENT_MGMT_FAIL_RX,
    ...
    if ((uc_frame_sub_type != WLAN_FC0_SUBTYPE_AUTH) ||
        (us_auth_seq_num != WLAN_AUTH_TRASACTION_NUM_TWO)) {
        return OAL_SUCC;
    }
...
OAL_STATIC oal_uint32 hmac_roam_process_assoc_rsp(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    hmac_rx_ctl_stru            *pst_rx_ctrl = NULL;
    oal_uint8                   *puc_mac_hdr = NULL;
    oal_uint8                   *puc_payload = NULL;
    oal_uint8                   *puc_tmp_ie = NULL;
    ...
    /* mac地址校验 */
    mac_get_address3_host(puc_mac_hdr, auc_bss_addr, WLAN_MAC_ADDR_LEN);
    ...
    OAM_WARNING_LOG3(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
        "{hmac_roam_process_assoc_rsp::frame_sub_type = 0x%x, en_asoc_status = 0x%x, us_frame_len = %d}",
        uc_frame_sub_type, en_asoc_status, pst_rx_ctrl->st_rx_info.us_frame_len);

    if (uc_frame_sub_type == WLAN_FC0_SUBTYPE_ASSOC_RSP || uc_frame_sub_type == WLAN_FC0_SUBTYPE_REASSOC_RSP) {
        if (en_asoc_status != MAC_SUCCESSFUL_STATUSCODE) {
            ul_ret = hmac_roam_connect_fsm_action(pst_roam_info,
                ROAM_CONNECT_FSM_EVENT_MGMT_FAIL_RX, (oal_void *)OAL_PTR_NULL);
            if (ul_ret != OAL_SUCC) {
                OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                    "{hmac_roam_process_assoc_rsp::fsm_action FAIL[%d]!}", ul_ret);
            }
            return OAL_FAIL;
        }
    } else if(uc_frame_sub_type == WLAN_FC0_SUBTYPE_DEAUTH || uc_frame_sub_type == WLAN_FC0_SUBTYPE_DISASSOC) {
        /* 增加接收到去认证帧或者去关联帧时的维测信息 */
        mac_rx_get_sa((mac_ieee80211_frame_stru *)puc_mac_hdr, &puc_sa);
        OAM_WARNING_LOG4(0, OAM_SF_ROAM,
            "{hmac_roam_process_assoc_rsp::Because of error_code[%d], received deauth or disassoc frame frome \
            source addr, sa [%02X:XX:XX:XX:%02X:%02X]}",
    ...
    hmac_sta_up_update_edca_params(puc_payload, us_msg_len, us_ie_offset, pst_hmac_vap, uc_frame_sub_type,
        pst_hmac_user);
...
#ifdef _PRE_WLAN_FEATURE_TXBF
    /* 更新11n txbf能力 */
    puc_tmp_ie = mac_find_vendor_ie(MAC_HUAWEI_VENDER_IE, MAC_EID_11NTXBF, puc_payload + us_ie_offset,
        (oal_int32)(us_msg_len - us_ie_offset));
    hmac_mgmt_update_11ntxbf_cap(puc_tmp_ie, pst_hmac_user);
#endif

    /* 更新11ac VHT capabilities ie */
    memset_s(&(pst_hmac_user->st_user_base_info.st_vht_hdl), OAL_SIZEOF(mac_vht_hdl_stru), 0,
    ...
    if (g_uc_histream_switch == HISTREAM_SWITCH_ON) {
        /* 更新histream信息 */
        puc_tmp_ie = mac_find_vendor_ie(MAC_HUAWEI_VENDER_IE, MAC_HISI_HISTREAM_IE, puc_payload + us_ie_offset,
    ...
    OAM_WARNING_LOG3(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
        "{hmac_roam_process_assoc_rsp::mac_vap->en_protocol:%d,mac_user->en_protocol_mode:%d,\
        en_avail_protocol_mode:%d.}", pst_hmac_vap->st_vap_base_info.en_protocol,
        pst_hmac_user->st_user_base_info.en_protocol_mode, pst_hmac_user->st_user_base_info.en_avail_protocol_mode);
    ...
    ul_ret = hmac_user_set_avail_num_space_stream(&pst_hmac_user->st_user_base_info,
        pst_hmac_vap->st_vap_base_info.en_vap_rx_nss);
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
    ...
    if (puc_tmp_ie != OAL_PTR_NULL) {
        ul_ret = hmac_ie_proc_opmode_field(&(pst_hmac_vap->st_vap_base_info), &pst_hmac_user->st_user_base_info,
            (mac_opmode_notify_stru *)puc_tmp_ie, mac_get_frame_sub_type(puc_mac_hdr));
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG0(0, OAM_SF_ROAM,
                "{hmac_roam_process_assoc_rsp::hmac_ie_proc_opmode_field fail.return err code}");
        }
    }
#endif
    ...
    if (mac_mib_get_privacyinvoked(&pst_hmac_vap->st_vap_base_info) != OAL_TRUE) {
        /* 非加密情况下，漫游成功 */
...
#endif // _PRE_WLAN_FEATURE_11R
        if (mac_mib_get_rsnaactivated(&pst_hmac_vap->st_vap_base_info) == OAL_TRUE) {
...
OAL_STATIC oal_uint32 hmac_roam_connect_fail(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    /* connect信息初始化 */
    memset_s(pst_roam_info->st_connect.auc_bssid, WLAN_MAC_ADDR_LEN, 0, WLAN_MAC_ADDR_LEN);
...
oal_uint32 hmac_roam_connect_start(hmac_vap_stru *pst_hmac_vap, mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    l_ret += memcpy_s(pst_roam_info->st_connect.auc_bssid, WLAN_MAC_ADDR_LEN, pst_bss_dscr->auc_bssid,
        WLAN_MAC_ADDR_LEN);
    l_ret += memcpy_s(&(pst_roam_info->st_connect.st_channel), OAL_SIZEOF(mac_channel_stru),
        &(pst_bss_dscr->st_channel), OAL_SIZEOF(mac_channel_stru));
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ROAM, "{hmac_roam_connect_start::memcpy_s failed!}");
        return OAL_FAIL;
    }
...
oal_void hmac_roam_connect_rx_tbtt(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_connect_rx_tbtt::EVENT_TBTT FAIL[%d]!}", ul_ret);
    }
    return;
}
...
oal_uint32 hmac_roam_connect_ft_reassoc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_roam_info->uc_enable == 0) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_connect_ft_reassoc::roam disabled.}");
        return OAL_ERR_CODE_ROAM_DISABLED;
    }
