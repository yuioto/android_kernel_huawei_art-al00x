    ...
    hmac_tx_major_flow_stru *pst_max)
{
    ...
    if (memcpy_s(&pst_cfm_info->st_cfm_flow_info,
                 OAL_SIZEOF(hmac_tx_flow_info_stru),
                 &pst_max->st_flow_info,
                 OAL_SIZEOF(hmac_tx_flow_info_stru)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_tx_add_cfm_traffic::memcpy_s failed!");
        return OAL_FAIL;
    }
...
OAL_STATIC oal_uint32 hmac_tx_traffic_judge(hmac_user_stru *pst_hmac_user, hmac_tx_major_flow_stru *pst_major_flow,
    oal_uint8 *puc_tid)
{
...
OAL_STATIC oal_uint32 hmac_tx_find_major_traffic(hmac_user_stru *pst_hmac_user, oal_uint8 *puc_tid)
{
    ...
    memset_s(&st_mark, OAL_SIZEOF(hmac_tx_major_flow_stru), 0, OAL_SIZEOF(hmac_tx_major_flow_stru));
    memset_s(&st_max, OAL_SIZEOF(hmac_tx_major_flow_stru), 0, OAL_SIZEOF(hmac_tx_major_flow_stru));
    /* 队列超时，清空队列记录 */
    ...
    for (uc_cache_idx_i = 0; uc_cache_idx_i < (MAX_JUDGE_CACHE_LENGTH >> 1); uc_cache_idx_i++) {
        ...
        l_ret += memcpy_s(&st_mark, OAL_SIZEOF(hmac_tx_major_flow_stru), pst_judge_info,
        ...
        for (uc_cache_idx_j = 0; uc_cache_idx_j < MAX_JUDGE_CACHE_LENGTH; uc_cache_idx_j++) {
            ...
            if ((pst_judge_info->uc_flag == OAL_TRUE) && !oal_memcmp(&pst_judge_info->st_flow_info,
                &st_mark.st_flow_info, OAL_SIZEOF(hmac_tx_flow_info_stru))) {
                pst_judge_info->uc_flag   = OAL_FALSE;
            ...
            if (st_mark.ul_wait_check_num > st_max.ul_wait_check_num) {
                l_ret += memcpy_s(&st_max, OAL_SIZEOF(hmac_tx_major_flow_stru), &st_mark,
                    OAL_SIZEOF(hmac_tx_major_flow_stru));
                if (l_ret != EOK) {
                    OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_tx_find_major_traffic::memcpy_s failed!");
                    return OAL_FAIL;
                }
...
oal_void hmac_tx_traffic_classify(mac_tx_ctl_stru *pst_tx_ctl, mac_ip_header_stru *pst_ip, oal_uint8 *puc_tid)
{
    ...
    l_ret += memcpy_s(&(pst_judge_info->st_flow_info), OAL_SIZEOF(hmac_tx_flow_info_stru), &st_flow_info,
        OAL_SIZEOF(hmac_tx_flow_info_stru));

    pst_rtp_hdr = (hmac_tx_rtp_hdr *)(pst_udp_hdr + 1);                 /* 偏移一个UDP头，取RTP头 */
    pst_judge_info->uc_flag         = OAL_TRUE;

    pst_judge_info->ul_len          = OAL_NET2HOST_SHORT(pst_ip->us_tot_len) - OAL_SIZEOF(mac_ip_header_stru) -
    ...
    l_ret += memcpy_s(&(pst_judge_info->ul_rtpssrc), sizeof(oal_uint32), &(pst_rtp_hdr->ul_SSRC), sizeof(oal_uint32));
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_tx_traffic_classify::memcpy_s failed!");
        return ;
    }

    /* 若待识别队列已满，尝试提取队列主要业务并进行业务识别 */
    if (pst_judge_list->ul_to_judge_num >= MAX_JUDGE_CACHE_LENGTH) { /* "<=":防止进程并发使此值大于待识别队列长度而踩内存 */
