...
oal_uint32 hmac_wapi_calc_mic_data(mac_ieee80211_frame_stru *pst_mac_hdr, oal_uint8 uc_mac_hdr_len,
    oal_uint8 uc_keyidx, const oal_uint8 *puc_payload, oal_uint16 us_pdu_len,
    oal_uint8 *puc_mic, oal_uint16 us_mic_len)
{
    ...
    /* frame control */
    l_ret += memcpy_s(puc_mic, us_mic_len, (oal_uint8 *)&(pst_mac_hdr->st_frame_control), OAL_SIZEOF(pst_mac_hdr->st_frame_control));
    ...
    /* addr1 */
    mac_get_address1_host((oal_uint8 *)pst_mac_hdr, puc_mic, OAL_MAC_ADDR_LEN);
    puc_mic += OAL_MAC_ADDR_LEN;

    /* addr2 */
    mac_get_address2_host((oal_uint8 *)pst_mac_hdr, puc_mic, OAL_MAC_ADDR_LEN);
    puc_mic += OAL_MAC_ADDR_LEN;
    /* 序列控制 */
    ...
    /* addr3 */
    mac_get_address3_host((oal_uint8 *)pst_mac_hdr, puc_mic, WLAN_MAC_ADDR_LEN);
    puc_mic += OAL_MAC_ADDR_LEN;
    ...
    /* qos ctrl */
    if (hmac_wapi_is_qos(pst_mac_hdr) == OAL_TRUE) {
    ...
    *(puc_mic + 1) = (oal_uint8)(us_pdu_len & 0x00ff);
    ...
    /* 填充第2部分 */
    offset = (hmac_wapi_is_qos(pst_mac_hdr) == OAL_TRUE) ? SMS4_MIC_PART1_QOS_LEN : SMS4_MIC_PART1_NO_QOS_LEN;
    puc_mic_oringin += offset;
    l_ret += memcpy_s(puc_mic_oringin, us_mic_len - offset, puc_payload, us_pdu_len);
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_wapi_calc_mic_data::memcpy_s fail!");
        return OAL_FAIL;
    }
...
oal_bool_enum_uint8 hmac_wapi_is_pn_odd_ucast(const oal_uint8 *puc_pn)
{
    return (oal_bool_enum_uint8)(((*puc_pn & BIT0) == 0) ? OAL_FALSE : OAL_TRUE);
}


oal_uint8 hmac_wapi_is_pn_bigger(const oal_uint8 *puc_pn, const oal_uint8 *puc_pn_rx)
{
    oal_uint8   uc_pnidx;
...
oal_uint8 hmac_wapi_is_pn_odd_bcast(const oal_uint8 *puc_pn)
{
    return OAL_TRUE;
}
...
oal_uint32 hmac_wapi_add_key(hmac_wapi_stru *pst_wapi, oal_uint8 uc_key_index, const oal_uint8 *puc_key)
{
    ...
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_wapi_add_key::memcpy_s failed!");
        return OAL_FAIL;
    }
...
oal_uint32 hmac_wapi_decrypt(hmac_wapi_stru *pst_wapi, oal_netbuf_stru  *pst_netbuf)
{
    ...
    if (memcpy_s(pst_rx_ctl, MAC_TX_CTL_SIZE, OAL_NETBUF_CB(pst_netbuf), MAC_TX_CTL_SIZE) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_wpi_decrypt::memcpy_s fail!");
    ...
    /* 4.解密 */
    ul_wapi_result = hmac_wpi_decrypt(puc_pn, puc_pdu, (us_pdu_Len + SMS4_MIC_LEN),            /* 需要解密的数据长度 */
...
oal_uint32 hmac_wapi_encrypt(hmac_wapi_stru *pst_wapi, oal_netbuf_stru  *pst_netbuf)
{
    ...
    oal_uint8                       *puc_key_ek      = OAL_PTR_NULL;
    oal_uint8                        auc_calc_mic[SMS4_MIC_LEN];
    oal_uint8                        auc_pn_swap[SMS4_PN_LEN] = { 0 }; /* 保存变换后的pn,用来计算mic和加密 */
    oal_netbuf_stru                 *pst_netbuff_des = OAL_PTR_NULL;
    oal_uint8                       *puc_wapi_hdr    = OAL_PTR_NULL;
    ...
    OAM_WARNING_LOG2(0, OAM_SF_ANY, "{hmac_wapi_encrypt::uc_mac_hdr_len %u! bit_more==%u.}", uc_mac_hdr_len,
        pst_mac_hdr->st_frame_control.bit_more_frag);
    ...
    hmac_wapi_calc_mic_data(pst_mac_hdr, uc_mac_hdr_len, uc_key_index, puc_payload, us_pdu_Len, puc_mic_data,
        us_mic_len);
    puc_key_ck = pst_wapi->ast_wapi_key[uc_key_index].auc_wpi_ck;
    ...
    /* 计算mic */
    ul_result = hmac_wpi_pmac(auc_pn_swap, puc_mic_data, (us_mic_len >> 4), puc_key_ck, auc_calc_mic,
        sizeof(auc_calc_mic));
    hmac_wapi_mic_free(puc_mic_data);
    ...
    l_ret += memcpy_s(puc_datain, SMS4_MIC_LEN + us_pdu_Len, puc_payload, us_pdu_Len);
    /* 拷贝mic */
    l_ret += memcpy_s(puc_datain + us_pdu_Len, SMS4_MIC_LEN, auc_calc_mic, SMS4_MIC_LEN);
    puc_key_ek = pst_wapi->ast_wapi_key[uc_key_index].auc_wpi_ek;
    /* 4. 加密 */
    ul_result = hmac_wpi_encrypt(auc_pn_swap, puc_datain, us_pdu_Len + SMS4_MIC_LEN, puc_key_ek,
    ...
    *(puc_wapi_hdr + SMS4_KEY_IDX)  = 0;
    /* 填写PN */
    l_ret += memcpy_s((puc_wapi_hdr + SMS4_KEY_IDX + SMS4_WAPI_HDR_RESERVE), SMS4_PN_LEN, pst_wapi->ast_wapi_key[uc_key_index].auc_pn_tx, SMS4_PN_LEN);
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_wapi_encrypt::memcpy_s fail!");
        oal_netbuf_free(pst_netbuff_des);
        return OAL_FAIL;
    }
    /* 再次填写cb */
...
oal_void hmac_wapi_dump_frame(const oal_uint8 *puc_info, const oal_uint8 *puc_data, oal_uint32 ul_len)
{
    oal_uint32 ul_loop;
    OAL_IO_PRINT("%s: \r\n", puc_info);
    for (ul_loop = 0; ul_loop < ul_len; ul_loop += 4) {
        ...
        OAL_IO_PRINT("%2x %2x %2x %2x \r\n", puc_data[ul_loop], puc_data[ul_loop + 1],
                     puc_data[ul_loop + 2], puc_data[ul_loop + 3]); //lint !e679
    }
}
...
oal_uint32 hmac_wapi_display_info(mac_vap_stru *pst_mac_vap, oal_uint16 us_usr_idx)
{
    ...
    hmac_user_stru *pst_hmac_multi_user = OAL_PTR_NULL;
