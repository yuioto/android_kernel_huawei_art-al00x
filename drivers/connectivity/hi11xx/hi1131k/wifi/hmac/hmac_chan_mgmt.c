...
oal_uint32 hmac_send_ht_notify_chan_width(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_data)
{
    ...
    if ((pst_mac_vap == OAL_PTR_NULL) || (puc_data == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_2040, "{hmac_send_ht_notify_chan_width::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    /* 申请ht_notify_chan_width 帧空间 */
    pst_netbuf = (oal_netbuf_stru *)OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, MAC_80211_FRAME_LEN +
        MAC_HT_NOTIFY_CHANNEL_WIDTH_LEN, OAL_NETBUF_PRIORITY_MID);
    if (pst_netbuf == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_2040,
            "{hmac_send_ht_notify_chan_width::Alloc netbuf(size %d) null in normal_netbuf.}", MAC_80211_FRAME_LEN +
    ...
    if (ul_ret != OAL_SUCC) {
        ...
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_2040,
            "{hmac_send_ht_notify_chan_width::hmac_tx_mgmt_send_event failed[%d].}", ul_ret);
        return ul_ret;
    }
...
OAL_STATIC oal_void  hmac_chan_initiate_switch_to_new_channel(mac_vap_stru *pst_mac_vap, oal_uint8 uc_channel,
    wlan_channel_bandwidth_enum_uint8 en_bandwidth)
{
    ...
    OAM_INFO_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_2040, "{hmac_chan_initiate_switch_to_new_channel::"
        "uc_announced_channel=%d,en_announced_bandwidth=%d}", uc_channel, en_bandwidth);
    ...
    FRW_EVENT_HDR_INIT(&(pst_event->st_event_hdr), FRW_EVENT_TYPE_WLAN_CTX,
                       ...
                       OAL_SIZEOF(dmac_set_ch_switch_info_stru), FRW_EVENT_PIPELINE_STAGE_1,
...
oal_void hmac_dfs_set_channel(mac_vap_stru *pst_mac_vap, oal_uint8 uc_channel)
{
    ...
    pst_mac_device = mac_res_get_dev(pst_mac_vap->uc_device_id);
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_DFS, "{hmac_dfs_set_channel::pst_device(%d) null.}",
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_DFS,
            "{hmac_dfs_set_channel::mac_get_channel_idx_from_num failed[%d].}", ul_ret);
        return;
    }
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if ((pst_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) && (pst_vap->en_vap_state == MAC_VAP_STATE_UP)) {
            pst_mac_device->uc_csa_vap_cnt++;
            if (uc_channel == pst_vap->st_channel.uc_chan_number) {
                OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_DFS,
                    "{hmac_dfs_set_channel::same channel num %d, no need to change.}", uc_channel);
                return;
            }
...
oal_void  hmac_chan_do_sync(mac_vap_stru *pst_mac_vap, dmac_set_chan_stru *pst_set_chan)
{
    ...
    /* 更新VAP下的主20MHz信道号、带宽模式、信道索引 */
    ul_ret = mac_get_channel_idx_from_num(pst_mac_vap->st_channel.en_band, pst_set_chan->st_channel.uc_chan_number,
        &uc_idx);
    if (ul_ret != OAL_SUCC) {
    ...
    if (memcpy_s(frw_get_event_payload(pst_event_mem), OAL_SIZEOF(dmac_set_chan_stru), (oal_uint8 *)pst_set_chan,
        OAL_SIZEOF(dmac_set_chan_stru)) != EOK) {
    ...
    hmac_eval_scan_report_stru   *pst_chan_scan_report)
{
    ...
    for (uc_chan_idx = 0; uc_chan_idx < uc_num_supp_chan; uc_chan_idx++) {
        ...
        en_flag = hmac_chan_is_this_40MHz_set_possible(pst_mac_device, pst_mac_vap, uc_chan_idx, MAC_SCB,
            pst_chan_scan_report);
        if (en_flag == OAL_TRUE) {
            en_fortyMHz_poss = OAL_TRUE;
        }
    ...
    oal_uint8                     uc_num_supp_chan)
{
    ...
    memset_s(pst_chan_scan_report, uc_num_supp_chan * OAL_SIZEOF(*pst_chan_scan_report), 0,
        uc_num_supp_chan * OAL_SIZEOF(*pst_chan_scan_report));
    ...
    mac_sec_ch_off_enum_uint8     en_user_chan_offset)
{
    ...
    if ((mac_mib_get_2040BSSCoexistenceManagementSupport(pst_mac_vap) == OAL_TRUE) && (en_band == WLAN_BAND_2G)) {
    ...
    wlan_channel_bandwidth_enum_uint8   *pen_new_bandwidth)
{
    ...
    oal_uint16                  us_cumulative_networks;
    ...
    pst_chan_scan_report = (hmac_eval_scan_report_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL,
        uc_max_supp_channle * OAL_SIZEOF(*pst_chan_scan_report), OAL_TRUE);
    if (OAL_UNLIKELY(pst_chan_scan_report == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                       "{hmac_chan_select_channel_for_operation::pst_chan_scan_report memory alloc failed, size[%d].}",
                       (uc_max_supp_channle * OAL_SIZEOF(*pst_chan_scan_report)));
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_UNLIKELY(pst_mac_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                       "{hmac_chan_select_channel_for_operation::pst_mac_device null,device_id=%d.}",
                       pst_mac_vap->uc_device_id);
...
oal_void  hmac_chan_reval_status(mac_device_stru *pst_mac_device, mac_vap_stru *pst_mac_vap)
{
    ...
    if (OAL_UNLIKELY((pst_mac_device == OAL_PTR_NULL) || (pst_mac_vap == OAL_PTR_NULL))) {
        OAM_ERROR_LOG0(0, OAM_SF_SCAN, "{hmac_chan_reval_status::param null}");
        return;
    }
    ...
    if (uc_new_channel != pst_mac_vap->st_channel.uc_chan_number) {
    ...
    } else if (en_new_bandwidth != pst_mac_vap->st_channel.en_bandwidth) { /* 主20MHz信道不变，次20MHz or 40MHz信道改变 */
        ...
        OAM_INFO_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
            "{hmac_chan_reval_status::No Channel change after re-evaluation.}");
    }
}
...
oal_void  hmac_chan_update_csw_info(mac_vap_stru *pst_mac_vap, const oal_uint8 *puc_payload, oal_uint16 us_frame_len,
    oal_uint16 us_offset)
{
    ...
    if (en_csa_ie_present == OAL_FALSE) {
        if (pst_mac_vap->st_ch_switch_info.en_waiting_for_ap == OAL_TRUE) {
    ...
    oal_uint8                           *puc_end_ch_idx)
{
    ...
    if (en_user_pref_bandwidth == WLAN_BAND_WIDTH_40PLUS) {
        ...
        OAM_WARNING_LOG1(0, OAM_SF_SCAN, "{hmac_chan_get_affected_channels::invalid bandwidth[%d].}",
            en_user_pref_bandwidth);
    ...
    oal_uint8         *puc_end_ch_idx)
{
    ...
    if ((mac_mib_get_2040BSSCoexistenceManagementSupport(pst_mac_vap) == OAL_TRUE) && (en_band == WLAN_BAND_5G)) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
            "{hmac_chan_is_chan_scan_needed::20/40 MHz BSS only in 2.4G.}");
        return OAL_FALSE;
    }
#endif /* _PRE_WLAN_FEATURE_5G */
    ...
    if (mac_device_is_auto_chan_sel_enabled(pst_mac_device) == OAL_TRUE) {
        OAM_INFO_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
            "{hmac_chan_is_chan_scan_needed::Auto Channel Selection in progress.}");
        *puc_start_ch_idx = 0;
    ...
    if ((mac_mib_get_2040BSSCoexistenceManagementSupport(pst_mac_vap) == OAL_TRUE) &&
        ...
        ul_ret = hmac_chan_get_affected_channels(en_band, uc_user_ch_idx,
            hmac_chan_get_user_pref_bandwidth(pst_mac_vap), puc_start_ch_idx, puc_end_ch_idx);
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
...
oal_uint32  hmac_chan_start_bss(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (mac_get_acs_switch(pst_mac_device) == MAC_ACS_SW_INIT ||
        ...
        if (!pst_hmac_device ||
            pst_hmac_device->ast_best_channel[pst_mac_vap->st_channel.en_band].uc_chan_number == 0) {
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                "{hmac_chan_start_bss::init acs enabled,best channel of BAND%d is 0", pst_mac_vap->st_channel.en_band);
        } else {
    ...
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
            "{hmac_chan_start_bss::hmac_config_send_event failed[%d].}", ul_ret);
        return ul_ret;
    }
    ...
    oal_bool_enum_uint8    en_first_time)
{
    ...
    oal_uint8                       uc_channel = 0;
    oal_bool_enum_uint8             en_need_for_scan;
    ...
    for (; ;) {
        if (en_first_time) {
            ...
            /* 清信道扫描结果列表 */
            memset_s(pst_mac_device->st_ap_channel_list, OAL_SIZEOF(pst_mac_device->st_ap_channel_list), 0,
                OAL_SIZEOF(pst_mac_device->st_ap_channel_list));
        ...
        if (hmac_chan_scan_availability(pst_mac_device,
            &pst_mac_device->st_ap_channel_list[pst_mac_device->uc_ap_chan_idx]) == OAL_TRUE) {
            /* 切换至当前信道 */
...
oal_uint32  hmac_ie_proc_ch_switch_ie(mac_vap_stru *pst_mac_vap, const oal_uint8 *puc_payload,
                                      mac_eid_enum_uint8 en_eid_type)
{
    ...
    if (pst_mac_vap->st_ch_switch_info.en_waiting_to_shift_channel == OAL_TRUE) {
    ...
    if (ul_check != OAL_SUCC) {
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_ie_proc_ch_switch_ie::mac_is_channel_num_valid failed[%d], uc_new_chan=%d.}", ul_check, uc_new_chan);
        return ul_check;
    }
...
oal_uint32  hmac_ie_proc_wide_bandwidth_ie(mac_vap_stru *pst_mac_vap, const oal_uint8 *puc_payload)
{
    ...
    if (OAL_UNLIKELY((pst_mac_vap == OAL_PTR_NULL) || (puc_payload == OAL_PTR_NULL))) {
        MAC_ERR_LOG(0, "hmac_ie_proc_wide_bandwidth_ie, pst_mac_vap or puc_payload is null.");
    ...
    if (ul_check != OAL_SUCC) {
        ...
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{mac_ie_proc_wide_bandwidth_ie::mac_is_channel_num_valid"
            "failed[%d], uc_new_chan=%d.}", ul_check, uc_new_channel);
        return ul_check;
    }

    /* IE长度检查 MAC_WIDE_BW_CH_SWITCH_IE_LEN:3 */
    if (puc_payload[1] < MAC_WIDE_BW_CH_SWITCH_IE_LEN) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{hmac_ie_proc_wide_bandwidth_ie::invalid wide bw ch switch ie len[%d]}",
            puc_payload[1]);
        return OAL_FAIL;
    }
    ...
    if (uc_channel_width >= 1) {
        ...
        pst_mac_vap->st_ch_switch_info.en_new_bandwidth = mac_get_bandwith_from_center_freq_seg0(uc_new_channel,
            uc_channel_center_freq_seg0);
    }
...
oal_bool_enum_uint8  hmac_ie_check_csa_ie(mac_vap_stru *pst_mac_vap, const oal_uint8 *puc_payload,
    oal_uint16 us_frame_len, oal_uint16 us_offset)
{
    ...
    while (us_index < us_frame_len) {
        if (puc_payload[us_index] == MAC_EID_CHANSWITCHANN) {
            us_ch_index = us_index;
        } else if (puc_payload[us_index] == MAC_EID_EXTCHANSWITCHANN) {
            us_ch_index = us_index;
            break;
        }
