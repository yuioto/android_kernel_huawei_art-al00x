...
oal_uint32  hmac_config_set_acs_cmd(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, const oal_uint8 *puc_param);
...
oal_uint32 hmac_init_event_process(frw_event_mem_stru *pst_event_mem)
{
    ...
    oal_uint8             *pst_data_mac_addr = OAL_PTR_NULL;
    ...
    pst_data_mac_addr = (oal_uint8*)(oal_netbuf_data(pst_ctx_event->pst_netbuf)) +
    ...
    l_ret += memcpy_s((oal_uint8 *)(pst_mac_device->st_mac_rates_11g),
                      sizeof(pst_mac_device->st_mac_rates_11g),
                      (oal_uint8 *)pst_data_rate,
                      sizeof(mac_data_rate_stru) * MAC_DATARATES_PHY_80211G_NUM);
    l_ret += memcpy_s((oal_uint8 *)g_auc_hmac_macaddr, WLAN_MAC_ADDR_LEN, (oal_uint8 *)pst_data_mac_addr,
                      WLAN_MAC_ADDR_LEN);
    if (l_ret != EOK) {
...
OAL_STATIC oal_void hmac_event_fsm_tx_adapt_subtable_register_main_function(oal_void)
{
#ifdef _PRE_WLAN_FEATURE_OPMODE_NOTIFY
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_PSM_OPMODE_NOTIFY].p_tx_adapt_func =
        hmac_hcc_tx_convert_event_to_netbuf_uint16;
#endif
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_SET_RX_FILTER].p_tx_adapt_func =
    ...
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPR_SWITCH_TO_HOME_CHAN].p_tx_adapt_func =
        hmac_hcc_tx_convert_event_to_netbuf_uint16;
#endif
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPR_DFS_TEST].p_tx_adapt_func =
        hmac_hcc_tx_convert_event_to_netbuf_uint16;
#endif
...
{
    /* 注册WLAN_CTX事件处理函数表 */
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_ACTION].p_tx_adapt_func =
    ...
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_MGMT].p_tx_adapt_func =
    ...
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_DPD_DATA_PROCESSED].p_tx_adapt_func =
        hmac_dpd_data_processed_event_tx_adapt;
#endif
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_CALI_HMAC2DMAC].p_tx_adapt_func =
        hmac_send_cali_data_tx_adapt;
    ...
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_CONN_RESULT].p_tx_adapt_func =
        hmac_hcc_tx_convert_event_to_netbuf_uint32;
    hmac_event_fsm_tx_adapt_subtable_register_main_function();
#ifdef _PRE_WLAN_FEATURE_TCP_UDP_KEEPALIVE
    g_ast_dmac_wlan_ctx_event_sub_table[DMAC_TX_HOST_DATA_FRAME_FILTER].p_tx_adapt_func =
        hmac_send_event_netbuf_tx_adapt;
#endif /* liuming add support keep alive code for 1102 */ /* 注册WLAN_DTX事件处理函数表 */
}
...
OAL_STATIC oal_void hmac_event_fsm_rx_adapt_subtable_register(oal_void)
{
...
#ifdef _PRE_WLAN_FEATURE_FLOWCTL
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_FLOWCTL_BACKP].p_rx_adapt_func =
    ...
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_EVERY_SCAN_RESULT].p_rx_adapt_func =
        hmac_rx_convert_netbuf_to_netbuf_default;
#ifdef  _PRE_WLAN_RF_110X_CALI_DPD
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_TO_HMAC_DPD_CALIBRATED_DATA_SEND].p_rx_adapt_func =
        hmac_rx_convert_netbuf_to_netbuf_default;
#endif
    ...
    g_ast_hmac_wlan_misc_event_sub_table[DMAC_MISC_SUB_TYPE_DEV_SYNC_HOST].p_rx_adapt_func =
...
OAL_STATIC void hmac_event_fsm_action_subtable_register_sub(void)
{
    /* 注册HMAC模块WLAN_CRX事件子表 */
    ...
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_OBSS_SCAN_COMP].p_func =
        hmac_scan_proc_obss_scan_comp_event;
#endif
...
OAL_STATIC oal_void hmac_event_fsm_action_subtable_register(oal_void)
{
    ...
    hmac_event_fsm_action_subtable_register_sub();
    /* 注册TBTT事件子表 */
...
#endif
#ifdef _PRE_WLAN_FEATURE_APF
    g_ast_hmac_wlan_misc_event_sub_table[DMAC_MISC_SUB_TYPE_DEV_SYNC_HOST].p_func = hmac_apf_program_report_event;
#endif
}
...
oal_int32 hmac_wakelock_info_print(char* buf, oal_int32 buf_len)
{
    ...
    if (g_st_hmac_wakelock.locked_addr) {
        ret +=  snprintf_s(buf + ret, buf_len - ret, (buf_len - ret)-1, "wakelocked by:%lu\n",
                           g_st_hmac_wakelock.locked_addr);
    }
#endif
...
OAL_STATIC ssize_t hmac_get_rxthread_info(struct kobject *dev, struct kobj_attribute *attr, char* buf)
{
    ...
    ret +=  snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1,
        ...
        g_st_rxdata_thread.en_rxthread_enable, oal_netbuf_list_len(&g_st_rxdata_thread.st_rxdata_netbuf_head),
...
OAL_STATIC struct kobj_attribute dev_attr_rxdata_info =
    __ATTR(rxdata_info, (S_IRUGO | S_IWUSR), hmac_get_rxthread_info, hmac_set_rxthread_enable); //lint !e64
...
{
    if (oal_netbuf_list_len(&g_st_rxdata_thread.st_rxdata_netbuf_head) > 1000) {
...
OAL_STATIC oal_void hmac_hisi_thread_exit(oal_void)
{
    ...
    if (hcc_get_default_handler()->hcc_transer_info.hcc_transfer_thread != NULL) {
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    if (g_st_rxdata_thread.pst_rxdata_thread != NULL) {
...
OAL_STATIC oal_uint32 hmac_hisi_thread_init(oal_void)
{
    ...
    oal_kthread_param_stru st_thread_rxdata_param = {0};
#endif
    ...
    hcc_get_default_handler()->hcc_transer_info.hcc_transfer_thread =
        oal_kthread_create("hisi_hcc", hcc_transfer_thread, (void*)hcc_get_default_handler(), &st_thread_param);
    ...
    memset_s(&st_thread_rxdata_param, OAL_SIZEOF(oal_kthread_param_stru), 0, OAL_SIZEOF(oal_kthread_param_stru));
    st_thread_rxdata_param.l_cpuid      = NOT_BIND_CPU;
    st_thread_rxdata_param.l_policy     = HCC_TRANS_RXDATA_THREAD_POLICY;
    st_thread_rxdata_param.l_prio       = HCC_TRANS_RXDATA_THERAD_PRIORITY;
    st_thread_rxdata_param.ul_stacksize = HCC_TRANS_RXDATA_THERAD_STACKSIZE;
    g_st_rxdata_thread.pst_rxdata_thread =
        oal_kthread_create("hisi_rxdata", hmac_rxdata_thread, NULL, &st_thread_rxdata_param);
                                  ...
                                  oal_uint16       us_len)
{
    ...
    if (memcpy_s(pst_event->auc_event_data, us_len - OAL_IF_NAME_SIZE + 1,
...
OAL_STATIC oal_uint32  hmac_syn_info_event(frw_event_mem_stru  *pst_event_mem)
{
    ...
    PS_PRINT_WARNING("device sync data to host,protocol %d to %d\n", pst_mac_user->en_cur_protocol_mode,
        pst_syn_info_event->uc_cur_protocol);
#endif
...
oal_uint32 hmac_config_set_acs_cmd(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, const oal_uint8 *puc_param)
{
    ...
    if ((pst_hmac_device == OAL_PTR_NULL) || pst_hmac_device->pst_device_base_info == OAL_PTR_NULL ||
        ...
        OAM_ERROR_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_config_set_acs_cmd::null param,pst_hmac_device=%x puc_param=%x}", pst_hmac_device, puc_param);
        return OAL_ERR_CODE_PTR_NULL;
    }

    pst_mac_device = pst_hmac_device->pst_device_base_info;

    if (pst_mac_device == OAL_PTR_NULL || pst_mac_device->pst_acs == OAL_PTR_NULL) {
    ...
    } else if (pst_acs_cmd->uc_cmd == DMAC_ACS_CMD_SET_BEST_CHANNEL && pst_acs_cmd->auc_arg[0] == MAC_ACS_SET_CH_INIT) {
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_config_set_acs_cmd::hmac_config_send_event failed[%d].}", ul_ret);
    }
...
OAL_STATIC oal_uint32  hmac_flowctl_backp_event(frw_event_mem_stru  *pst_event_mem)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_flowctl_backp_event::frw_event_dispatch_event null.}", ul_ret);
        return ul_ret;
    }
