                                         ...
                                         oal_uint32 ul_max_tx_unit)
{
    ...
    do {
        ...
        if (memcpy_s(pst_tx_ctl_copy, MAC_TX_CTL_SIZE, pst_tx_ctl, MAC_TX_CTL_SIZE) != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_frag_process::memcpy_s fail!");
        ...
        pst_frag_header = (mac_ieee80211_frame_stru *)(OAL_NETBUF_PAYLOAD(pst_netbuf) -
            MAC_80211_QOS_HTC_4ADDR_FRAME_LEN);
        pst_tx_ctl_copy->bit_80211_mac_head_type = 1;  /* 指示mac头部在skb中 */

        /* 拷贝帧头内容 */
        if (memcpy_s(pst_frag_header, MAC_80211_QOS_HTC_4ADDR_FRAME_LEN,
                     pst_mac_header, pst_tx_ctl->uc_frame_header_length) != EOK) {
        ...
        oal_netbuf_copydata(pst_netbuf_original, (ul_offset + ul_mac_hdr_size), OAL_NETBUF_PAYLOAD(pst_netbuf),
            ul_payload);
                                         ...
                                         oal_uint32 ul_max_tx_unit)
{
    ...
    do {
        ...
        pst_tx_ctl_copy = (mac_tx_ctl_stru *)OAL_NETBUF_CB(pst_netbuf);
        /* 拷贝cb字段 */
        if (memcpy_s(pst_tx_ctl_copy, (ul_frag_size + MAC_80211_QOS_HTC_4ADDR_FRAME_LEN),
                     pst_tx_ctl, MAC_TX_CTL_SIZE) != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_frag_process::memcpy fail!");
            oal_netbuf_free(pst_netbuf);
            return OAL_FAIL;
        }
        ...
        /* 拷贝帧头内容 */
        if (memcpy_s(pst_frag_header, pst_tx_ctl->uc_frame_header_length, pst_mac_header, pst_tx_ctl->uc_frame_header_length) != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_frag_process::memcpy fail!");
            oal_netbuf_free(pst_netbuf);
            return OAL_FAIL;
        }
    ...
    oal_netbuf_stru *pst_netbuf, mac_tx_ctl_stru *pst_tx_ctl)
{
    ...
    if (pst_hmac_user->st_user_base_info.st_key_info.en_cipher_type == WLAN_80211_CIPHER_SUITE_TKIP) {
        uc_last_frag = OAL_NETBUF_LEN(pst_netbuf) %
            (ul_threshold - (oal_uint32)uc_ic_header - pst_tx_ctl->uc_frame_header_length);
        if ((uc_last_frag > 0) && (uc_last_frag <= 8)) {
            ...
            OAM_INFO_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                "{hmac_frag_process_proc::adjust the frag threshold to %d.}", ul_threshold);
        }
    }
#endif
    /* 进行分片处理 */
...
oal_netbuf_stru* hmac_defrag_process(hmac_user_stru *pst_hmac_user, oal_netbuf_stru *pst_netbuf, oal_uint32 ul_hrdsize)
{
    ...
    oal_uint16                us_last_seq;
    oal_uint8                 uc_frag_num;
    oal_uint8                 uc_last_frag_num;
    ...
    if (pst_hmac_user->pst_defrag_netbuf == OAL_PTR_NULL) {
        ...
        FRW_TIMER_CREATE_TIMER(&pst_hmac_user->st_defrag_timer,
            hmac_defrag_timeout_fn, HMAC_FRAG_TIMEOUT, pst_hmac_user, OAL_FALSE, OAM_MODULE_ID_HMAC, ul_core_id);
#ifdef _PRE_LWIP_ZERO_COPY
        pst_new_buf = oal_pbuf_netbuf_alloc(HMAC_MAX_FRAG_SIZE);
#else
        ...
        if (memcpy_s(OAL_NETBUF_CB(pst_new_buf), MAC_TX_CTL_SIZE, OAL_NETBUF_CB(pst_netbuf), MAC_TX_CTL_SIZE) != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_defrag_process::memcpy_s failed!");
            oal_netbuf_free(pst_netbuf);
            return OAL_PTR_NULL;
        }
        ...
        if (OAL_UNLIKELY(pst_last_hdr == OAL_PTR_NULL)) {
            OAM_ERROR_LOG0(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ANY,
                "{hmac_defrag_process::pst_last_hdr is NULL.}");
            return OAL_PTR_NULL;
        }
    ...
    if (en_more_frag) {
        ...
        if (ul_ret != OAL_SUCC) {
            ...
            OAM_INFO_LOG1(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ANY,
