...
int hw_sensor_get_dt_data(struct platform_device *pdev, sensor_t *sensor)
{
	...
	rc = of_property_read_string(of_np, "huawei,sensor_name",
		&sensor_info->name);
	cam_debug("%s huawei,sensor_name %s, rc %d", __FUNCTION__,
		sensor_info->name, rc);
	if (rc < 0) {
		cam_err("%s failed %d", __FUNCTION__, __LINE__);
		goto fail;
	...
	rc = of_property_read_u32(of_np, "huawei,is_fpga", (u32*)&g_is_fpga);
	cam_debug("%s huawei,is_fpga: %d, rc %d", __FUNCTION__, g_is_fpga, rc);
	if (rc < 0) {
		cam_err("%s failed %d", __FUNCTION__, __LINE__);
		goto fail;
	}
	...
	cam_debug("%s huawei,sensor_index %d, rc %d", __FUNCTION__,
	...
	cam_debug("%s huawei,sensor_spec %s, rc %d", __FUNCTION__,
		sensor_info->sensor_spec, rc);
	if (rc < 0 || !(sensor_info->sensor_spec) ||
		*(sensor_info->sensor_spec) == '\0') {
		cam_err("%s no huawei,sensor_spec config", __FUNCTION__);
	} else {
	...
	cam_debug("%s huawei,pd_valid %d, rc %d", __FUNCTION__,
	...
	cam_debug("%s huawei,reset_valid %d, rc %d", __FUNCTION__,
	...
	cam_debug("%s huawei,vcmpd_valid %d, rc %d", __FUNCTION__,
	...
	rc = of_property_read_u32(of_np, "huawei,mclk",
		&sensor_info->mclk);
	cam_debug("%s huawei,mclk 0x%x, rc %d", __FUNCTION__,
		sensor_info->mclk, rc);
	if (rc < 0) {
		cam_err("%s failed %d", __FUNCTION__, __LINE__);
		goto fail;
	...
	struct sensor_power_setting_array *power_setting_array, int is_power_on)
{
	...
	seq_type_name = (is_power_on ? "huawei,cam-power-seq-type" :
	...
	seq_val_name = (is_power_on ? "huawei,cam-power-seq-val" :
	...
	seq_cfg_name = (is_power_on ? "huawei,cam-power-seq-cfg-val" :
	...
	seq_delay_name = (is_power_on ? "huawei,cam-power-seq-delay" :
