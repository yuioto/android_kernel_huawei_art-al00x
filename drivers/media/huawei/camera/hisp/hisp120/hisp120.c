...
static int hisp120_unmap_a7isp_addr(void *cfg)
{
	...
	if (cfg == NULL) {
		("func %s: cfg is NULL", __FUNCTION__);
		return -1;
	}
...
static int hisp120_alloc_r8isp_addr(void *cfg)
{
	...
	if (pcfg->param.type == MAP_TYPE_STATIC) {
		...
		return -ENODEV;
#else
		cam_info("func %s static", __FUNCTION__);
	...
	r8_iova = (unsigned int)hisp_mem_pool_alloc_iova(pcfg->param.size,
		(unsigned int)ipool);
	if (!r8_iova) {
		cam_err("func %s: hisp_mem_pool_alloc_iova error", __FUNCTION__);
		rc = -ENOMEM;
		goto alloc_err;
	}
...
static int hisp120_config(hisp_intf_t *i, void *cfg)
{
	...
	case HISP_CONFIG_POWER_ON:
		...
		if (!remote_processor_up) {
			...
			cam_notice("%s power on the hisp120. ", __FUNCTION__);
			rc = hisp120_power_on(i);
		} else {
			cam_warn("%s hisp120 is still on power-on state,"
	...
	case HISP_CONFIG_POWER_OFF:
		...
		mutex_unlock(&hisp_lowpower_mode_mutex);
		if (remote_processor_up) {
			cam_notice("%s power off the hisp120", __FUNCTION__);
			rc = hisp120_power_off(i);
		}
		break;
	case HISP_CONFIG_GET_MAP_ADDR:
		...
		cam_notice("%s get a7 map address 0x%x",
			__FUNCTION__, pcfg->param.moduleAddr);
		break;
...
static int hisp120_power_on(hisp_intf_t *i)
{
	...
	hisi_serv->ept = rpmsg_create_ept(hisi_serv->rpdev,
		hisp120_rpmsg_ept_cb, hisi_serv, chinfo);
...
{
	cam_info("enter %s,current_ddr_bandwidth:%d ", __FUNCTION__,
